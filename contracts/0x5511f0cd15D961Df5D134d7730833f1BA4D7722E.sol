...
function sub_38c33314(?) {
    ...
    require arg1 / 10 / 3 >= 0
    require 430000 * arg2 >= 0
    if (430000 * arg2) + (arg1 / 10 / 3) + ext_call.return_data[0] + (ext_call.return_data[0] / 10 / 3) <= arg1:
        return 0
    return 1
}
...
function run() payable {
    ...
    require 430000 * block.gasprice >= 0
    if (430000 * block.gasprice) + (msg.value / 10 / 3) + ext_call.return_data[0] + (ext_call.return_data[0] / 10 / 3) <= msg.value:
        revert with 0, 'Will not make a profit'
