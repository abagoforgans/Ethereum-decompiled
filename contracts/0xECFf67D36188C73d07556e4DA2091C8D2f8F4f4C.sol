contract main {


// =======================  Init code  ======================


uint256 stor0;
uint256 stor1;
uint256 stor2;
uint256 stor3;
uint256 stor4;
uint256 stor5;
uint256 stor6;
uint256 stor7;
uint256 stor8;

function _fallback() payable {
    stor0 = 0
    stor1 = 0
    stor2 = 0
    stor3 = 0
    stor4 = 1100
    stor5 = 0
    stor6 = 20
    stor7 = 30
    stor8 = msg.sender or Mask(96, 160, stor8)
    return code.data[75 len 1767]
}



// =====================  Runtime code  =====================


uint256 stor0;
uint256 stor1;
uint256 watchLastPayout;
uint256 watchWinningPot;
uint256 stor4;
uint256 watchFees;
uint256 stor6;
uint256 stor7;
address stor8;
uint256 stor8;
array of struct stor9;
array of uint256 stor49791959467252497455735130940088646708311117250336157395101362029847983278000;
array of uint8 stor49791959467252497455735130940088646708311117250336157395101362029847983278001;

function WatchFees() payable {
    return watchFees
}

function WatchWinningPot() payable {
    return watchWinningPot
}

function WatchLastPayout() payable {
    return watchLastPayout
}

function Total_of_Players() payable {
    return stor9.length
}

function WatchBalance() payable {
    return (stor0 / 10^15)
}

function PayoutQueueSize() payable {
    return (stor9.length - stor1)
}

function NextPayout() payable {
    require stor1 < stor9.length
    return (stor[code.data[1735 len 32] + (3 * stor1)] / 10^15)
}

function ChangeOwnership(address arg1) payable {
    if msg.sender == address(stor8):
        uint256(stor8) = arg1 or Mask(96, 160, uint256(stor8))
}

function PlayerInfo(uint256 arg1) payable {
    if arg1 > stor9.length:
        return 0
    require arg1 < stor9.length
    return address(stor9[arg1].field_0), stor9[arg1].field_256 / 10^15, stor6E15[arg1]
}

function CollectAllFees() payable {
    if msg.sender == address(stor8):
        require watchFees != 0
        call address(stor8) with:
           value watchFees wei
             gas 0 wei
        stor6--
        watchFees = 0
}

function GetAndReduceFeesByFraction(uint256 arg1) payable {
    if msg.sender == address(stor8):
        if 0 == watchFees:
            stor6--
        call address(stor8) with:
           value arg1 * watchFees / 1000 wei
             gas 0 wei
        watchFees += -1 * arg1 * watchFees / 1000
}

function _fallback() payable {
    if msg.value < 10^15:
        call msg.sender with:
           value msg.value wei
             gas 0 wei
    if msg.value <= 10^17:
        stor9.length++
        if not stor9.length <= stor9.length + 1:
            idx = (3 * stor9.length) + 3
            while 3 * stor9.length > idx:
                address(stor9[idx].field_0) = 0
                stor9[idx].field_256 = 0
                uint8(stor9[idx].field_512) = 0
                idx = idx + 1
                continue 
        stor9[stor9.length].field_0 = msg.sender or Mask(96, 160, stor9[stor9.length].field_0)
        if stor0 >= 3 * 10^15:
            if stor9.length % 10 != 0:
                stor9[stor9.length].field_256 = msg.value * stor4 / 1000
                uint8(stor9[stor9.length].field_512) = 0
                watchWinningPot += msg.value * stor7 / 1000
                watchFees += msg.value * stor6 / 1000
                stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                if msg.value <= 10^16:
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
                else:
                    require stor1 < stor9.length
                    if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
            else:
                if stor9.length <= 1:
                    stor9[stor9.length].field_256 = msg.value * stor4 / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    if msg.value <= 10^16:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        require stor1 < stor9.length
                        if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                        else:
                            if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                                call msg.sender with:
                                   value watchWinningPot wei
                                     gas 0 wei
                                watchWinningPot = 0
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                else:
                    stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    if msg.value <= 10^16:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        require stor1 < stor9.length
                        if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                        else:
                            if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                                call msg.sender with:
                                   value watchWinningPot wei
                                     gas 0 wei
                                watchWinningPot = 0
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
        else:
            if stor9.length % 10 != 0:
                stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                uint8(stor9[stor9.length].field_512) = 0
                watchWinningPot += msg.value * stor7 / 1000
                watchFees += msg.value * stor6 / 1000
                stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                if msg.value <= 10^16:
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
                else:
                    require stor1 < stor9.length
                    if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
            else:
                if stor9.length <= 1:
                    stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    if msg.value <= 10^16:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        require stor1 < stor9.length
                        if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                        else:
                            if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                                call msg.sender with:
                                   value watchWinningPot wei
                                     gas 0 wei
                                watchWinningPot = 0
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                else:
                    stor9[stor9.length].field_256 = (200 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    if msg.value <= 10^16:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        require stor1 < stor9.length
                        if msg.value <= stor[code.data[1735 len 32] + (3 * stor1)]:
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
                        else:
                            if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                                call msg.sender with:
                                   value watchWinningPot wei
                                     gas 0 wei
                                watchWinningPot = 0
                            if stor0 <= stor6E15[stor1]:
                            require stor1 < stor9.length
                            # nil
    else:
        call msg.sender with:
           value msg.value - 10^17 wei
             gas 0 wei
        stor9.length++
        if not stor9.length <= stor9.length + 1:
            idx = (3 * stor9.length) + 3
            while 3 * stor9.length > idx:
                address(stor9[idx].field_0) = 0
                stor9[idx].field_256 = 0
                uint8(stor9[idx].field_512) = 0
                idx = idx + 1
                continue 
        stor9[stor9.length].field_0 = msg.sender or Mask(96, 160, stor9[stor9.length].field_0)
        if stor0 >= 3 * 10^15:
            if stor9.length % 10 != 0:
                stor9[stor9.length].field_256 = msg.value * stor4 / 1000
                uint8(stor9[stor9.length].field_512) = 0
                watchWinningPot += msg.value * stor7 / 1000
                watchFees += msg.value * stor6 / 1000
                stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                require stor1 < stor9.length
                if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
                else:
                    if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                        call msg.sender with:
                           value watchWinningPot wei
                             gas 0 wei
                        watchWinningPot = 0
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
            else:
                if stor9.length <= 1:
                    stor9[stor9.length].field_256 = msg.value * stor4 / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    require stor1 < stor9.length
                    if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                else:
                    stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    require stor1 < stor9.length
                    if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
        else:
            if stor9.length % 10 != 0:
                stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                uint8(stor9[stor9.length].field_512) = 0
                watchWinningPot += msg.value * stor7 / 1000
                watchFees += msg.value * stor6 / 1000
                stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                require stor1 < stor9.length
                if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
                else:
                    if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                        call msg.sender with:
                           value watchWinningPot wei
                             gas 0 wei
                        watchWinningPot = 0
                    if stor0 <= stor6E15[stor1]:
                    require stor1 < stor9.length
                    # nil
            else:
                if stor9.length <= 1:
                    stor9[stor9.length].field_256 = (100 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    require stor1 < stor9.length
                    if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                else:
                    stor9[stor9.length].field_256 = (200 * msg.value) + (stor4 * msg.value) / 1000
                    uint8(stor9[stor9.length].field_512) = 0
                    watchWinningPot += msg.value * stor7 / 1000
                    watchFees += msg.value * stor6 / 1000
                    stor0 += (1000 * msg.value) - (stor7 * msg.value) - (stor6 * msg.value) / 1000
                    require stor1 < stor9.length
                    if 10^17 <= stor[code.data[1735 len 32] + (3 * stor1)]:
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
                    else:
                        if 0 == (block.hash((block.timestamp * block.number / block.timestamp % 5) + (block.timestamp % 300) + (block.number / 3) + watchLastPayout) / 100 * block.timestamp / 100 % 100) + 1 % 5:
                            call msg.sender with:
                               value watchWinningPot wei
                                 gas 0 wei
                            watchWinningPot = 0
                        if stor0 <= stor6E15[stor1]:
                        require stor1 < stor9.length
                        # nil
}



}
