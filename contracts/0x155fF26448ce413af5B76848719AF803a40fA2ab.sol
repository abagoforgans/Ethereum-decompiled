...
function creditorAmounts(uint256 arg1) payable {
    ...
    return uint256(stor[code.data[2264 len 32] + arg1])
}
...
function totalPayedOut() payable {
    ...
    while idx < lastCreditorPayedOut:
        ...
        mem[0] = 4
        idx = idx + 1
        s = s + uint256(stor[idx + code.data[2264 len 32]])
        continue 
...
function lendGovernmentMoney(address arg1) payable {
    if lastTimeOfNewCredit + (12 * 3600) < block.timestamp:
        ...
        while stor3.length + 0x56c2575a0e9e593c00f959f8c92f12db2869c3395a3b0502d05e2516446f71f8 > idx:
            ...
            continue 
        stor4.length = 0
        idx = code.data[2264 len 32]
        while code.data[2264 len 32] + stor4.length > idx:
            uint256(stor[idx]) = 0
            idx = idx + 1
            continue 
    ...
    if uint256(stor[code.data[2264 len 32] + uint32(stor0.field_0)]) <= eth.balance(this.address) - profitFromCrash:
        require lastCreditorPayedOut < stor3.length
        require lastCreditorPayedOut < stor4.length
        call creditor[uint32(stor0.field_0)] with:
           value uint256(stor[code.data[2264 len 32] + uint32(stor0.field_0)]) wei
             gas 0 wei
        require lastCreditorPayedOut < stor3.length
        stor6[stor56C2[uint32(stor0.field_0)]] -= uint256(stor[uint32(stor0.field_0) + code.data[2264 len 32]])
