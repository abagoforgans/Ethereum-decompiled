...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 14000 * value / value == 14000
        if 14000 * value:
            require 14000 * value
            require 14000 * value * msg.value / 14000 * value == msg.value
        ...
        if 14000 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (14000 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 14000 * value * msg.value / 10^18
            require 14000 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 14000 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (14000 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 14000 * value * msg.value / 10^18
            emit Distr((14000 * value * msg.value / 10^18), msg.sender);
            emit Transfer((14000 * value * msg.value / 10^18), 0, msg.sender);
        else:
