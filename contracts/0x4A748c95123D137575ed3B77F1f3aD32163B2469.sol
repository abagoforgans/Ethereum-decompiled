...
function _fallback() payable {
    ...
    stor1 = 10^18 * code.data[2140 len 32]
    stor2 = 10^15 * code.data[2172 len 32]
    uint8(stor8.field_0) = uint8(code.data[2204 len 32])
    Mask(248, 0, stor8.field_8) = 0
    stor5 = code.data[2236 len 32]
    stor6 = 0
    return code.data[111 len 2029]
}
...
function _fallback() payable {
    ...
    if eth.balance(this.address) >= stor1.length:
        ...
        while idx < stor4.length:
            ...
            stor56A6[stor7.length - 1] = eth.balance(this.address) - (eth.balance(this.address) / 100)
            idx = 0
            while idx < stor4.length:
                mem[0] = contributors[idx]
                mem[32] = 3
