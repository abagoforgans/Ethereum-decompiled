...
function _fallback() payable {
    ...
    require code.data[1409 len 32] > block.number
    require code.data[1441 len 32] > code.data[1409 len 32]
    require code.data[1485 len 20]
    stor3 = code.data[1409 len 32]
    stor4 = code.data[1441 len 32]
    stor2 = code.data[1485 len 20]
    stor5 = code.data[1441 len 32] + 153000
    return code.data[163 len 1246]
}
...
const min_goal_amount = 3 * 10^17

const max_goal_amount = 10 * 10^18
...
function transfer_funds_to_project() {
    require transfered_total >= 3 * 10^17
...
function _fallback() payable {
    ...
    require block.number <= presale_end_block
    require msg.value
    require transfered_total < 10 * 10^18
    if transfered_total + msg.value <= 10 * 10^18:
        balances[address(msg.sender)] += msg.value
        transfered_total += msg.value
    else:
        call msg.sender with:
           value transfered_total + msg.value - 10 * 10^18 wei
             gas 2300 * is_zero(value) wei
        require ext_call.success
        balances[address(msg.sender)] = -transfered_total + balances[address(msg.sender)] + 10 * 10^18
        transfered_total = 10 * 10^18
}
