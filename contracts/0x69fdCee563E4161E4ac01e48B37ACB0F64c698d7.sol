...
function getHolders(uint256 arg1) payable {
    require calldata.size - 4 >= 32
    if stor6.length:
        mem[128 len 32 * stor6.length] = code.data[5551 len 32 * stor6.length]
    idx = 0
    s = 0
    ...
    mem[(32 * stor6.length) + 160 len 32 * s] = code.data[5551 len 32 * s]
    t = 0
    while t < s:
        require t < stor6.length
        require t < s
