...
function _fallback() payable {
    mem[96 len -1025] = code.data[1487 len -1025]
    mem[64] = -929
...
function Mint(uint256 arg1) payable {
    if arg1 == quota:
        quota++
        target = balanceOf[address(this.address)]
        balanceOf[msg.sender] += balanceOf[address(this.address)]
        balanceOf[address(this.address)] -= target
        emit Transfer(target, this.address, msg.sender);
}
