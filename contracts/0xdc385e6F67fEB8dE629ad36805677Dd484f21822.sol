...
function _fallback() payable {
    ...
    mem[96 len -5050] = code.data[5630 len -5050]
    mem[64] = -4954
    require mem[96] < -1
    require mem[223 len 1] < 255
    stor6 = mem[96] * 10^mem[223 len 1]
...
function sub_e377f664(?) {
    players.length++
    if not players.length <= players.length + 1:
...
function getAllPlayers() {
    if not players.length:
        mem[(32 * players.length) + 160] = 32
        mem[(32 * players.length) + 192] = players.length
        mem[(32 * players.length) + 224 len floor32(players.length)] = mem[160 len floor32(players.length)]
        return memory
          from (32 * players.length) + 160
           len (96 * players.length) + 64
    mem[160] = address(players.field_0)
    idx = 160
    s = 0
    while (32 * players.length) + 128 > idx:
        mem[idx + 32] = players[s].field_256
        idx = idx + 32
        s = s + 1
        continue 
    mem[(32 * players.length) + 224 len floor32(players.length)] = mem[160 len floor32(players.length)]
    return Array(len=players.length, data=mem[160 len floor32(players.length)], mem[(32 * players.length) + floor32(players.length) + 224 len (32 * players.length) - floor32(players.length)]), 
}
