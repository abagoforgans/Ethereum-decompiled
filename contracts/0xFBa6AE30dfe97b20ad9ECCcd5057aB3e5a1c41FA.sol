...
function niceGuys(uint256 arg1) payable {
    require arg1 < stor1
    return address(stor[code.data[969 len 32] + arg1])
}
    ...
    if investorIndex >= 8:
        if investorIndex <= 7:
            ...
            if not stor1 <= stor1 + 1:
                idx = stor1 + code.data[969 len 32] + 1
                while code.data[969 len 32] + stor1 > idx:
                    address(stor[idx]) = 0
                    idx = idx + 1
                    continue 
            require stor1 < stor1
            uint256(stor[code.data[969 len 32] + stor1]) = msg.sender or Mask(96, 160, uint256(stor[code.data[969 len 32] + stor1]))
            if investorIndex <= 8:
                if investorIndex >= 9:
                    investorIndex = 0
                else:
                ...
                currentNiceGuyAddress = address(stor[code.data[969 len 32] + stor3])
                currentNiceGuyIndex++
                if investorIndex >= 9:
                    investorIndex = 0
                else:
