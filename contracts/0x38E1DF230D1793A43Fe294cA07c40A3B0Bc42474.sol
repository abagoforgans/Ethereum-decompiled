...
function getMyReward() payable {
    require msg.value <= 0
    call rewardAccount.accumulatedInput() with:
...
function retrieveDAOReward(bool arg1) payable {
    require msg.value <= 0
    call dAOrewardAccount.accumulatedInput() with:
         gas gas_remaining - 25050 wei
    require ext_call.success
...
function refund() payable {
    ...
    if block.timestamp > closingTime:
        if not isFueled:
            call extraBalanceAddress.accumulatedInput() with:
                 gas gas_remaining - 25050 wei
            require ext_call.success
...
function splitDAO(uint256 arg1, address arg2) payable {
    ...
    if address(newDAO[sha3(code.data[10806 len 32] + (14 * arg1) + 8)]):
        ...
        call daoCreatorAddress.createDAO(address rg1, uint256 rg2, uint256 rg3, uint256 rg4) with:
             gas gas_remaining - 25050 wei
