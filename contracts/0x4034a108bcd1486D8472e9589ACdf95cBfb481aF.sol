...
function _fallback() payable {
    ...
    stor7 = 100
    stor6 = code.data[4806 len 32]
    stor3 = 417648 * 24 * 3600
    stor4 = 1504223999
    stor5 = 500
    return code.data[268 len 1992]
}
...
function _fallback() payable {
    ...
    require 100 * msg.value / 100 == msg.value
    if 100 * msg.value / 10^18:
        require 100 * msg.value / 10^18
        require bonusPercent * 100 * msg.value / 10^18 / 100 * msg.value / 10^18 == bonusPercent
    require rate
    require bonusPercent * 100 * msg.value / 10^18 / 100 / rate > 0
    require ext_code.size(tokenAddress)
    call tokenAddress.0x40c10f19 with:
         gas gas_remaining - 710 wei
        args msg.sender, bonusPercent * 100 * msg.value / 10^18 / 100 / rate
    ...
    require (bonusPercent * 100 * msg.value / 10^18 / 100 / rate) + tokenAmountOf[address(msg.sender)] >= tokenAmountOf[address(msg.sender)]
    tokenAmountOf[address(msg.sender)] += bonusPercent * 100 * msg.value / 10^18 / 100 / rate
    require msg.value + weiRaised >= weiRaised
    weiRaised += msg.value
    require (bonusPercent * 100 * msg.value / 10^18 / 100 / rate) + tokensSold >= tokensSold
    tokensSold += bonusPercent * 100 * msg.value / 10^18 / 100 / rate
    call stor1 with:
       value msg.value wei
