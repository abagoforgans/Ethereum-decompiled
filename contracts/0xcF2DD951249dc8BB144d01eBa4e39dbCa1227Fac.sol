...
function repeat() payable {
    ...
    call stor1.scheduleCall(address rg1, bytes4 rg2, uint256 rg3, uint256 rg4, uint8 rg5, uint256 rg6, uint256 rg7) with:
       value 2 * 10^18 wei
        ...
        args address(this.address), 0x7429c08600000000000000000000000000000000000000000000000000000000, block.number + (sha3(this.address, tx.origin, block.number, msg.sender) % 240) + 325, 10^6, 255, (sha3(this.address, tx.origin, block.number, msg.sender) % 4 * 10^15) + 10^15, 0
    ...
    if ext_call.return_data[0]:
        counter++
        sub_59b19b7a = block.number + (sha3(this.address, tx.origin, block.number, msg.sender) % 240) + 325
