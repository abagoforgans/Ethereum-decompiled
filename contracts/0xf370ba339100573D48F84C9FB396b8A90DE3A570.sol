...
function _fallback() {
    stor0 = msg.sender
    stor2 = code.data[4419 len 20]
    stor3 = code.data[4451 len 20]
    return code.data[133 len 4274]
}
...
function _fallback() payable {
    ...
    if block.timestamp >= startTime + (48 * 24 * 3600):
        ...
        emit LogCoinsEmited(((6 * 10^9 * msg.value / 10^18 / 5) + (6 * 10^9 * msg.value / 10^18)), msg.sender);
