...
function sub_845b07ab(?) payable {
    require calldata.size - 4 >= 32
    require ext_code.size(stor3)
    staticcall stor3.0x20f78e4a with:
            gas gas_remaining wei
           args arg1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function sub_13eb44f1(?) payable {
    ...
    mem[320 len 224] = code.data[7560 len 224]
    require ext_code.size(stor3)
    staticcall stor3.0xaaf96581 with:
            gas gas_remaining wei
    ...
    while uint8(idx) < uint8(ext_call.return_data[0]):
        ...
        if ext_call.return_data[32]:
            ...
            while uint8(s) < uint8(ext_call.return_data[0]):
                ...
                require return_data.size >= 96
                if ext_call.return_data[32]:
                    require uint8((2 * idx) + s + 3) < 7
                    mem[(32 * uint8((2 * idx) + s + 3)) + 320] = address(ext_call.return_data[0])
                s = s + 1
                continue 
        idx = idx + 1
        continue 
