...
function _fallback() payable {
    ...
    if not msg.value:
        stor0 = msg.sender
        require code.data[6635 len 20]
        require code.data[6667 len 20]
        stor3 = code.data[6635 len 20]
        stor4 = code.data[6667 len 20]
        if ext_code.size(stor3):
            call stor3.0x15226b54 with:
                 gas gas_remaining - 710 wei
                args stor4
            if ext_call.success:
                return code.data[976 len 5647]
    revert
}
                ...
                if ext_call.return_data[0]:
                    if not msg.value:
                        if ext_code.size(address(etherTokenAddress)):
                            call address(etherTokenAddress).approve(address rg1, uint256 rg2) with:
                                 gas gas_remaining - 710 wei
                                args tokenSenderAddress, 10 * 10^6 * msg.value
                            if ext_call.success:
                                if ext_call.return_data[0]:
                                    if not msg.value:
                                        if ext_code.size(tokenChangerAddress):
