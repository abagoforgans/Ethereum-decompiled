...
function readcolorXteam() {
    mem[8288 len 8192] = code.data[10403 len 8192]
    idx = 0
...
function readhistoricalrounds() {
    mem[96] = (2 * roundID) - 2
    if (2 * roundID) - 2:
        mem[128 len 32 * (2 * roundID) - 2] = code.data[10403 len 32 * (2 * roundID) - 2]
    idx = 1
    while idx < roundID:
...
function endround() {
    ...
    mem[96] = 86
    mem[128 len 2752] = code.data[10403 len 2752]
    idx = 0
    while uint16(idx) < 86:
        mem[0] = roundID
        mem[32] = 6467
    ...
    s = 0
    idx = 1
    while uint16(idx) < 2750:
        require uint16(idx) / 32 < mem[96]
        require 2^(8 * idx % 32)
        if not uint16(mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)):
            s = mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)
            idx = idx + 1
            continue 
        t = mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)
        s = idx + 1
        while uint16(s) < 2750:
            require uint16(s) / 32 < mem[96]
            require 2^(8 * s % 32)
            if uint16(mem[Mask(11, 5, s) + 128] and 255 * 2^(8 * s % 32) / 2^(8 * s % 32)) > 0:
                require uint16(mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)) == uint16(mem[Mask(11, 5, s) + 128] and 255 * 2^(8 * s % 32) / 2^(8 * s % 32))
            t = mem[Mask(11, 5, s) + 128] and 255 * 2^(8 * s % 32) / 2^(8 * s % 32)
            s = s + 1
            continue 
        uint256(readteamXaddr[stor1942][104][mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32) << 240].field_0) = uint256(readteamXaddr[stor1942].field_27136) - (2 * uint256(readteamXaddr[stor1942].field_27136) / 20)
        uint256(readteamXaddr[stor1942].field_29696) = uint256(readteamXaddr[stor1942][105][mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32) << 240].field_0)
        call stor1940 with:
           value uint256(readteamXaddr[stor1942].field_27136) / 20 wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        uint256(readteamXaddr[stor1942 + 1].field_28928) = trucetime
        uint256(readteamXaddr[stor1942 + 1].field_28416) = block.timestamp
        uint16(readteamXaddr[stor1942 + 1].field_28672) = beginterritories
        Mask(240, 0, readteamXaddr[stor1942 + 1].field_28688) = maxroll
        uint256(readteamXaddr[stor1942 + 1].field_27136) = uint256(readteamXaddr[stor1942].field_27136) / 20
        uint256(readteamXaddr[stor1942 + 1].field_29184) = price
        uint256(readteamXaddr[stor1942 + 1].field_29440) = maxextensiontruce
        roundID++
        emit onendround()
    t = s
    idx = 0
    while uint16(idx) < 2750:
        require uint16(idx) / 32 < mem[96]
        require 2^(8 * idx % 32)
        if uint16(mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)) > 0:
            require 0 == uint16(mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32))
        t = mem[Mask(11, 5, idx) + 128] and 255 * 2^(8 * idx % 32) / 2^(8 * idx % 32)
        idx = idx + 1
        continue 
    uint256(readteamXaddr[stor1942][104][0].field_0) = uint256(readteamXaddr[stor1942].field_27136) - (2 * uint256(readteamXaddr[stor1942].field_27136) / 20)
    uint256(readteamXaddr[stor1942].field_29696) = uint256(readteamXaddr[stor1942][105][0].field_0)
    call stor1940 with:
       value uint256(readteamXaddr[stor1942].field_27136) / 20 wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
...
function roll(uint16[] arg1, uint16 arg2) payable {
    ...
    if uint16(readteamXaddr[stor1942][101][msg.sender].field_0):
        ...
        mem[(32 * arg1.length) + 224 len 32 * uint16(readteamXaddr[stor1942].field_28688)] = code.data[10403 len 32 * uint16(readteamXaddr[stor1942].field_28688)]
