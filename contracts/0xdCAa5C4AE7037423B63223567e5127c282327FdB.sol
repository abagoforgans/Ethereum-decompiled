...
function _fallback() payable {
    ...
    stor6 = 20
    bool(stor8.length) = 0
    stor8.length.field_1 = 10
    stor8.length.field_8 = 'Loft Token' / 256
    idx = 0
    ...
    stor9.length.field_8 = 'Lottc' / 256
    idx = 0
    ...
    if stor11:
        require stor11
        require 90 * stor11 / stor11 == 90
    stor14 = 90 * stor11 / 100
    stor15 = stor11
    stor16 = 0
    stor17 = 1
    stor18 = 500000
    ...
    Mask(240, 0, stor19.field_16) = 0
    require not msg.value
    stor0 = msg.sender
    stor7 = code.data[16754 len 20]
    require not uint8(stor19.field_0)
    require stor15 >= 0
    require stor14 <= stor15
    require stor16 + stor14 >= stor16
    stor16 += stor14
    require stor14 <= stor15
    stor15 -= stor14
    require stor1[stor7] + stor14 >= stor1[stor7]
    stor1[stor7] += stor14
    emit Transfer(stor14, 0, stor7);
    return code.data[1508 len 15234]
}
...
uint8 finishFreeGetToken; offset 8
uint8 finishEthGetToken; offset 16
uint256 stor19; offset 16
uint256 stor19; offset 8
...
function finishFreeGetToken() {
    return bool(finishFreeGetToken)
}
...
function finishEthGetToken() {
    return bool(finishEthGetToken)
}
...
function increase(address arg1) {
...
function changeBaseValue(uint256 arg1, uint256 arg2, uint256 arg3, uint256 arg4) {
    require msg.sender == owner
    toGiveBase = arg1
    increaseBase = arg2
    etherGetBase = arg3
...
function changeIncrease(address[] arg1, uint256[] arg2) {
    mem[128 len 32 * arg1.length] = call.data[arg1 + 36 len 32 * arg1.length]
    ...
    while uint8(idx) < arg1.length:
        ...
        require mem[(32 * uint8(idx)) + (32 * arg1.length) + 160] <= maxIncrease
        require uint8(idx) < arg2.length
        require uint8(idx) < arg1.length
        mem[0] = mem[(32 * uint8(idx)) + 140 len 20]
        mem[32] = 4
        increase[mem[(32 * uint8(idx)) + 140 len 20]] = mem[(32 * uint8(idx)) + (32 * arg1.length) + 160]
        idx = idx + 1
        continue 
}
...
function getTokens() payable {
    ...
    if msg.value > 10^15:
        require not finishEthGetToken
        if msg.value:
            require msg.value
            require msg.value * etherGetBase / msg.value == etherGetBase
        require (msg.value * etherGetBase) + toGiveBase >= msg.value * etherGetBase
        require (msg.value * etherGetBase) + toGiveBase <= totalRemaining
        require not distributionFinished
        require totalRemaining >= 0
        require (msg.value * etherGetBase) + toGiveBase <= totalRemaining
        require totalDistributed + (msg.value * etherGetBase) + toGiveBase >= totalDistributed
        totalDistributed = totalDistributed + (msg.value * etherGetBase) + toGiveBase
        require (msg.value * etherGetBase) + toGiveBase <= totalRemaining
        totalRemaining = totalRemaining - (msg.value * etherGetBase) - toGiveBase
        require balanceOf[address(msg.sender)] + (msg.value * etherGetBase) + toGiveBase >= balanceOf[address(msg.sender)]
        balanceOf[address(msg.sender)] = balanceOf[address(msg.sender)] + (msg.value * etherGetBase) + toGiveBase
        emit Transfer(((msg.value * etherGetBase) + toGiveBase), 0, msg.sender);
        call owner with:
           value msg.value wei
             gas 2300 * is_zero(value) wei
        require ext_call.success
    else:
        require not finishFreeGetToken
        require toGiveBase <= totalRemaining
        require increase[address(msg.sender)] <= maxIncrease
        require block.timestamp >= unlockUnixTime[address(msg.sender)]
        if increase[address(msg.sender)]:
            require increase[address(msg.sender)]
            require increase[address(msg.sender)] * increaseBase / increase[address(msg.sender)] == increaseBase
        require increase[address(msg.sender)] * increaseBase >= 0
        require (increase[address(msg.sender)] * increaseBase) + toGiveBase >= increase[address(msg.sender)] * increaseBase
        increase[address(msg.sender)]++
        require not distributionFinished
        require totalRemaining >= 0
        require (increase[address(msg.sender)] * increaseBase) + toGiveBase <= totalRemaining
        require totalDistributed + (increase[address(msg.sender)] * increaseBase) + toGiveBase >= totalDistributed
        totalDistributed = totalDistributed + (increase[address(msg.sender)] * increaseBase) + toGiveBase
        require (increase[address(msg.sender)] * increaseBase) + toGiveBase <= totalRemaining
        totalRemaining = totalRemaining - (increase[address(msg.sender)] * increaseBase) - toGiveBase
        require balanceOf[address(msg.sender)] + (increase[address(msg.sender)] * increaseBase) + toGiveBase >= balanceOf[address(msg.sender)]
        balanceOf[address(msg.sender)] = balanceOf[address(msg.sender)] + (increase[address(msg.sender)] * increaseBase) + toGiveBase
        emit Transfer(((increase[address(msg.sender)] * increaseBase) + toGiveBase), 0, msg.sender);
