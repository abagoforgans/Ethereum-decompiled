...
function sub_9af25d0a(?) payable {
    ...
    call address(create.new_address).0x23599848 with:
       value msg.value wei
         gas gas_remaining wei
        args ext_call.return_data[0] << 248, address(arg1), 1
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(create.new_address))
    call address(create.new_address).0x43d726d6 with:
         gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    emit Debug(string arg1, uint256 arg2):
               0x43d726d600000000000000000000000000000000000000000000000000000000,
               msg.value,
               11,
               'Trade costs',
