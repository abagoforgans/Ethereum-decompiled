...
function _fallback() payable {
    ...
    stor0 = code.data[2830 len 20]
    stor1 = 10^18 * code.data[2850 len 32]
    stor4 = block.timestamp + (60 * code.data[2882 len 32])
    stor5 = 1337 * 10^10
    stor6 = 747943160
    stor7 = 747943160
    stor8 = code.data[2926 len 20]
    return code.data[336 len 2482]
}
...
function _fallback() payable {
    ...
    if msg.value > 0:
        ...
        stor3 += msg.value
        require price
        currentTokenAmount += -1 * 10^18 * msg.value / price
        require ext_code.size(tokenRewardAddress)
        call tokenRewardAddress.0xa9059cbb with:
             gas gas_remaining - 710 wei
