...
function _fallback() payable {
    ...
    stor10 = 0
    stor11 = 0
    stor12 = 1000000000 * 10^18
    stor13 = Mask(254, 0, stor7.field_2)
    stor14 = Mask(254, 0, stor7.field_2)
    uint8(stor15.field_0) = 0
    uint16(stor15.field_0) = 0
    require not msg.value
    stor5 = code.data[5848 len 20]
    stor6 = code.data[5880 len 20]
    stor3 = code.data[5900 len 32]
    stor4 = code.data[5932 len 32]
    require uint8(stor15.field_8) <= 3
    if uint8(stor15.field_8) != 3:
        require uint8(stor15.field_8) <= 3
        if uint8(stor15.field_8) != 2:
            require uint8(stor15.field_8) <= 3
            if not uint8(stor15.field_8):
                if block.number >= stor3:
                    uint8(stor15.field_8) = 1
            require uint8(stor15.field_8) <= 3
            if uint8(stor15.field_8) == 1:
                if block.number > stor4:
                    uint8(stor15.field_8) = 2
        else:
            if block.number <= stor4:
                require uint8(stor15.field_8) <= 3
                if not uint8(stor15.field_8):
...
function allocateEcosystemTokens() {
    ...
    require ecosystemAllocation + totalTokens >= totalTokens
    totalTokens += ecosystemAllocation
    require reservedAllocation + balanceOf[stor5] >= balanceOf[stor5]
    balanceOf[stor5] += reservedAllocation
    require reservedAllocation + totalTokens >= totalTokens
    totalTokens += reservedAllocation
    stor15 = 1
}
...
function deliver(address arg1, uint256 arg2, string arg3) {
    ...
    if owner != msg.sender:
        require founderAddress == msg.sender
    require arg2 > 0
    require totalTokens < totalSupply
    require _saleState <= 3
    require _saleState != 3
    require _saleState <= 3
    if not _saleState:
    ...
    require arg2 + totalTokens >= totalTokens
    totalTokens += arg2
    require _saleState <= 3
    if not _saleState:
