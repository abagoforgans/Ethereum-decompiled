...
function sub_8bab6e82(?) payable {
    ...
    totalSupply += arg2 / 2
    emit code.data[3218 len 32]: arg2, owner, arg1
    return 1
}
...
function reFundByOther(address arg1) payable {
    ...
    emit code.data[3218 len 32]: 0, msg.sender, this.address
    return 1
}
    ...
    if block.timestamp >= closeTime:
        ...
        if arg1 != this.address:
            ...
            if not ext_call.success:
                return 0
    emit code.data[3218 len 32]: arg2, msg.sender, arg1
    return 1
}
...
function _fallback() payable {
    ...
    if closeTime - (336 * 24 * 3600) > block.timestamp:
        ...
        if balanceOf[stor2] >= msg.value:
            ...
            emit code.data[3218 len 32]: msg.value, owner, msg.sender
    else:
        ...
        if balanceOf[stor2] >= s:
            ...
            if eth.balance(this.address) >= maxFundedValue:
                uint8(stor9.field_0) = 1
            balanceOfFund[address(msg.sender)] += msg.value
            emit code.data[3218 len 32]: s, owner, msg.sender
}

function sub_e96730c4(?) payable {
    require owner != msg.sender
    require block.timestamp <= closeTime
    require block.timestamp >= startTime
    require not uint8(stor9.field_0)
