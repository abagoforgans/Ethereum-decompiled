...
function tokensale(address arg1) payable {
    ...
    require _totalSupply >= msg.value * price
    require msg.value * price <= balanceOf[stor7]
    balanceOf[stor7] += -1 * msg.value * price
    require balanceOf[address(arg1)] + (msg.value * price) >= balanceOf[address(arg1)]
    balanceOf[address(arg1)] += msg.value * price
    require msg.value * price <= _totalSupply
    _totalSupply += -1 * msg.value * price
...
function sendBTCPTokenToMultiAddr(address[] arg1, uint256[] arg2) {
    ...
    while idx < arg1.length:
        ...
        require idx < arg2.length
        require mem[(32 * idx) + (32 * arg1.length) + 160] <= _totalSupply
        _totalSupply -= mem[(32 * idx) + (32 * arg1.length) + 160]
        idx = idx + 1
        continue 
}
