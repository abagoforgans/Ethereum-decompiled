...
function sub_58a2bb05(?) {
    ...
    require dog.length <= test266151307()
    require msg.sender
    require not ownerOf[stor14.length]
    ownerOf[stor14.length] = msg.sender
    require balanceOf[address(msg.sender)] + 1 >= balanceOf[address(msg.sender)]
    balanceOf[address(msg.sender)]++
    emit Transfer(0, msg.sender, dog.length);
    stor6[stor14.length] = tokenOfOwnerByIndex[address(msg.sender)]
    tokenOfOwnerByIndex[address(msg.sender)]++
    tokenOfOwnerByIndex[address(msg.sender)][tokenOfOwnerByIndex[address(msg.sender)]] = dog.length
    stor8[stor14.length] = tokenByIndex.length
    tokenByIndex.length++
    tokenByIndex[tokenByIndex.length] = dog.length
    require ownerOf[stor14.length]
    uint256(tokenURI[stor14.length][]) = Array(len=arg2.length, data=arg2[all])
    return 1
}
