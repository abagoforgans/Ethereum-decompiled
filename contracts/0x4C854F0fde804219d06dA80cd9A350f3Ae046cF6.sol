...
function _fallback() payable {
    ...
    stor7 = 100
    stor6 = code.data[4783 len 32]
    stor3 = 417648 * 24 * 3600
    stor4 = 1504223999
    stor5 = 500
    return code.data[268 len 1969]
}
...
function _fallback() payable {
    ...
    if msg.value * rate / 10^18:
        ...
        require bonusPercent * msg.value * rate / 10^18 / msg.value * rate / 10^18 == bonusPercent
    require bonusPercent * msg.value * rate / 10^18 / 100 > 0
    require ext_code.size(tokenAddress)
    call tokenAddress.0x40c10f19 with:
         gas gas_remaining - 710 wei
        args msg.sender, bonusPercent * msg.value * rate / 10^18 / 100
    ...
    require (bonusPercent * msg.value * rate / 10^18 / 100) + tokenAmountOf[address(msg.sender)] >= tokenAmountOf[address(msg.sender)]
    tokenAmountOf[address(msg.sender)] += bonusPercent * msg.value * rate / 10^18 / 100
    require msg.value + weiRaised >= weiRaised
    weiRaised += msg.value
    require (bonusPercent * msg.value * rate / 10^18 / 100) + tokensSold >= tokensSold
    tokensSold += bonusPercent * msg.value * rate / 10^18 / 100
    call stor1 with:
       value msg.value wei
