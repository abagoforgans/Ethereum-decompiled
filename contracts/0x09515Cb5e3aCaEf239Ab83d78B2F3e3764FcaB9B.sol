...
function persons(uint256 arg1) payable {
    require arg1 < stor0
    return address(stor[code.data[1009 len 32] + arg1])
}
    ...
    if investor <= 8:
        if investor >= 9:
            ...
            if not stor0 <= stor0 + 1:
                idx = stor0 + code.data[1009 len 32] + 1
                while code.data[1009 len 32] + stor0 > idx:
                    address(stor[idx]) = 0
                    idx = idx + 1
                    continue 
            require stor0 < stor0
            uint256(stor[code.data[1009 len 32] + stor0]) = msg.sender or Mask(96, 160, uint256(stor[code.data[1009 len 32] + stor0]))
            investor++
            if 11 == investor + 1:
                investor = 0
            call currentNiceGuyAddress with:
