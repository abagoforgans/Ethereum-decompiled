...
function status() {
    return eth.balance(this.address), stor2.length, block.timestamp - stor1, block.timestamp - stor1 / 60
}
...
function performPayouts() {
    s = 0
    s = 0
    while stor1 + 60 < block.timestamp:
        idx = stor2.length
        while idx > 0:
            require idx - 1 < stor2.length
            mem[0] = 2
            if stor2[idx].field_0 <= stor1 + 60:
                require idx - 1 < stor2.length
                require idx - 1 < stor2.length
                mem[0] = 2
