...
function buyTicket(uint256 arg1) payable {
    if block.number % 106 < 40:
        ...
        if not stor2 <= stor2 + 1:
            idx = stor2 + code.data[1294 len 32] + 1
            while code.data[1294 len 32] + stor2 > idx:
                uint256(stor[idx]) = 0
                idx = idx + 1
                continue 
        require stor2 - 1 < stor2
        uint256(stor[code.data[1294 len 32] + stor2 - 1]) = msg.sender or Mask(96, 160, uint256(stor[code.data[1294 len 32] + stor2 - 1]))
}
...
function payout() payable {
    ...
    idx = 0
    while idx < stor2:
        stor5[address(stor[code.data[1294 len 32] + idx])] = 0
        mem[0] = address(stor[code.data[1294 len 32] + idx])
        mem[32] = 6
        uint256(stor6[address(stor[code.data[1294 len 32] + idx])].field_0) = 0
        idx = idx + 1
        continue 
    tickets.length = 0
