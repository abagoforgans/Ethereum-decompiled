...
function sub_432dcdb8(?) payable {
    ...
    while idx < stor260.length:
        mem[0] = stor564C[idx]
        mem[32] = 274
        if not address(sub_67f7c83a[stor564C[idx]].field_0):
            require idx < stor260.length
            mem[0] = stor564C[idx]
            if not Mask(255, 1, (256 * not bool(sub_67f7c83a[stor564C[idx]].field_512)) - 1 and sub_67f7c83a[stor564C[idx]].field_512):
                idx = idx + 1
                s = s
                continue 
        idx = idx + 1
