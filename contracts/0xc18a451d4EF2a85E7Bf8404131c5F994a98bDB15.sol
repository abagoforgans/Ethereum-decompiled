...
function participants(uint256 arg1) payable {
    ...
    return address(stor5629[arg1]), stor[code.data[1116 len 32] + (2 * arg1)]
}
...
function enter() payable {
    if msg.value < 5 * 10^17:
        ...
        stor[code.data[1116 len 32] + (2 * stor0.length)] = msg.value
        if not stor0.length:
        ...
        if balance > 103 * stor[code.data[1116 len 32] + (2 * stor1)] / 100:
            require payoutIdx < stor0.length
            call address(stor5629[stor1]) with:
               value stor5629[stor1] - (stor5629[stor1] / 10) wei
                 gas 0 wei
            balance -= 103 * stor[code.data[1116 len 32] + (2 * stor1)] / 100
            payoutIdx++
}
