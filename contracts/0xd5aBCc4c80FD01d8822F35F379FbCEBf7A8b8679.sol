...
function _fallback() {
    stor0 = msg.sender
    stor2 = code.data[3718 len 20]
    stor1 = code.data[3686 len 20]
    stor4 = code.data[3814 len 20]
    stor6 = code.data[3770 len 32]
    stor5 = code.data[3738 len 32]
    stor7 = code.data[3834 len 32]
    return code.data[238 len 3436]
}
...
function authorizeSpender(address arg1, bool arg2) {
    ...
    emit SpenderAuthorization(address(arg1), arg2);
}
...
function collectAuthorizedPayment(uint256 arg1) {
    ...
    call address(authorizedPayments[arg1].field_784) or 0 or 0 with:
       value uint256(authorizedPayments[arg1].field_1024) wei
         gas 2300 * is_zero(value) wei
    require ext_call.success
    emit PaymentExecuted(arg1, address(authorizedPayments[arg1].field_768), uint256(authorizedPayments[arg1].field_1024));
}
...
function authorizePayment(string arg1, address arg2, uint256 arg3, uint256 arg4) {
    ...
    if authorizedPayments[authorizedPayments.length].length + 31 / 32 > Mask(251, 0, arg1.length + 31) >> 5:
        ...
        while authorizedPayments[authorizedPayments.length].length + 31 / 32 > idx:
            ...
            idx = idx + 1
            continue 
    emit PaymentAuthorized(authorizedPayments.length, address(authorizedPayments[authorizedPayments.length].field_768), uint256(authorizedPayments[authorizedPayments.length].field_1024));
    return authorizedPayments.length
}
