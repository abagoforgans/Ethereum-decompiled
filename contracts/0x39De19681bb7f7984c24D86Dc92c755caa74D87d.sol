...
function _fallback() payable {
    stor0 = code.data[5026 len 32]
    return code.data[92 len 4934]
}
...
function createToken(address arg1) payable {
    ...
    create contract with 0 wei
                    code: code.data[2806 len 2128], address(ext_call.return_data[0])
...
function sub_a916a49a(?) payable {
    call arg1.updateAuthority(address rg1, uint8 rg2) with:
         gas gas_remaining - 25050 wei
        args stor0, 0
    require ext_call.success
    call stor0.0x2bf33671 with:
         gas gas_remaining - 25050 wei
    ...
    call arg1.setCanCall(address rg1, address rg2, bytes4 rg3, bool rg4) with:
         gas gas_remaining - 25050 wei
        args address(create.new_address), address(ext_call.return_data[0]), Mask(32, 224, sha3('addBalance(address,uint256)')), 1
    call arg1.setCanCall(address rg1, address rg2, bytes4 rg3, bool rg4) with:
         gas gas_remaining - 25050 wei
        args address(create.new_address), address(ext_call.return_data[0]), Mask(32, 224, sha3('subBalance(address,uint256)')), 1
    call address(create.new_address).updateAuthority(address rg1, uint8 rg2) with:
         gas gas_remaining - 25050 wei
        args address(arg1), 1
    return address(ext_call.return_data[0]), address(create.new_address)
