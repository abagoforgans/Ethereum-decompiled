...
function _fallback() payable {
    ...
    stor1 = ext_call.return_data[0] + 3
    stor2 = ext_call.return_data[0] + 2
    stor3 = block.timestamp + 1200
    stor4 = 0
    stor5 = 0
    stor6 = 0
    idx = 0
...
function accessOperatingBudget(uint256 arg1) payable {
    if address(owner) == msg.sender:
        require eth.balance(this.address) - (10^16 * numTickets) >= 10^15
        require arg1 <= eth.balance(this.address) + (-1 * 10^16 * numTickets) - 10^15
...
function _fallback() payable {
    ...
    call stor0.getLastBlockHeight() with:
         gas gas_remaining - 25050 wei
    require ext_call.success
    if block.timestamp + (600 * decidingBlock) + (-600 * ext_call.return_data[0]) - 600 < cutoffTimestamp:
        cutoffTimestamp = block.timestamp + (600 * decidingBlock) + (-600 * ext_call.return_data[0]) - 600
    ...
    if nearestKnownBlock != 0:
        ...
        while idx < 5:
            ...
            call stor0.getLastBlockHeight() with:
                 gas gas_remaining - 25050 wei
            require ext_call.success
            decidingBlock = ext_call.return_data[0] + 3
            cutoffBlock = ext_call.return_data[0] + 2
            cutoffTimestamp = block.timestamp + 1200
            nearestKnownBlock = 0
