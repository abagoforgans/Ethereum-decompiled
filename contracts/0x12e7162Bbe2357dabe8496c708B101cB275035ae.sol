...
function _fallback() payable {
    ...
    stor0 = msg.sender
    require code.data[7590 len 20]
    require code.data[7622 len 20]
    require code.data[7654 len 20]
    require code.data[7674 len 32]
    stor2 = code.data[7590 len 20]
    stor3 = code.data[7622 len 20]
    stor4 = code.data[7654 len 20]
    stor5 = code.data[7674 len 32]
    return code.data[531 len 7047]
}
...
const MIN_INVESTEMENT = 10^11

const INVESTOR_TOKENS = 10^13

const MIN_PRICE = 10^11

const TOTAL_TOKENS_FOR_PRICE = 10^13
...
function prolongate() {
    require msg.sender == owner
    require icoDuration < 672 * 24 * 3600
    icoDuration += 336 * 24 * 3600
}
...
function isCrowdsaleFull() {
    ...
    require 10^13 * 10^ext_call.return_data[0] / 10^13 == 10^ext_call.return_data[0]
    if tokensSold >= 10^13 * 10^ext_call.return_data[0]:
        return tokensSold >= 10^13 * 10^ext_call.return_data[0]
...
function finalize() {
    ...
    if block.timestamp < startsAt:
        if weiRaised < 1000 * 10^18:
            ...
            require ext_call.success
            require 10^13 * 10^ext_call.return_data[0] / 10^13 == 10^ext_call.return_data[0]
            if 10^13 * 10^ext_call.return_data[0] <= tokensSold:
                require ext_code.size(tokenAddress)
                call tokenAddress.0x5f412d4f with:
                     gas gas_remaining - 710 wei
                require ext_call.success
            else:
                require tokensSold <= 10^13 * 10^ext_call.return_data[0]
                require ext_code.size(tokenAddress)
                call tokenAddress.0x40c10f19 with:
                     gas gas_remaining - 710 wei
                    args teamWalletAddress, (10^13 * 10^ext_call.return_data[0]) - tokensSold
                require ext_call.success
                require ext_code.size(tokenAddress)
                call tokenAddress.0x5f412d4f with:
                     gas gas_remaining - 710 wei
                        ...
                        if not msg.value:
                            ...
                            if investedAmountOf[address(arg1)]:
                                ...
                                call tokenAddress.0x313ce567 with:
                                     gas gas_remaining - 710 wei
                                require ext_call.success
                                require tokensSold == (10^13 * tokensSold / 10^13) + (tokensSold % 10^13)
                                require 10^ext_call.return_data[0]
                                require 2 * 10^11 >= -(10^11 * tokensSold / 10^13) - (10^11 * 10^ext_call.return_data[0]) / 10^ext_call.return_data[0]
                                emit PriceChanged(tokenPrice, ((10^11 * tokensSold / 10^13) - (10^11 * 10^ext_call.return_data[0]) / 10^ext_call.return_data[0]) + 2 * 10^11);
                                tokenPrice = ((10^11 * tokensSold / 10^13) - (10^11 * 10^ext_call.return_data[0]) / 10^ext_call.return_data[0]) + 2 * 10^11
                                require ext_code.size(tokenAddress)
                                call tokenAddress.0x40c10f19 with:
                                     gas gas_remaining - 710 wei
