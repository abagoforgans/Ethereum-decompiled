    ...
    if bool(stor8) == 1:
        ...
        if block.timestamp >= time + waitTime:
            ...
            if not msg.value - bet:
                ...
                require ((10 * msg.value) - (10 * bet) / 20) + bet >= (10 * msg.value) - (10 * bet) / 20
                call playerAddress with:
                   value ((10 * msg.value) - (10 * bet) / 20) + bet wei
                     gas 2300 * is_zero(value) wei
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            if not msg.value - bet:
                ...
                require (10 * msg.value) - (10 * bet) / msg.value - bet == 10
                require ((10 * msg.value) - (10 * bet) / 20) + bet >= (10 * msg.value) - (10 * bet) / 20
                emit Withdrawal(playerAddress, block.timestamp, ((10 * msg.value) - (10 * bet) / 20) + bet, bet);
            if not msg.value - bet:
                call stor5 with:
                     gas 2300 wei
            else:
                ...
                call stor5 with:
                   value msg.value - bet / 20 wei
                     gas 2300 * is_zero(value) wei
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            if not msg.value - bet:
                require sub_40d5ff58 >= sub_40d5ff58
            else:
                require msg.value - bet
                require (9 * msg.value) - (9 * bet) / msg.value - bet == 9
                require sub_40d5ff58 + ((9 * msg.value) - (9 * bet) / 20) >= sub_40d5ff58
                sub_40d5ff58 += (9 * msg.value) - (9 * bet) / 20
            playerAddress = msg.sender
            time = block.timestamp
            bet = msg.value
            if not bet:
