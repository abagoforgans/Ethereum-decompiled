...
function _fallback() {
    ...
    call mem[108 len 20].addMeByRC(address rg1) with:
...
function euroRaisedRc() {
    require ext_code.size(stor0)
    call stor0.euroRaised(uint256 rg1) with:
         gas gas_remaining - 710 wei
...
function _fallback() payable {
    ...
    if block.timestamp < endRC:
        require ext_code.size(stor0)
        call stor0.buy(address rg1) with:
           value msg.value wei
        ...
        require ext_code.size(stor0)
        call stor0.claimPremium(address rg1, uint256 rg2) with:
             gas gas_remaining - 710 wei
