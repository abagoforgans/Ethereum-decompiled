...
function purchaseMembership(address arg1, address arg2) payable {
    ...
    if block.timestamp - start >= 336 * 24 * 3600:
        recipientVIP[address(arg2)] += 10 * msg.value
        totalVIP += 10 * msg.value
        require (10 * msg.value) + totalVIP <= limitVIP
        require ext_code.size(legendsTokenAddress)
        call legendsTokenAddress.addTokens(address rg1, uint256 rg2) with:
             gas gas_remaining - 50 wei
            args address(arg2), 10 * msg.value
        require ext_call.success
        emit VIPPurchase(msg.value, 10 * msg.value, arg1, arg2);
    else:
        recipientVIP[address(arg2)] += 100 * msg.value / 9
        totalVIP += 100 * msg.value / 9
        require (100 * msg.value / 9) + totalVIP <= limitVIP
        require ext_code.size(legendsTokenAddress)
        call legendsTokenAddress.addTokens(address rg1, uint256 rg2) with:
             gas gas_remaining - 50 wei
            args address(arg2), 100 * msg.value / 9
        require ext_call.success
        emit VIPPurchase(msg.value, 100 * msg.value / 9, arg1, arg2);
}
