...
function _fallback() payable {
    ...
    return code.data[1286 len 5378]
}
...
function sub_3697d5ee(?) {
    require ext_code.size(stor19)
    call stor19.totalMonster() with:
         gas gas_remaining - 710 wei
    require ext_call.success
    uint64(stor0.field_0) = uint64(ext_call.return_data[0] + 1)
    Mask(192, 0, stor0.field_64) = 0
    require ext_code.size(stor18)
    call stor18.getRandom(address arg1, uint256 arg2, uint256 arg3) with:
         gas gas_remaining - 710 wei
        args address(this.address), block.number - 1, sub_fe2ca572
    require ext_call.success
    uint256(seed) = ext_call.return_data[0]
    idx = 0
    s = 0
