...
function checkGoalReached() payable {
    ...
    if amountRaised < fundingGoal:
        emit code.data[1209 len 32]: 0, 11, 0
        idx = 0
        while idx < stor6.length:
            ...
            mem[mem[64] + 64] = 0
            emit code.data[1209 len 32]: address(stor40F6[idx]), stor56F6[idx], 0
            idx = idx + 1
            continue 
    else:
        call beneficiaryAddress with:
           value amountRaised wei
             gas 0 wei
        emit code.data[1209 len 32]: beneficiaryAddress, amountRaised, 0
...
function _fallback() payable {
    ...
    emit code.data[1209 len 32]: address(stor40F6[stor6.length + 1]), stor40F6[stor6.length + 1], 1
}
