...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 270 * value / value == 270
        if 270 * value:
            require 270 * value
            require 270 * value * msg.value / 270 * value == msg.value
        ...
        if 270 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (270 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 270 * value * msg.value / 10^18
            require 270 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 270 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (270 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 270 * value * msg.value / 10^18
            emit Distr((270 * value * msg.value / 10^18), msg.sender);
            emit Transfer((270 * value * msg.value / 10^18), 0, msg.sender);
        else:
