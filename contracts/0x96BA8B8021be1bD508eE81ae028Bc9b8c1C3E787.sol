...
function _fallback() payable {
    ...
    stor15 = 0
    stor7 = msg.sender
    address(stor6.field_24) = code.data[6120 len 20]
    stor1[code.data[6120 len 20]] = stor9
    require stor9 <= stor8
    stor8 -= stor9
...
uint8 stor6; offset 16
address stor6;
address founderAddress; offset 24
address owner;
uint256 totalTokens;
uint256 team;
uint256 bounty;
...
function changeFounder(address arg1) {
    ...
    balanceOf[address(stor6.field_24)] = 0
    founderAddress = arg1
}
...
function _fallback() payable {
    ...
    if uint8(stor6.field_8):
        ...
        require (msg.value / 10^15 / 2) + (msg.value / 10^15) <= balanceOf[stor7]
        balanceOf[stor7] = balanceOf[stor7] - (msg.value / 10^15 / 2) - (msg.value / 10^15)
        if uint8(stor6.field_8):
        ...
        emit 0xeff18d5a: ((msg.value / 10^15 / 2) + (msg.value / 10^15)), msg.sender
        emit ContributionReceived(msg.value, msg.sender);
        emit Transfer(((msg.value / 10^15 / 2) + (msg.value / 10^15)), owner, msg.sender);
    else:
        ...
        emit 0xeff18d5a: (msg.value / 10^15), msg.sender
        emit ContributionReceived(msg.value, msg.sender);
        emit Transfer((msg.value / 10^15), owner, msg.sender);
}
