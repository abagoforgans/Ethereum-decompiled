...
function _fallback() payable {
    mem[96 len -2595] = code.data[3105 len -2595]
    mem[64] = -2499
...
function sendRewardBlockCDN(address arg1, uint256 arg2) {
    ...
    totalSupply += arg2 / 2
    emit code.data[2563 len 32]: arg2, owner, arg1
    return 1
}
...
function reFundByOther(address arg1) {
    ...
    emit code.data[2563 len 32]: 0, msg.sender, this.address
    return 1
}
    ...
    if block.timestamp >= closeTime:
        ...
        if arg1 == this.address:
            ...
            if not ext_call.success:
                return 0
    emit code.data[2563 len 32]: arg2, msg.sender, arg1
    return 1
}
...
function buyBlockCDN() {
    ...
    if closeTime - (336 * 24 * 3600) > block.timestamp:
        ...
        emit code.data[2563 len 32]: msg.value, this.address, msg.sender
    else:
        ...
        if eth.balance(this.address) >= maxFundedValue:
            isFundedMax = 1
        balanceOfFund[address(msg.sender)] += msg.value
        emit code.data[2563 len 32]: s, this.address, msg.sender
    return 1
}
