...
uint8 stor4;
address erc223Address; offset 8
uint256 stor4;
uint256 startTime;
uint256 endTime;
...
function arrayAirDropReceivers(uint256 arg1) {
    ...
    return address(arrayAirDropReceivers[arg1])
}
...
function getTotalAirDroppedAmount() {
    if not airDropAmount:
        return 0
    require arrayAirDropReceivers.length * airDropAmount / airDropAmount == arrayAirDropReceivers.length
    return (arrayAirDropReceivers.length * airDropAmount)
}

function toggle() {
    require msg.sender == owner
    uint256(stor4.field_0) = not uint8(stor4.field_0) or Mask(248, 8, uint256(stor4.field_0))
    if not uint8(not uint8(stor4.field_0)):
        emit LogStart()
    else:
        emit LogStop()
}
...
function getRemainingToken() {
    require ext_code.size(erc223Address)
    call erc223Address.0x70a08231 with:
...
function isValidAirDropForAll() {
    ...
    if uint8(stor4.field_0):
        return not bool(uint8(stor4.field_0))
...
function withdraw(address arg1) {
    ...
    require return_data.size >= 32
    require ext_code.size(erc223Address)
    call erc223Address.0xa9059cbb with:
         gas gas_remaining wei
...
function isValidAirDropForIndividual() {
    ...
    if block.timestamp < startTime:
        ...
        if block.timestamp > endTime:
            return block.timestamp <= endTime
    return not bool(stor2[msg.sender])
}
...
function receiveAirDrop() {
    ...
    uint256(arrayAirDropReceivers[arrayAirDropReceivers.length]) = msg.sender or Mask(96, 160, uint256(arrayAirDropReceivers[arrayAirDropReceivers.length]))
    require ext_code.size(erc223Address)
    call erc223Address.0xa9059cbb with:
         gas gas_remaining wei
