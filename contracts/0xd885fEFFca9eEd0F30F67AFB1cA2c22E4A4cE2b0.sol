...
function _fallback() payable {
    ...
    stor0 = code.data[7186 len 20]
    stor8.length++
    if not stor8.length > stor8.length + 1:
        stor8[stor8.length] = code.data[7206 len 32]
        stor8.length++
        stor8[stor8.length] = code.data[7238 len 32]
        stor8.length++
    else:
        idx = stor8.length + 1
        ...
        if not stor8.length > stor8.length + 1:
            ...
            if not stor8.length <= stor8.length + 1:
                ...
                while stor8.length > idx:
                    ...
                    idx = idx + 1
                    continue 
    stor8[stor8.length] = code.data[7270 len 32]
    stor11 = stor12
    stor5 = code.data[7302 len 32]
    stor9 = stor10
    return code.data[867 len 6307]
}
    ...
    if not stor2:
        ...
        if block.timestamp > intervals:
            if stor2 < threshold:
                ...
                call stor11.0xa9059cbb with:
                     gas gas_remaining - 710 wei
                    args stor1, stor2 * decimals
                require ext_call.success
                stor2 = 0
                require ext_code.size(stor9)
                call stor9.0x58e1835d with:
                     gas gas_remaining - 710 wei
                    args msg.sender, stor2, stor4
                require ext_call.success
    else:
        if stor4 >= intervals.length:
            ...
            if block.timestamp > intervals[stor4]:
                ...
                if stor2 <= threshold:
                    ...
                    stor2 -= stor3 / 3
                    emit 0xad463e62: 160, stor2, 0, 0, msg.sender, 14, 'updated amount'
