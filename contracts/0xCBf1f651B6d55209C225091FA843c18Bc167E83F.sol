...
const _fallback = code.data[18 len 5216]
...
function sub_fa5af988(?) payable {
    return sub_fa5af988[0 len sub_fa5af988.length]
}
...
function sub_e4c9c4e9(?) payable {
    if msg.sender == address(stor4):
        sub_fa5af988[] = Array(len=arg1.length, data=arg1[all])
}
...
function sub_4991bee4(?) payable {
    ...
    mem[160] = 'b'
    contractName.length = 67
    s = 0
    idx = 128
    while 161 > idx:
        contractName[s] = mem[idx]
        s = s + 1
        idx = idx + 32
        continue 
    idx = 2
