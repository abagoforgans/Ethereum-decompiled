...
function _fallback() payable {
    ...
    if not msg.value:
        stor0 = msg.sender
        require code.data[5026 len 20]
        require code.data[5058 len 20]
        stor3 = code.data[5026 len 20]
        stor4 = code.data[5058 len 20]
        if ext_code.size(stor3):
            call stor3.getReserveBalance(address rg1) with:
                 gas gas_remaining - 710 wei
                args stor4
            if ext_call.success:
                return code.data[884 len 4130]
    revert
}
...
function _fallback() payable {
    ...
    call address(etherTokenAddress).approve(address rg1, uint256 rg2) with:
         gas gas_remaining - 710 wei
        args tokenChangerAddress, changeAmount + msg.value
    require ext_call.success
    require ext_call.return_data[0]
    ...
    call tokenChangerAddress.change(address rg1, address rg2, uint256 rg3, uint256 rg4) with:
         gas gas_remaining - 710 wei
        args 0, uint32(stor4), address(ext_call.return_data[0]), changeAmount + msg.value, 1
    require ext_call.success
