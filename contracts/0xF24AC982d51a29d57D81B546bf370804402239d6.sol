...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 375 * value / value == 375
        if 375 * value:
            require 375 * value
            require 375 * value * msg.value / 375 * value == msg.value
        ...
        if 375 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (375 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 375 * value * msg.value / 10^18
            require 375 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 375 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (375 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 375 * value * msg.value / 10^18
            emit Distr((375 * value * msg.value / 10^18), msg.sender);
            emit Transfer((375 * value * msg.value / 10^18), 0, msg.sender);
        else:
