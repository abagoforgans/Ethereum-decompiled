...
function _fallback() payable {
    ...
    mem[96 len -2396] = code.data[3060 len -2396]
    mem[64] = -2300
    stor10 = msg.sender
    ...
    idx = -2108
    s = 0
    while stor12.length - 2108 > idx + 32:
        mem[idx + 32] = stor[s + sha3(mem[0]) + 1]
        idx = idx + 32
        s = s + 1
        continue 
    emit LogFunderInitialized(address rg1, address rg2, string rg3, uint256 rg4, uint256 rg5):
                              mem[mem[64] len stor12.length + (floor32(stor12.length - 1) + -stor12.length + 32 % 32) + -mem[64] - 2108],
    return code.data[664 len 2396]
}
