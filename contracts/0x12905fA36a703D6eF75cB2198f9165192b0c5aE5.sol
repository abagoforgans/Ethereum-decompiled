...
function contributors(uint256 arg1) payable {
    require arg1 < stor0
    return stor5629[arg1], uint256(stor[(3 * arg1) + code.data[1514 len 32]]), stor6329[arg1]
}
    ...
    if msg.sender == address(profitAddr):
        ...
        if stor0 != 0:
            if msg.value != 0:
                ...
                while idx < stor0:
                    mem[0] = 0
                    stor6329[idx] += (stor5629[idx] * msg.value) + (uint256(stor[code.data[1514 len 32] + (3 * idx)]) * msg.value) / totalSUM
                    idx = idx + 1
                    s = stor5629[idx] + uint256(stor[code.data[1514 len 32] + (3 * idx)])
                    s = (stor5629[idx] * msg.value) + (uint256(stor[code.data[1514 len 32] + (3 * idx)]) * msg.value) / totalSUM
                    continue 
        totalSUM += msg.value
    else:
        ...
        while idx < stor0:
            ...
            if stor5629[idx] != msg.sender:
                ...
                continue 
            require idx < stor0
            uint256(stor[code.data[1514 len 32] + (3 * idx)]) += msg.value
        ...
        uint256(stor[code.data[1514 len 32] + (3 * stor0) - 3]) = msg.value
        stor6329[stor0] = 0
}
