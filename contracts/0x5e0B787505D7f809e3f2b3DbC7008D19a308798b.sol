...
function sub_1937f998(?) {
    if stor0 == msg.sender:
        ...
        if not uint8(stor20[arg1].field_1024):
            require arg1 < stor20.length
            uint8(stor20[arg1].field_1024) = 1
    ...
    if stor0 != msg.sender:
        ...
        if status != stor5:
            ...
            while idx < stor20.length:
                mem[0] = 20
                if uint8(stor20[idx].field_1024):
                    idx = idx + 1
                    s = s
                    continue 
        ...
        if not arg2.length:
            ...
            if not arg3.length:
                ...
                if not stor20.length <= stor20.length + 1:
                    mem[0] = 20
                    idx = (5 * stor20.length) + 5
                    while sha3(20) + (5 * stor20.length) > idx + sha3(mem[0]):
                        ...
                        idx = idx + 5
                        continue 
                uint8(stor20[stor20.length].field_0) = 0
                Mask(248, 0, stor20[stor20.length].field_8) = mem[ceil32(arg1.length) + ceil32(arg2.length) + ceil32(arg3.length) + 384 len 31]
                idx = 0
                while stor20[stor20.length].length + 31 / 32 > idx:
                    uint256(stor20[(5 * stor20.length) + idx].field_0) = 0
                    idx = idx + 1
                    continue 
                uint256(stor[sha3((5 * stor20.length) + ('name', 'stor20', 20) + 1)][].field_0) = Array(len=arg1.length, data=arg1[all])
                uint256(stor[sha3((5 * stor20.length) + ('name', 'stor20', 20) + 2)][].field_0) = Array(len=arg2.length, data=arg2[all])
                uint256(stor[sha3((5 * stor20.length) + ('name', 'stor20', 20) + 3)][].field_0) = Array(len=arg3.length, data=arg3[all])
                uint8(stor20[stor20.length].field_1024) = 0
