...
const CROWDSALE_OPENING_TIME = (427095 * 3600)

const CROWDSALE_CLOSING_TIME = (429267 * 3600)

const PRESALE_WEI_CAP = 2500 * 10^18

const INITIAL_SUPPLY = 2480 * 10^6

const PRESALE_RATE = 500000

const CROWDSALE_WEI_GOAL = 800 * 10^18

const PRESALE_CLOSING_TIME = (426747 * 3600)

const CROWDSALE_WEI_CAP = 20000 * 10^18

const PRESALE_WEI_GOAL = 100 * 10^18

const PRESALE_OPENING_TIME = (426495 * 3600)
...
uint8 isPresaleFinalized;
uint8 isCrowdsaleFinalized; offset 8
...
function isPresaleFinalized() {
    return bool(isPresaleFinalized)
}
...
function presaleGoalReached() {
    return presaleWeiRaised >= 100 * 10^18
}

function crowdsaleGoalReached() {
    return crowdsaleWeiRaised >= 800 * 10^18
}
...
function finalizeCrowdsale() {
    ...
    require presaleWeiRaised >= 100 * 10^18
    require not isCrowdsaleFinalized
    require block.timestamp > 429267 * 3600
    if crowdsaleWeiRaised < 800 * 10^18:
        emit RefundsEnabled()
    else:
...
function getCrowdsaleUserCap() {
    require block.timestamp >= 427095 * 3600
    require block.timestamp <= 429267 * 3600
    require 427095 * 3600 <= block.timestamp
    idx = 0
    while idx < crowdsaleUserCaps.length:
        require idx < crowdsaleMinElapsedTimeLevels.length
        mem[0] = 7
        if block.timestamp - (427095 * 3600) >= crowdsaleMinElapsedTimeLevels[idx]:
            require idx < crowdsaleMinElapsedTimeLevels.length
            mem[0] = 7
...
function finalizePresale() {
    require not isPresaleFinalized
    require block.timestamp > 426747 * 3600
    if presaleWeiRaised < 100 * 10^18:
        emit RefundsEnabled()
    else:
    ...
    isPresaleFinalized = 1
}

function claimRefund() {
    if not isCrowdsaleFinalized:
        require isPresaleFinalized
        require presaleWeiRaised < 100 * 10^18
...
function buyTokens(address arg1) payable {
    ...
    require arg1
    require msg.value
    if block.timestamp < 426495 * 3600:
        require block.timestamp >= 426495 * 3600
        require stor12[address(arg1)]
        require msg.value
        require msg.value
        require 500000 * msg.value / msg.value == 500000
        require 500000 * msg.value / 10^18
        require presaleWeiRaised + msg.value >= presaleWeiRaised
        presaleWeiRaised += msg.value
        require totalSupply + (500000 * msg.value / 10^18) >= totalSupply
        totalSupply += 500000 * msg.value / 10^18
        require balanceOf[address(arg1)] + (500000 * msg.value / 10^18) >= balanceOf[address(arg1)]
        balanceOf[address(arg1)] += 500000 * msg.value / 10^18
        emit Transfer((500000 * msg.value / 10^18), 0, arg1);
        emit TokenPurchase(msg.value, 500000 * msg.value / 10^18, msg.sender, arg1);
        if block.timestamp < 427095 * 3600:
            if block.timestamp >= 426495 * 3600:
        ...
        if block.timestamp > 426747 * 3600:
            if block.timestamp < 427095 * 3600:
                require block.timestamp <= 426747 * 3600
                require stor12[address(arg1)]
                require msg.value
                ...
                if block.timestamp > 429267 * 3600:
                    require block.timestamp <= 426747 * 3600
                    require stor12[address(arg1)]
                    require msg.value
                    require msg.value
                    ...
                    if presaleWeiRaised < 100 * 10^18:
                        ...
                        if crowdsaleWeiRaised + msg.value > 20000 * 10^18:
                            require block.timestamp <= 426747 * 3600
                            require stor12[address(arg1)]
                            require msg.value
                            require msg.value
            ...
            require presaleWeiRaised + msg.value >= presaleWeiRaised
            if block.timestamp < 427095 * 3600:
                require presaleWeiRaised + msg.value <= 2500 * 10^18
                require stor12[address(arg1)]
                require msg.value
                require msg.value
                require 500000 * msg.value / msg.value == 500000
                ...
                emit TokenPurchase(msg.value, 500000 * msg.value / 10^18, msg.sender, arg1);
                if block.timestamp < 427095 * 3600:
                    if presaleWeiRaised + msg.value <= 2500 * 10^18:
                        require presaleDeposited[address(arg1)] + msg.value >= presaleDeposited[address(arg1)]
                        presaleDeposited[address(arg1)] += msg.value
                else:
                    require crowdsaleContributions[address(arg1)] + msg.value >= crowdsaleContributions[address(arg1)]
...
function _fallback() payable {
    ...
    if block.timestamp < 426495 * 3600:
        ...
        require totalSupply + (500000 * msg.value / 10^18) >= totalSupply
        totalSupply += 500000 * msg.value / 10^18
        require balanceOf[address(msg.sender)] + (500000 * msg.value / 10^18) >= balanceOf[address(msg.sender)]
        balanceOf[address(msg.sender)] += 500000 * msg.value / 10^18
        emit Transfer((500000 * msg.value / 10^18), 0, msg.sender);
        emit TokenPurchase(msg.value, 500000 * msg.value / 10^18, msg.sender, msg.sender);
        if block.timestamp < 427095 * 3600:
            if block.timestamp >= 426495 * 3600:
