...
function getPoolBalance(uint256 arg1) {
    require calldata.size - 4 >= 32
    return pool[arg1]
}
...
function sub_c07904ac(?) payable {
    ...
    if (block.timestamp - startTime / roundTime) + 1 > 1:
        if not balances[(block.timestamp - stor5 / stor6) + 1]:
            balances[stor13] = 0
            require pool[stor13] + balances[(block.timestamp - stor5 / stor6) + 1] >= balances[(block.timestamp - stor5 / stor6) + 1]
            balances[(block.timestamp - stor5 / stor6) + 1] += pool[stor13]
    ...
    if not msg.value:
        if not msg.value:
            if not msg.value:
                ...
                require pool[(block.timestamp - stor5 / stor6) + 1] >= pool[(block.timestamp - stor5 / stor6) + 1]
            ...
            if not msg.value:
                ...
                require (sub_e082a8b3 * msg.value / 100) + pool[(block.timestamp - stor5 / stor6) + 1] >= pool[(block.timestamp - stor5 / stor6) + 1]
                pool[(block.timestamp - stor5 / stor6) + 1] += sub_e082a8b3 * msg.value / 100
                call jackpotAddress with:
                     gas 2300 wei
            else:
                require jackpotPercent * msg.value / msg.value == jackpotPercent
