...
function sub_58867762(?) {
    ...
    idx = 0
    s = 1000 * arg2 / stor7
    while idx < ext_call.return_data[0] / stor9:
        if not s:
            idx = idx + 1
            s = 0
            continue 
...
function _fallback() payable {
    ...
    if not msg.value:
        ...
        require stor9
        idx = 0
        s = msg.value * ext_call.return_data[0] / stor7
        while idx < ext_call.return_data[0] / stor9:
            if not s:
                idx = idx + 1
                s = 0
    ...
    require return_data.size >= 32
    require ext_call.return_data[0]
    emit TokensPurchased(msg.value, s, msg.sender, msg.sender);
...
function buyTokens(address arg1) payable {
    ...
    require return_data.size >= 32
    require ext_call.return_data[0]
    emit TokensPurchased(msg.value, s, msg.sender, arg1);
    call walletAddress with:
       value msg.value wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
