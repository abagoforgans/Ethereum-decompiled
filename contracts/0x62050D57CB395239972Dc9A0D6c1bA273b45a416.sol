...
function getTokens() payable {
    ...
    if tokensPerEth:
        ...
        if tokensPerEth * msg.value / 10^16 > 0:
            require not stor7
            require totalDistributed + (tokensPerEth * msg.value / 10^16) >= totalDistributed
            totalDistributed += tokensPerEth * msg.value / 10^16
            require balanceOf[address(msg.sender)] + (tokensPerEth * msg.value / 10^16) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += tokensPerEth * msg.value / 10^16
            emit Distr((tokensPerEth * msg.value / 10^16), msg.sender);
            emit Transfer((tokensPerEth * msg.value / 10^16), 0, msg.sender);
    if totalDistributed >= totalSupply:
        stor7 = 1
}
