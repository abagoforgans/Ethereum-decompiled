...
function _fallback() payable {
    ...
    stor7 = 100
    stor6 = code.data[4769 len 32]
    stor3 = 417648 * 24 * 3600
    stor4 = 1504223999
    stor5 = 500
    return code.data[268 len 1955]
}
...
function _fallback() payable {
    ...
    require 10 * 10^6 * msg.value / 10 * 10^6 == msg.value
    require rate
    require 10 * 10^6 * msg.value / 10^18 / rate > 0
    require ext_code.size(tokenAddress)
    call tokenAddress.0x40c10f19 with:
         gas gas_remaining - 710 wei
        args msg.sender, 10 * 10^6 * msg.value / 10^18 / rate
    ...
    require (10 * 10^6 * msg.value / 10^18 / rate) + tokenAmountOf[address(msg.sender)] >= tokenAmountOf[address(msg.sender)]
    tokenAmountOf[address(msg.sender)] += 10 * 10^6 * msg.value / 10^18 / rate
    require msg.value + weiRaised >= weiRaised
    weiRaised += msg.value
    require (10 * 10^6 * msg.value / 10^18 / rate) + tokensSold >= tokensSold
    tokensSold += 10 * 10^6 * msg.value / 10^18 / rate
    call stor1 with:
       value msg.value wei
