...
function _fallback() payable {
    ...
    stor4.length.field_8 = 'Love Air Coffee' / 256
    idx = 0
    ...
    stor7 = code.data[6638 len 32] * 10^stor6
    stor2[address(msg.sender)] = stor7
    emit Transfer(stor7, 0, stor1);
    return code.data[821 len 5817]
}
...
function setFrozenCoin(bool arg1) {
    require msg.sender == stor1
...
function startBuyingTokens(bool arg1, uint256 arg2, uint256 arg3) {
    require msg.sender == stor1
    require state <= 1
    require not state
    uint8(stor9.field_0) = uint8(arg1)
    minEther = arg2 * 10^decimals
    maxEther = arg3 * 10^decimals
    state = 1
}
