...
function _fallback() {
    stor0 = msg.sender
    stor2 = code.data[4530 len 20]
    stor3 = code.data[4562 len 20]
    return code.data[133 len 4385]
}
...
const MIN_CAP = 2948 * 10^10

const COIN_PER_ETHER = 10^9

const MAX_CAP = 6348 * 10^10
...
function withdrawPayments() {
    require payments[address(stor0.field_0)]
    require eth.balance(this.address) >= payments[address(stor0.field_0)]
    payments[address(stor0.field_0)] = 0
    call owner with:
       value payments[address(stor0.field_0)] wei
         gas 2300 * is_zero(value) wei
    require ext_call.success
    emit LogRefundETH(owner, payments[address(stor0.field_0)]);
}

function finalize() {
    require owner == msg.sender
    if block.timestamp < endTime:
        require coinSentToEther == 6348 * 10^10
    if coinSentToEther < 2948 * 10^10:
...
function refund(uint256 arg1) {
    ...
    require coinSentToEther < 2948 * 10^10
...
function getRemainCoins() {
    ...
    if block.timestamp >= startTime + (48 * 24 * 3600):
        require -coinSentToEther + 6348 * 10^10 <= 50 * 10^6
    else:
        require -coinSentToEther + 6348 * 10^10 <= 60 * 10^6
    require ext_code.size(coinAddress)
    call coinAddress.0xa9059cbb with:
         gas gas_remaining - 710 wei
        args owner, -coinSentToEther + 6348 * 10^10
    require ext_call.success
    require -coinSentToEther + backers[address(stor0.field_0)].field_256 + 6348 * 10^10 >= backers[address(stor0.field_0)].field_256
    backers[address(stor0.field_0)].field_256 = -coinSentToEther + backers[address(stor0.field_0)].field_256 + 6348 * 10^10
    require 6348 * 10^10 >= coinSentToEther
    coinSentToEther = 6348 * 10^10
    emit LogCoinsEmited((-coinSentToEther + 6348 * 10^10), this.address);
...
function _fallback() payable {
    ...
    if block.timestamp >= startTime + (48 * 24 * 3600):
        ...
        require coinSentToEther + (10^9 * msg.value / 10^18) <= 6348 * 10^10
        require ext_code.size(coinAddress)
        call coinAddress.0xa9059cbb with:
             gas gas_remaining - 710 wei
        ...
        require coinSentToEther + (10^9 * msg.value / 10^18 / 5) + (10^9 * msg.value / 10^18) <= 6348 * 10^10
        require ext_code.size(coinAddress)
        call coinAddress.0xa9059cbb with:
             gas gas_remaining - 710 wei
