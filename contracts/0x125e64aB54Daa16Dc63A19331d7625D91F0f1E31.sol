...
function _fallback() payable {
    ...
    require code.data[3889 len 32] <= 10000
    stor2 = code.data[3889 len 32]
    require ext_code.size(address(code.data[3857 len 32]))
    call address(code.data[3857 len 32]).supportsInterface(bytes4 rg1) with:
         gas gas_remaining - 710 wei
        args 0x9a20483d00000000000000000000000000000000000000000000000000000000
    require ext_call.success
    require ext_call.return_data[0]
    stor1 = address(code.data[3857 len 32])
    return code.data[386 len 3471]
}
