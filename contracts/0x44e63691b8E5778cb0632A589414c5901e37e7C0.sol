...
uint8 preIcoActive;
uint8 icoActive; offset 8
uint8 preBountyAdded; offset 16
uint8 bountyAdded; offset 24
uint8 ownersStakeAdded; offset 32
address stor12;
...
function ownersStakeAdded() {
    return bool(ownersStakeAdded)
}

function icoActive() {
    return bool(icoActive)
}
...
function stopIco(address arg1) {
    require msg.sender == owner
    require not bountyAdded
    require not ownersStakeAdded
    require arg1
    Mask(248, 0, stor12.field_8) = 0
    preIcoActive = 0
    bool(currentState.length) = 0
