...
const CROWDFUNDING_PERIOD = (672 * 24 * 3600)

const TOKEN_TARGET = 500000800

const TOKEN_LOCKING_PERIOD = (17520 * 24 * 3600)
...
function twoYearsPassed() payable {
    require msg.value <= 0
    return block.timestamp - startDate >= 17520 * 24 * 3600
}
            ...
            if 0 == stage:
                if msg.value >= valuePerShare:
                    call address(singularDTVTokenAddress).0x18160ddd with:
                         gas gas_remaining - 25050 wei
                    if ext_call.success:
                        if ext_call.return_data[0] + (msg.value / valuePerShare) <= 500001000:
                            if msg.value <= msg.value / valuePerShare * valuePerShare:
                                ...
                                call address(singularDTVTokenAddress).0x475a9fa9 with:
                                     gas gas_remaining - 25050 wei
                                        ...
                                        if stage != 0:
                                            if stage != 2:
                                                ...
                                                call address(singularDTVTokenAddress).0x18160ddd with:
                                                     gas gas_remaining - 25050 wei
                                                if ext_call.success:
                                                    if 500001000 == ext_call.return_data[0]:
                                                        stage = 3
                                                    if fundBalance <= eth.balance(this.address):
                                                        return (msg.value / valuePerShare)
                                        else:
                            ...
                            call address(singularDTVTokenAddress).0x18160ddd with:
                                 gas gas_remaining - 25050 wei
                            if ext_call.success:
                                if msg.value <= (500001000 * valuePerShare) - (ext_call.return_data[0] * valuePerShare):
                                    fundBalance = (500001000 * valuePerShare) - (ext_call.return_data[0] * valuePerShare) + fundBalance
                                    investments[address(msg.sender)] = (500001000 * valuePerShare) - (ext_call.return_data[0] * valuePerShare) + investments[address(msg.sender)]
                                    call address(singularDTVTokenAddress).0x475a9fa9 with:
                                         gas gas_remaining - 25050 wei
                                        args msg.sender, -ext_call.return_data[0] + 500001000
                                    if ext_call.success:
                                        if ext_call.return_data[0]:
                                            if stage != 0:
                                                if stage != 2:
                                                    if fundBalance <= eth.balance(this.address):
                                                        return (-ext_call.return_data[0] + 500001000)
                                                else:
                                                    call address(singularDTVTokenAddress).0x18160ddd with:
                                                         gas gas_remaining - 25050 wei
                                                    if ext_call.success:
                                    ...
                                    call msg.sender with:
                                       value msg.value - (500001000 * valuePerShare) + (ext_call.return_data[0] * valuePerShare) wei
                                         gas 0 wei
                                    if ext_call.success:
                                        fundBalance = (500001000 * valuePerShare) - (ext_call.return_data[0] * valuePerShare) + fundBalance
                                        investments[address(msg.sender)] = (500001000 * valuePerShare) - (ext_call.return_data[0] * valuePerShare) + investments[address(msg.sender)]
