...
uint256 migrationAccountCounter;
...
function daoStakeClaimed() {
    return bool(uint8(stor1.field_176))
}

function devRewardClaimed() {
...
function migrationAccountCounter() {
    return migrationAccountCounter
}
...
function getCurrentDaoStakeSupply() {
    return (realVotedSupply - (realVotedSupply * realDevReward / 10000) / 100 * 10^6)
}
...
function getCurrentDevReward() {
    return ((realVotedSupply * realDevReward) - (creditsExchanged * realDevReward) / 10000 / 100 * 10^6)
}
...
function claimDevReward(address arg1) {
    ...
    call creditbitAddress.mintMigrationTokens(address rg1, uint256 rg2) with:
         gas gas_remaining - 50 wei
        args address(arg1), (realVotedSupply * realDevReward) - (creditsExchanged * realDevReward) / 10000
    require ext_call.success
    if ext_call.return_data[0]:
        return 1
    creditsExchanged += (realVotedSupply * realDevReward) - (creditsExchanged * realDevReward) / 10000
...
function addNewAccount(string arg1, address arg2, uint256 arg3, uint256 arg4, uint256 arg5) {
    ...
    if stor10[address(arg2)]:
        ...
        stor10[address(arg2)] = migrationAccountCounter
