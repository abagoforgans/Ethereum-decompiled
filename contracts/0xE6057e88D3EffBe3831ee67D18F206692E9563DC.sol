...
function remove() payable {
    ...
    call address(stor0).hasRelation(bytes32 rg1, bytes32 rg2, address rg3) with:
        ...
        args stor1.length, 0x6f776e6572000000000000000000000000000000000000000000000000000000, msg.sender
...
function transactions(uint256 arg1) payable {
    ...
    return stor[(2 * arg1) + code.data[2981 len 32]], stor568A[arg1]
}
                                ...
                                if counter >= periodicity:
                                    ...
                                    while uint8(idx) < stor4.length:
                                        ...
                                        if stor568A[2 * uint8(idx)] != t:
                                            ...
                                            mem[(32 * s) + 228] = stor[code.data[2981 len 32] + (2 * uint8(idx))]
                                            require uint8(idx) < stor4.length
                                            mem[0] = 4
                                            if stor568A[2 * uint8(idx)] <= t:
                                                idx = idx + 1
                                        ...
                                        mem[228] = stor[code.data[2981 len 32] + (2 * uint8(idx))]
                                        idx = idx + 1
                                        s = 1
                                        ...
                                        if msize < (32 * uint8(auctionSize)) + 228:
                                            ...
                                            if startTime > block.timestamp:
                                                ...
                                                while uint8(w) < v:
                                                    ...
                                                    call address(elcoinAddress).rewardTo(address rg1, uint256 rg2) with:
                                                         gas gas_remaining - 25050 wei
                                    ...
                                    round++
                                    stor4.length = 0
                                    idx = code.data[2981 len 32]
                                    while code.data[2981 len 32] + (2 * stor4.length) > idx:
                                        stor[idx] = 0
                                        stor1[idx] = 0
                                        idx = idx + 2
                                        continue 
                                        ...
                                        if counter >= periodicity:
                                            ...
                                            while uint8(idx) < stor4.length:
                                                ...
                                                if stor568A[2 * uint8(idx)] != t:
                                                    ...
                                                    require s < uint8(auctionSize)
                                                    mem[(32 * s) + 256] = stor[code.data[2981 len 32] + (2 * uint8(idx))]
                                                    require uint8(idx) < stor4.length
                                                    mem[0] = 4
                                                    if stor568A[2 * uint8(idx)] <= t:
                                                        idx = idx + 1
                                                ...
                                                mem[0] = 4
                                                require 0 < uint8(auctionSize)
                                                mem[256] = stor[code.data[2981 len 32] + (2 * uint8(idx))]
                                                idx = idx + 1
                                                s = 1
                                                t = stor568A[2 * uint8(idx)]
                                                continue 
