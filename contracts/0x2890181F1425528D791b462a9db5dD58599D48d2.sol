...
function _fallback() payable {
    ...
    stor0.length.field_8 = 'Afreet Token 6' / 256
    idx = 0
        ...
        if block.timestamp <= startTime + 600:
            ...
            if block.timestamp < startTime + 900:
                ...
                if block.timestamp <= startTime + 1500:
                    if PRICE:
                        require PRICE
                        require 35 * PRICE / PRICE == 35
                    if msg.value:
                        require msg.value
                        require 35 * PRICE / 20 * msg.value / msg.value == 35 * PRICE / 20
                    require _icoSupply >= 35 * PRICE / 20 * msg.value
                    require 35 * PRICE / 20 * msg.value <= balanceOf[stor9]
                    balanceOf[stor9] += -1 * 35 * PRICE / 20 * msg.value
                    require (35 * PRICE / 20 * msg.value) + balanceOf[arg1] >= balanceOf[arg1]
                    balanceOf[address(arg1)] = (35 * PRICE / 20 * msg.value) + balanceOf[arg1]
                    require 35 * PRICE / 20 * msg.value <= _icoSupply
                    _icoSupply += -1 * 35 * PRICE / 20 * msg.value
                    emit TokenPurchase(msg.value, 35 * PRICE / 20 * msg.value, msg.sender, arg1);
                    if 35 * PRICE / 20 * msg.value:
                        call owner with:
                           value msg.value wei
                             gas 2300 * is_zero(value) wei
                    else:
        ...
        if block.timestamp <= startTime + 600:
            ...
            if block.timestamp < startTime + 900:
                ...
                if block.timestamp <= startTime + 1500:
                    ...
                    require 35 * PRICE / 20 * msg.value <= balanceOf[stor9]
                    balanceOf[stor9] += -1 * 35 * PRICE / 20 * msg.value
                    require (35 * PRICE / 20 * msg.value) + balanceOf[msg.sender] >= balanceOf[msg.sender]
                    balanceOf[address(msg.sender)] = (35 * PRICE / 20 * msg.value) + balanceOf[msg.sender]
                    require 35 * PRICE / 20 * msg.value <= _icoSupply
                    _icoSupply += -1 * 35 * PRICE / 20 * msg.value
                    emit TokenPurchase(msg.value, 35 * PRICE / 20 * msg.value, msg.sender, msg.sender);
                    if 35 * PRICE / 20 * msg.value:
                        call owner with:
                           value msg.value wei
                             gas 2300 * is_zero(value) wei
