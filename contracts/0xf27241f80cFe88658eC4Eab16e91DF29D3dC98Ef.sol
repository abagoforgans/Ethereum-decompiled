        ...
        if msg.value < 10^16:
            ...
            call address(owner) with:
               value msg.value / 10 wei
                 gas 0 wei
            fee = 0
            balanceLimit = 8 * balance / 10
            if 9 * msg.value / 10 <= 8 * balance / 10:
                if bool(block.gasprice * block.timestamp) != 0:
                ...
                call msg.sender with:
                   value (9 * msg.value / 10) - balanceLimit wei
                     gas 0 wei
                if bool(block.gasprice * block.timestamp) != 0:
                    balance += balanceLimit
                else:
                    call msg.sender with:
                       value 2 * balanceLimit wei
                         gas 0 wei
}
