...
const _fallback = code.data[25 len 7245]
...
uint8 alwaysRequireCosignature; offset 160
...
function alwaysRequireCosignature() {
    return bool(alwaysRequireCosignature)
}
...
function enableSecureMode() {
    if not stor0[address(msg.sender)]:
        if msg.sender != this.address:
            emit Error(0x4163636573732064656e696564000000000000000000000000000000000000);
            return 0
    if alwaysRequireCosignature:
        if cosignerAddress:
            ...
            if not ext_call.return_data[0]:
                emit Error(0x436f7369676e65723a206163636573732064656e6965640000000000000000);
                return 0
    stor1 = 1
    return 1
}
...
function forward(address arg1, uint256 arg2, bytes arg3) {
    ...
    if not arg3.length:
        if not arg3.length % 32:
            require ext_code.size(proxyAddress)
            call proxyAddress.forward(address rg1, uint256 rg2, bytes rg3, bool rg4) with:
                 gas gas_remaining - 50 wei
