...
function sub_f602d622(?) {
    ...
    call tokenAddress.0xa9059cbb with:
        ...
        args msg.sender, loanAmount
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
}

function sub_4cd54218(?) payable {
    require currentState <= 4
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if ext_call.return_data[0] == loanAmount:
        amountRaised += loanAmount
        currentState = 1
}

function sub_5d8a04e6(?) {
    require msg.sender == sub_7ba61ea5Address
    require currentState <= 4
    ...
    payments[payments.length].field_512 = sub_8770998e
    require ext_code.size(tokenAddress)
    call tokenAddress.0xa9059cbb with:
         gas gas_remaining wei
...
function sub_9e0fb0fe(?) {
    require msg.sender == sub_7ba61ea5Address
    require currentState <= 4
    require currentState == 2
    sub_ee86afd8 += sub_8770998e
    require sub_8770998e + (arg1 - sub_8770998e / 2) >= sub_8770998e
    payments.length++
    payments[payments.length].field_0 = count
    payments[payments.length].field_256 = block.timestamp
    payments[payments.length].field_512 = arg1
    require ext_code.size(tokenAddress)
    call tokenAddress.0xa9059cbb with:
         gas gas_remaining wei
        args lenderAddress, sub_8770998e + (arg1 - sub_8770998e / 2)
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_code.size(tokenAddress)
    call tokenAddress.0xa9059cbb with:
         gas gas_remaining wei
        args address(arg2), arg1 - sub_8770998e / 2
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    count++
...
function sub_5cdb9829(?) payable {
    ...
    while idx < sub_67e198c8:
        ...
        mem[64] = mem[64] + 96
        mem[_15] = idx
        mem[_15 + 32] = block.timestamp + (24 * 3600 * s) + (24 * 3600 * sub_95950218)
        mem[_15 + 64] = sub_8770998e
        schedule.length++
        mem[0] = 17
        ...
        mem[64] = mem[64] + 96
        mem[_20] = idx
        mem[_20 + 32] = block.timestamp + s + sub_95950218 + (24 * 3600 * sub_97394d9a)
        mem[_20 + 64] = sub_aa9826d2
        sub_bfe59fc2.length++
        mem[0] = 19
