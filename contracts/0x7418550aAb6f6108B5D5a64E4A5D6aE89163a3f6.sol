...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 160 * value / value == 160
        if 160 * value:
            require 160 * value
            require 160 * value * msg.value / 160 * value == msg.value
        ...
        if 160 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (160 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 160 * value * msg.value / 10^18
            require 160 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 160 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (160 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 160 * value * msg.value / 10^18
            emit Distr((160 * value * msg.value / 10^18), msg.sender);
            emit Transfer((160 * value * msg.value / 10^18), 0, msg.sender);
        else:
