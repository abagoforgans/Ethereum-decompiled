...
function getConfirmations(uint256 arg1) {
    if stor3.length:
        mem[128 len 32 * stor3.length] = code.data[5385 len 32 * stor3.length]
    idx = 0
    s = 0
    while idx < stor3.length:
        mem[0] = address(stor3[idx].field_0)
        ...
        mem[(32 * s) + 128] = address(stor3[idx].field_0)
        idx = idx + 1
        s = s + 1
        continue 
    mem[(32 * stor3.length) + 128] = s
    if s:
        mem[(32 * stor3.length) + 160 len 32 * s] = code.data[5385 len 32 * s]
    t = 0
    while t < s:
...
function removeOwner(address arg1) {
    ...
    while idx < stor3.length - 1:
        ...
        if required > stor3.length:
            ...
            emit RequirementChange(stor3.length);
        emit OwnerRemoval(arg1);
    stor3.length--
    if stor3.length > stor3.length - 1:
        idx = stor3.length - 1
...
function getTransactionIds(uint256 arg1, uint256 arg2, bool arg3, bool arg4) {
    if transactionCount:
        mem[128 len 32 * transactionCount] = code.data[5385 len 32 * transactionCount]
    idx = 0
    s = 0
    while idx < transactionCount:
        if not arg3:
    ...
    if arg2 - arg1:
        mem[(32 * transactionCount) + 160 len 32 * arg2 - arg1] = code.data[5385 len 32 * arg2 - arg1]
    idx = arg1
    while idx < arg2:
