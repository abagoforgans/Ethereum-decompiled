...
function _fallback() {
    mem[96 len -4184] = code.data[4405 len -4184]
    mem[64] = -4088
    stor1 = mem[mem[96] + 96]
    idx = 0
            ...
            if stor259[call.data[0 len calldata.size]].field_0 > 1:
                ...
                if arg1 <= m_numOwners:
                    ...
                    while idx < stor260.length:
                        ...
                        s = sha3(sha3(stor260[s], 261) + 2)
                        while sha3(sha3(stor260[s], 261) + 2) + (stor261[stor260[s]][2].length + 31 / 32) > s:
                            stor[s] = 0
                            s = s + 1
                            continue 
                        s = sha3(sha3(stor260[s], 261) + 2) + (stor261[stor260[s]][2].length + 31 / 32) + 1
                        continue 
                    idx = 0
                    while idx < stor260.length:
                        mem[0] = 260
...
function execute(address arg1, uint256 arg2, bytes arg3) {
    ...
    if not stor258[address(msg.sender)]:
        if not address(stor261[call.data[0 len calldata.size]][block.number].field_0):
            address(stor261[call.data[0 len calldata.size]][block.number].field_0) = arg1
