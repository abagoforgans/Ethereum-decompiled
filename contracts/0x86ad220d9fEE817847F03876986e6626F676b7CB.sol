...
function hariKari() {
    if owner != msg.sender:
        emit code.data[4630 len 32]: 'err: not owner'
    else:
        require settingsState <= 1
        if settingsState != 1:
            selfdestruct(owner)
        emit code.data[4630 len 32]: 'err: locked'
}
...
function lock() {
    if owner != msg.sender:
        ...
        if settingsState == 1:
            ...
            settingsState = 1
            emit code.data[4630 len 32]: Array(len=19, data='ok: contract locked')
}
...
function reset() {
    if owner != msg.sender:
        ...
        if settingsState == 1:
            ...
            emit code.data[4630 len 32]: Array(len=19, data='ok: all accts reset')
}
...
function getAccountInfo(address arg1) {
    ...
    while idx < stor2.length:
        ...
        if stor1[idx].field_768 != arg1:
            ...
            idx = idx + 1
            continue 
        emit code.data[4630 len 32]: Array(len=14, data='ok: found acct')
        return idx, stor1[idx].field_512, bool(stor1[idx].field_928), stor1[idx].field_0, stor1[idx].field_256
    emit code.data[4630 len 32]: Array(len=19, data='err: acct not found')
    return 0
}
...
function addAccount(address arg1, uint256 arg2, bool arg3) {
    if owner != msg.sender:
        ...
        if settingsState == 1:
            ...
            if stor2.length >= 5:
                emit code.data[4630 len 32]: Array(len=17, data='err: max accounts')
            else:
                ...
                stor2.length++
                emit code.data[4630 len 32]: Array(len=14, data='ok: acct added')
}
...
function withdraw() payable {
    ...
    while idx < stor2.length:
        ...
        if not stor1[idx].field_256:
            ...
            mem[0] = idx
            emit code.data[4630 len 32]: Array(len=20, data='err: balance is zero')
        else:
            mem[0] = idx
            mem[32] = 1
        ...
        if totalFundsDistributed >= evenDistThresh:
            ...
            if holdoverBalance > 0:
                ...
                while idx < stor2.length:
                    ...
                    idx = idx + 1
                    continue 
        emit code.data[4630 len 32]: Array(len=21, data='ok: distributed funds')
}
