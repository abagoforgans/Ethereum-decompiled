...
function createTokens(address arg1) payable {
    ...
    if block.timestamp >= stor2 + (24 * 3600 * stor4):
        call tokenAddress.mint(address rg1, uint256 rg2) with:
             gas gas_remaining - 710 wei
            args address(arg1), stor7 * msg.value / 10^18
    else:
        call tokenAddress.mint(address rg1, uint256 rg2) with:
             gas gas_remaining - 710 wei
            args address(arg1), (stor7 * msg.value / 10^18) + (stor7 * msg.value / 10^18 / 5)
    require ext_call.success
    call stor5 with:
       value msg.value wei
         gas 2300 * is_zero(value) wei
