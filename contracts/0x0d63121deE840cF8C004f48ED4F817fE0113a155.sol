...
function register() payable {
    ...
    sub_625d0be9[msg.sender][gas_remaining][msg.value].field_0 = sha3(msg.sender, gas_remaining, msg.value)
    sub_625d0be9[msg.sender][gas_remaining][msg.value].field_256 = 0
    idx = 0
    while sub_625d0be9[msg.sender][gas_remaining][msg.value].field_256 > idx:
        sub_625d0be9[msg.sender][gas_remaining][msg.value][idx + 1].field_0 = 0
        idx = idx + 1
        continue 
    addressPresent[address(msg.sender)] = 1
    addressToPersonId[address(msg.sender)] = sha3(msg.sender, gas_remaining, msg.value)
    return sha3(msg.sender, gas_remaining, msg.value)
}
