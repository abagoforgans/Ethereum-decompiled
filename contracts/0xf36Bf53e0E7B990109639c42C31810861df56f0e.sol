...
function sub_eaf601b6(?) payable {
    ...
    return address(stor[code.data[1928 len 32] + arg1])
}
...
function sign() payable {
    ...
    while uint8(idx) < sub_8801f08e:
        ...
        mem[0] = 2
        if address(stor[code.data[1928 len 32] + idx]) == msg.sender:
...
function sub_31044037(?) payable {
    ...
    while 416 > idx:
        ...
        idx = idx + 32
        continue 
    idx = code.data[1928 len 32] + 10
    while code.data[1928 len 32] + stor2.length > idx:
        address(stor[idx]) = 0
        idx = idx + 1
        continue 
    require 0 < stor2.length
    emit 0xcbd53414: address(stor[code.data[1928 len 32]])
    require 1 < stor2.length
    emit 0xcbd53414: stor4057
    sub_8801f08e = arg3
