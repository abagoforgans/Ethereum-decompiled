...
function _fallback() {
    mem[96 len -9356] = code.data[10085 len -9356]
    mem[64] = -9260
...
uint256 sub_733e1b68;
mapping of uint256 sub_19b83a7b;
...
function sub_19b83a7b(?) {
    return sub_19b83a7b[arg1]
}
    ...
    while idx < owners.length:
        ...
        if not stor1[arg1][address(stor3[idx].field_0)]:
            ...
            if s + 1 != required:
                ...
                s = s + 1
                continue 
        sub_19b83a7b[arg1] = block.timestamp
...
function executeTransaction(uint256 arg1) payable {
    ...
    require not uint8(stor0[arg1].field_768)
    require var12001 < owners.length
    mem[0] = address(owners[var12001].field_0)
    mem[32] = sha3(arg1, 1)
    if not stor1[arg1][address(stor3[var12001].field_0)]:
        if var12002 != required:
            var12001 = var12001 + 1
            var12002 = var12002
            continue 
    else:
        if var12002 + 1 != required:
            var12001 = var12001 + 1
            var12002 = var12002 + 1
            continue 
    require block.timestamp >= sub_19b83a7b[arg1] + sub_733e1b68
...
function submitTransaction(address arg1, uint256 arg2, bytes arg3) {
    ...
    while idx < owners.length:
        ...
        if not stor1[stor5][address(stor3[idx].field_0)]:
            require s != required
            idx = idx + 1
            s = s
            continue 
    ...
    while idx < owners.length:
        ...
        if not stor1[stor5][address(stor3[idx].field_0)]:
            ...
            if s + 1 != required:
                ...
                s = s + 1
                continue 
        sub_19b83a7b[stor5] = block.timestamp
...
function sub_215ff46f(?) payable {
    ...
    while stor0[arg1][2].length + 96 > idx:
        ...
        s = s + 1
        continue 
    mem[ceil32(stor0[arg1][2].length) + 128] = 'pause()'
    idx = 0
    while idx < 4:
        require idx < stor0[arg1][2].length
        require Mask(8, 248, mem[idx + 128]) == Mask(8, -(('mask_shl', 256, 0, -3, ('sha3', "'pause()'")), 0) + 256, idx) << (('mask_shl', 256, 0, -3, ('sha3', "'pause()'")), 0) - 8
        idx = idx + 1
        continue 
    uint8(stor0[arg1].field_768) = 1
    mem[ceil32(stor0[arg1][2].length) + 128] = stor0[arg1][2].field_0
    idx = ceil32(stor0[arg1][2].length) + 128
    s = 0
    while ceil32(stor0[arg1][2].length) + stor0[arg1][2].length + 128 > idx + 32:
        mem[idx + 32] = stor0[arg1][s + 2].field_256
        idx = idx + 32
        s = s + 1
        continue 
    call address(stor0[arg1].field_0).mem[ceil32(stor0[arg1][2].length) + 128 len 4] with:
       value stor0[arg1].field_256 wei
         gas gas_remaining - 34710 wei
        args mem[ceil32(stor0[arg1][2].length) + 132 len stor0[arg1][2].length + (floor32(stor0[arg1][2].length - 1) + -stor0[arg1][2].length + 32 % 32) - 4]
    if ext_call.success:
        emit Execution(arg1);
    else:
        emit ExecutionFailure(arg1);
