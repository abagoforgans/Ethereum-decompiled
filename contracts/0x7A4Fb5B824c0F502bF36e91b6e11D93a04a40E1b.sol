...
function proofOfWork(uint256 arg1) {
    require sha3(arg1, currentChallenge) >= difficulty
    require block.timestamp - timeOfLastProof >= 1
    require 8 * block.timestamp - timeOfLastProof
    require (timeOfLastProof + 3)^2 / 8 * block.timestamp - timeOfLastProof
    balanceOf[msg.sender] = ((block.timestamp - timeOfLastProof / 3600)^2 / (timeOfLastProof + 3)^2 / 8 * block.timestamp - timeOfLastProof) + balanceOf[msg.sender] + 755
    require block.timestamp - timeOfLastProof
    difficulty = (600 * difficulty / block.timestamp - timeOfLastProof) + 1
