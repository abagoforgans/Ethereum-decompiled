...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 800 * value / value == 800
        if 800 * value:
            require 800 * value
            require 800 * value * msg.value / 800 * value == msg.value
        ...
        if 800 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (800 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 800 * value * msg.value / 10^18
            require 800 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 800 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (800 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 800 * value * msg.value / 10^18
            emit Distr((800 * value * msg.value / 10^18), msg.sender);
            emit Transfer((800 * value * msg.value / 10^18), 0, msg.sender);
        else:
