...
function _fallback() payable {
    ...
    if not msg.value:
        stor0 = msg.sender
        require code.data[6024 len 20]
        require code.data[6056 len 20]
        stor3 = code.data[6024 len 20]
        stor4 = code.data[6056 len 20]
        if ext_code.size(stor3):
            call stor3.0x15226b54 with:
                 gas gas_remaining - 710 wei
                args stor4
            if ext_call.success:
                return code.data[871 len 5141]
    revert
}
...
function _fallback() payable {
    ...
    require ext_call.return_data[0]
    require ext_code.size(address(etherTokenAddress))
    call address(etherTokenAddress).0x23b872dd with:
         gas gas_remaining - 710 wei
        args tokenChangerAddress, msg.sender, changeAmount
    require ext_call.success
    require ext_call.return_data[0]
