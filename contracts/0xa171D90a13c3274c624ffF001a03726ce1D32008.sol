    ...
    if block.timestamp < stor22:
        ...
        if stor22 > block.timestamp:
            if stor24 > block.timestamp:
                if stor26 > block.timestamp:
                    ...
                    if block.timestamp > stor27:
                        ...
                        if arg1:
                            require arg1
                            require rate * arg1 / arg1 == rate
                        if rate * arg1:
                            require rate * arg1
                            require 110 * rate * arg1 / rate * arg1 == 110
                        require (110 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                        if (110 * rate * arg1 / 100) + tokenAllocated <= stor30:
                            emit CurrentPeriod(3);
                            require (110 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                            if (110 * rate * arg1 / 100) + tokenAllocated > balanceOf[stor6]:
                                emit TokenLimitReached(tokenAllocated, 110 * rate * arg1 / 100, msg.sender);
                            else:
                                require arg1 + weiRaised >= weiRaised
                                if arg1 + weiRaised <= hardWeiCap:
                                    if arg1 >= weiMinSale:
                                        return (110 * rate * arg1 / 100)
                                else:
                                    emit HardCapReached()
                        else:
                            emit CurrentPeriod(0);
                ...
                if block.timestamp > stor25:
                    ...
                    if rate * arg1:
                        require rate * arg1
                        require 120 * rate * arg1 / rate * arg1 == 120
                    require (120 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                    if (120 * rate * arg1 / 100) + tokenAllocated <= stor29:
                        emit CurrentPeriod(2);
                        require (120 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                        if (120 * rate * arg1 / 100) + tokenAllocated > balanceOf[stor6]:
                            emit TokenLimitReached(tokenAllocated, 120 * rate * arg1 / 100, msg.sender);
                        else:
                            require arg1 + weiRaised >= weiRaised
                            if arg1 + weiRaised <= hardWeiCap:
                                if arg1 >= weiMinSale:
                                    return (120 * rate * arg1 / 100)
                            else:
                                emit HardCapReached()
                    else:
                        if arg1:
            ...
            if block.timestamp > stor23:
                ...
                if rate * arg1:
                    require rate * arg1
                    require 130 * rate * arg1 / rate * arg1 == 130
                require (130 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                if (130 * rate * arg1 / 100) + tokenAllocated <= stor28:
                    emit CurrentPeriod(1);
                    require (130 * rate * arg1 / 100) + tokenAllocated >= tokenAllocated
                    if (130 * rate * arg1 / 100) + tokenAllocated > balanceOf[stor6]:
                        emit TokenLimitReached(tokenAllocated, 130 * rate * arg1 / 100, msg.sender);
                    else:
                        require arg1 + weiRaised >= weiRaised
                        if arg1 + weiRaised <= hardWeiCap:
                            if arg1 >= weiMinSale:
                                return (130 * rate * arg1 / 100)
                        else:
                            emit HardCapReached()
                else:
                    if arg1:
