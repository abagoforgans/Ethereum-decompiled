...
function _fallback() payable {
    Mask(152, 0, stor1.field_0) = 0x47a8033cc6d6ca2ed5044674fd421f44884de8
    uint8(stor1.field_152) = 0
    return code.data[55 len 3407]
}
...
function register() payable {
    require not addressPresent[address(msg.sender)]
    sub_625d0be9[msg.sender][block.number].field_0 = sha3(msg.sender, block.number)
    sub_625d0be9[msg.sender][block.number].field_256 = 0
    idx = 0
    while sub_625d0be9[msg.sender][block.number].field_256 > idx:
        sub_625d0be9[msg.sender][block.number][idx + 1].field_0 = 0
        idx = idx + 1
        continue 
    addressPresent[address(msg.sender)] = 1
    addressToPersonId[address(msg.sender)] = sha3(msg.sender, block.number)
    return sha3(msg.sender, block.number)
}
...
function refreshScore(uint8 arg1, bytes32 arg2) payable {
    ...
    while sub_625d0be9[arg2][2][arg1 << 248][1].length + 196 > idx + 32:
        ...
        s = s + 1
        continue 
    call address(ext_call.return_data[0]).0x7f187d2d with:
         gas gas_remaining - 25050 wei
