...
function sub_415ae829(?) payable {
    ...
    while idx < 10:
        ...
        if not bool(left.length):
            ...
            mem[0] = 40
            stor[code.data[8280 len 32] + (0.03125 / idx)] = uint8(stor42[uint8(idx)].field_0) * 256^(-(idx % 32) + 31) or !(255 * 256^(-(idx % 32) + 31)) and stor[code.data[8280 len 32] + (0.03125 / idx)]
        idx = idx + 1
        continue 
    idx = 0
    while idx < 32:
