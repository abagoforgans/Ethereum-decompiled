...
function sub_f8aafec5(?) {
    idx = 0
    while idx < 30:
        require idx < agents.length
        mem[0] = 0
        mem[96] = 0x2296459e00000000000000000000000000000000000000000000000000000000
        require ext_code.size(agents[idx])
        call agents[idx].0x2296459e with:
             gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        idx = idx + 1
