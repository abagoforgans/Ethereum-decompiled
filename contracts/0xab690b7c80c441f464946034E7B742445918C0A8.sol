...
function checkDividends(address arg1) {
    if balanceOf[address(arg1)].field_0:
        if (totalFeesReceived * balanceOf[address(arg1)].field_256) - (balanceOf[address(arg1)].field_768 * balanceOf[address(arg1)].field_256) + balanceOf[address(arg1)].field_512 / 576200000 > payoutThreshold:
...
function withdrawDividends() {
    ...
    if not balanceOf[address(msg.sender)].field_512:
        emit StatEvent(Array(len=18, data='No accrued points!'));
        return 0
    if balanceOf[address(msg.sender)].field_512 / 576200000 <= payoutThreshold:
        emit StatEvent(Array(len=11, data='low Balance'));
    else:
        if developersAddress == msg.sender:
            if block.timestamp < vestTime:
                emit StatEvent(Array(len=22, data='Tokens not yet vested.'));
                return 0
        if balanceOf[address(msg.sender)].field_512 / 576200000 > eth.balance(this.address):
            emit StatEvent(Array(len=14, data='Balance Error!'));
        else:
            ...
            if not ext_call.return_data[0]:
                ...
                if ext_call.return_data[0] < ext_call.return_data[0]:
                    ...
                    if ext_call.return_data[0]:
                        emit StatEvent(Array(len=49, data='Transfer Error! old RecyleBin mu', 'st transfer back!'));
                    else:
