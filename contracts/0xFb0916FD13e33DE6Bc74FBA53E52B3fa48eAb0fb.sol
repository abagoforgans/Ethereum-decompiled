...
function _fallback() payable {
    ...
    stor1 = code.data[7381 len 32]
    stor2 = code.data[7413 len 32]
    stor3 = code.data[7445 len 32]
    emit ChangeOfRules(code.data[7381 len 32], code.data[7413 len 32], code.data[7445 len 32]);
    if address(code.data[7477 len 32]):
        stor0 = address(code.data[7477 len 32])
    require stor0 == msg.sender
    if stor6[0]:
    ...
    return code.data[1159 len 6222]
}
...
function removeMember(address arg1) {
    ...
    while idx < members.length - 1:
        ...
        if 31 >= stor[(3 * idx) + ('name', 'members', 7) + 4].length:
            ...
            uint256(members[idx].field_256) = Mask(255, 1, uint256(members[idx].field_1024) and (256 * not bool(members[idx].field_1024)) - 1) + 1
            if not Mask(255, 1, uint256(members[idx].field_1024) and (256 * not bool(members[idx].field_1024)) - 1):
                s = sha3((3 * idx) + sha3(7) + 1)
                while sha3((3 * idx) + sha3(7) + 1) + (stor[(3 * idx) + ('name', 'members', 7) + 1].length + 31 / 32) > s:
                    uint256(stor[s]) = 0
                    s = s + 1
