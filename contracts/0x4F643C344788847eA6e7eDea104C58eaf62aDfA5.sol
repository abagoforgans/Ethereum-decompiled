...
function _fallback() payable {
    ...
    mem[96 len -10214] = code.data[11120 len -10214]
    mem[64] = -10118
    ...
    return code.data[906 len 10214]
}
...
function buy() payable {
    ...
    require block.timestamp - startTime <= block.timestamp
    uint256(userBuys[(block.timestamp - stor9 / 600) + 1][address(msg.sender)]) += msg.value
    dailyTotals[(block.timestamp - stor9 / 600) + 1].field_0 += msg.value
    if not stor17[address(msg.sender)]:
        stor17[address(msg.sender)] = block.timestamp + (60 * stor14)
    call stor13 with:
       value msg.value wei
         gas 2300 * is_zero(value) wei
    require ext_call.success
    emit LogBuy((block.timestamp - startTime / 600) + 1, msg.sender, msg.value);
}
...
function claim(uint256 arg1) {
    require block.timestamp - startTime <= block.timestamp
    require (block.timestamp - startTime / 600) + 1 > arg1
...
function claimAll() payable {
    ...
    require block.timestamp - startTime <= block.timestamp
    idx = var37002
    while idx < (block.timestamp - startTime / 600) + 1:
        require block.timestamp - startTime <= block.timestamp
        require (block.timestamp - startTime / 600) + 1 > idx
        mem[0] = msg.sender
        mem[32] = sha3(idx, 18)
