...
function getTokens() payable {
    ...
    if msg.value >= 10^16:
        if value:
            require value
            require 2700 * value / value == 2700
        if 2700 * value:
            require 2700 * value
            require 2700 * value * msg.value / 2700 * value == msg.value
        ...
        if 2700 * value * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (2700 * value * msg.value / 10^18) >= totalDistributed
            totalDistributed += 2700 * value * msg.value / 10^18
            require 2700 * value * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 2700 * value * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (2700 * value * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 2700 * value * msg.value / 10^18
            emit Distr((2700 * value * msg.value / 10^18), msg.sender);
            emit Transfer((2700 * value * msg.value / 10^18), 0, msg.sender);
        else:
