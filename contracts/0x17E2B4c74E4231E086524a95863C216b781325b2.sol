...
const name = 'NetheriumToken'

const decimals = 18

const symbol = 'NTT'

const MIN_CONTRIBUTION = 10^15
...
function TouristToken() {
    stor1 = msg.sender
    require not distributionFinished
    require 2 * totalDistributed >= totalDistributed
    bool(stor5.field_0) = 0
    uint255(stor5.field_1) = uint255(stor5.field_0)
    require balanceOf[stor1] + totalDistributed >= balanceOf[stor1]
    balanceOf[stor1] += totalDistributed
    emit Distr(totalDistributed, stor1);
    emit Transfer(totalDistributed, 0, stor1);
}
