...
mapping of struct sub_bfd0f2ad;
uint8 stor6;
...
function participantBalance(address arg1) payable {
    require calldata.size - 4 >= 32
    return participantBalance[arg1]
}

function sub_bfd0f2ad(?) payable {
    require calldata.size - 4 >= 32
    return sub_bfd0f2ad[arg1].field_0, sub_bfd0f2ad[arg1].field_256
}
...
function dissolve() payable {
    ...
    while idx < participants.length:
        ...
        mem[32] = 4
        if participantBalance[stor1[idx]] > 0:
            require idx < participants.length
            mem[32] = 4
            participantBalance[stor1[idx]] = 0
            mem[0] = 1
            mem[100] = this.address
            mem[132] = participants[idx]
            mem[164] = participantBalance[stor1[idx]]
            require ext_code.size(stor0)
            call stor0.authorizedTransfer(address arg1, address arg2, uint256 arg3) with:
                 gas gas_remaining wei
                args address(this.address), participants[idx], participantBalance[stor1[idx]]
...
function contribute(address arg1, uint256 arg2) payable {
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    participantBalance[address(arg1)] += arg2
}
...
function sub_14d37274(?) payable {
    ...
    while idx < participants.length:
        ...
        mem[32] = 4
        idx = idx + 1
        s = s + participantBalance[stor1[idx]]
        continue 
