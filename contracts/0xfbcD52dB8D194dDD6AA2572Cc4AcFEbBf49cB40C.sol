...
function _fallback() payable {
    ...
    mem[160 len -3949] = code.data[4566 len -3949]
    stor0 = msg.sender
...
function buy() payable {
    require buyPrice
    require balanceOf[address(this.address)] >= msg.value / 10^15 / buyPrice
    balanceOf[address(msg.sender)] += msg.value / 10^15 / buyPrice
    balanceOf[this.address] -= msg.value / 10^15 / buyPrice
    emit Transfer((msg.value / 10^15 / buyPrice), this.address, msg.sender);
}
