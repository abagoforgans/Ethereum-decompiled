...
function _fallback() payable {
    ...
    require 23000 * msg.value / 10^18 / msg.value / 10^18 == 23000
    require 23000 * msg.value / 10^18 > 0
    require 23000 * msg.value / 10^18 <= totalAirDropToken
    totalAirDropToken += -23000 * msg.value / 10^18
    if not 23000 * msg.value / 10^18:
        require ext_code.size(tokenRewardContractAddress)
        call tokenRewardContractAddress.0xa9059cbb with:
             gas gas_remaining wei
            args msg.sender, 0
    else:
        require 23000 * 10^18 * msg.value / 10^18 / 23000 * msg.value / 10^18 == 10^18
        require ext_code.size(tokenRewardContractAddress)
        call tokenRewardContractAddress.0xa9059cbb with:
             gas gas_remaining wei
            args msg.sender, 23000 * 10^18 * msg.value / 10^18
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    call collectorAddress with:
    ...
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    emit FundTransfer(msg.sender, 23000 * msg.value / 10^18, 1);
}
