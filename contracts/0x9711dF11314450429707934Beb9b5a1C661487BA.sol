...
function _fallback() payable {
    ...
    require code.data[7721 len 20]
    require code.data[7753 len 20]
    stor3 = code.data[7721 len 20]
    stor2 = code.data[7753 len 20]
    return code.data[518 len 7191]
}
...
const isIcoOver = block.timestamp >= 418701 * 3600

const decimals = 18

const icoDeadline = (418701 * 3600)
...
const TOKEN_SUPPLY_LIMIT = 600000 * 10^18
...
function returnFundsFor(address arg1) {
    require block.timestamp >= 418701 * 3600
...
function buyTokens(address arg1) payable {
    ...
    require block.timestamp < 418701 * 3600
    require msg.value
    require totalSupply + (320 * msg.value) >= totalSupply
    require totalSupply + (320 * msg.value) <= 600000 * 10^18
...
function withdrawEther() {
    if totalSupply < 213440 * 10^18:
        if block.timestamp >= 418701 * 3600:
            if nextInListToReturn + 50 < stor8.length:
                s = 0
                ...
                while idx < nextInListToReturn + 50:
                    ...
                    mem[32] = 6
                    if ethBalances[address(stor8[idx])] > 0:
                        call address(stor8[idx]) with:
                           value ethBalances[address(stor8[idx])] wei
                             gas 2300 * is_zero(value) wei
                        require ext_call.success
                        mem[0] = address(stor8[idx])
                        mem[32] = 6
                        ethBalances[address(stor8[idx])] = 0
                    s = address(stor8[idx])
                    idx = idx + 1
                    continue 
                ...
                while idx < stor8.length:
                    ...
                    idx = idx + 1
                    continue 
                nextInListToReturn = stor8.length
    else:
        if msg.sender == tokenManagerAddress:
