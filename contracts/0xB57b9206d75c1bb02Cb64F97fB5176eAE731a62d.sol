...
function _fallback() payable {
    ...
    return code.data[1452 len 9956]
}
...
function getSlots() {
    if slots.length:
        ...
        while (32 * slots.length) + 128 > idx:
            ...
            s = s + 1
            continue 
    return Array(len=slots.length, data=mem[160 len 32 * slots.length])
}
...
function getPlayers() {
    if addresses.length:
        ...
        while (32 * addresses.length) + 128 > idx:
            ...
            s = s + 1
            continue 
    return Array(len=addresses.length, data=mem[160 len 32 * addresses.length])
}
