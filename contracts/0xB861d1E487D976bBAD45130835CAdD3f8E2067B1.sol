...
function _fallback() payable {
    ...
    while uint8(idx) < uint8(stor0.field_160):
        ...
        while uint8(s) < uint8(stor0.field_160):
            require s < 40
            require idx < 40
            stor[(0.03125 / idx) + (2 * s) + 1] = (uint8(stor0.field_160) * idx * 256^(idx % 32)) + (s * 256^(idx % 32)) or !(255 * 256^(idx % 32)) and stor[(0.03125 / idx) + (2 * s) + 1]
            s = s + 1
            continue 
        idx = idx + 1
        s = None
        continue 
    return code.data[280 len 350]
}
