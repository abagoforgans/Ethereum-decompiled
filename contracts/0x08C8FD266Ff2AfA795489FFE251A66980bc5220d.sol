...
function _fallback() payable {
    ...
    return code.data[136 len 14575]
}
...
function sub_60f1deb5(?) {
    bool(name.length) = 0
    uint255(name.length.field_1) = 10
    Mask(248, 0, name.length.field_8) = 'LIKER CASH' / 256
    idx = 0
    ...
    uint255(symbol.length.field_1) = 3
    Mask(248, 0, symbol.length.field_8) = 'LKC' / 256
    idx = 0
    while symbol.length + 31 / 32 > idx:
        ...
        continue 
    decimals = 18
    totalSupply = 300 * 10^6 * 10^decimals
    balanceOf[address(stor0.field_0)].field_0 = totalSupply
