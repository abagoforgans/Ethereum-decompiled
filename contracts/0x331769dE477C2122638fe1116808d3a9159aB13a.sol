...
function sub_430de4c6(?) {
    ...
    monthlyAmount = 0
    seasonalAmount = 0
    emit 0x41b5ae92: seasonalAmount + weeklyAmount + dailyAmount + monthlyAmount, arg1
    call arg1 with:
       value seasonalAmount + weeklyAmount + dailyAmount + monthlyAmount wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
}
...
function setOldVersion(address arg1) {
    ...
    while idx <= numberEnd:
        ...
        mem[0] = idx
        mem[32] = 0
        tickets[idx] = address(ext_call.return_data[0])
        idx = idx + 1
        continue 
    require ext_code.size(stor10)
