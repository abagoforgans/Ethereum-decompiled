...
function sub_13eb44f1(?) payable {
    ...
    mem[352 len 256] = code.data[1762 len 256]
    require ext_code.size(stor1)
    ...
    while uint8(idx) < uint8(ext_call.return_data[0]):
        mem[676] = ext_call.return_data[0] - 1
        mem[708] = uint8(idx)
        require ext_code.size(stor1)
        staticcall stor1.'#[[' with:
                gas gas_remaining wei
               args address(arg1), arg2, ext_call.return_data[0] - 1, uint8(idx)
        mem[608 len 96] = ext_call.return_data[0 len 96]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 96
        if ext_call.return_data[32]:
            require uint8(idx + 1) < 8
            mem[(32 * uint8(idx + 1)) + 352] = address(ext_call.return_data[0])
            s = 0
            while uint8(s) < uint8(ext_call.return_data[0]):
                mem[676] = ext_call.return_data[64]
                mem[708] = uint8(s)
                require ext_code.size(stor1)
                staticcall stor1.'#[[' with:
                        gas gas_remaining wei
                ...
                if not ext_call.return_data[32]:
                    s = s + 1
                    continue 
                require uint8(s + (2 * idx) + 3) < 8
                mem[(32 * uint8(s + (2 * idx) + 3)) + 352] = address(ext_call.return_data[0])
                idx = idx + 1
                continue 
        idx = idx + 1
        continue 
