...
function transferDevTokens(address arg1) {
    if owner != msg.sender:
        return 0
    require ext_code.size(gxcAddress)
    call gxcAddress.0x70a08231 with:
         gas gas_remaining - 710 wei
        args this.address
    require ext_call.success
    require ext_code.size(gxcAddress)
    call gxcAddress.0xa9059cbb with:
         gas gas_remaining - 710 wei
...
function refund() {
    require gXCSentToETH < minCap
    require backers[address(msg.sender)].field_256
    require ext_code.size(gxcAddress)
    call gxcAddress.0x9dc29fac with:
         gas gas_remaining - 710 wei
...
function finalize() {
    if owner == msg.sender:
        ...
        if block.number < endBlock:
            require gXCSentToETH >= maxCap - 100
        ...
        if gXCSentToETH <= minCap:
            ...
            require ext_call.return_data[0]
            require ext_code.size(gxcAddress)
            call gxcAddress.0xa69df4b5 with:
                 gas gas_remaining - 710 wei
            require ext_call.success
        stor12 = 1
}
...
function _fallback() payable {
    ...
    require (msg.value * stor15 / tokenPriceWei) + gXCSentToETH >= gXCSentToETH
    require (msg.value * stor15 / tokenPriceWei) + gXCSentToETH >= msg.value * stor15 / tokenPriceWei
    require (msg.value * stor15 / tokenPriceWei) + gXCSentToETH <= maxCap
    ...
    require (msg.value * stor15 / tokenPriceWei) + gXCSentToETH >= gXCSentToETH
    require (msg.value * stor15 / tokenPriceWei) + gXCSentToETH >= msg.value * stor15 / tokenPriceWei
    gXCSentToETH += msg.value * stor15 / tokenPriceWei
