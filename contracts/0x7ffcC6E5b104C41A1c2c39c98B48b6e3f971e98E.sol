...
function sub_bf4b45de(?) payable {
    ...
    call sub_43af9232Address.'Wvlq' with:
         gas gas_remaining wei
        args msg.sender, msg.value
    mem[224 len 352] = ext_call.return_data[0 len 352]
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    ...
    idx = 224
    while 384 > idx:
        stor38[s] = mem[idx + 12 len 20]
        s = s + 1
        idx = idx + 32
        continue 
    idx = 5
    while stor38.length > idx:
        stor38[idx] = 0
        idx = idx + 1
        continue 
    idx = 0
    while idx < ext_call.return_data[320]:
        require idx < stor39.length
        require idx < stor38.length
        mem[0] = stor38[idx]
        mem[32] = 12
        sub_08dd9989[stor38[idx]] += stor39[idx]
        idx = idx + 1
        continue 
}
...
function sub_4ecc9779(?) payable {
    ...
    call tokenAddress.0xd5078158 with:
         gas gas_remaining wei
        args msg.sender, sub_97905fc0Address, ext_call.return_data[0] * 10^decimals * sub_b51b9aaf / 10000 / price
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
