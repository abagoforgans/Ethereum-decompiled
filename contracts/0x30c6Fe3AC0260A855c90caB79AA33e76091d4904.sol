...
uint256 currentRate;
uint8 isMiningOpen;
uint8 miningActive; offset 8
uint8 swapOpen; offset 32
...
function miningTokens(uint256 arg1) {
    ...
    return address(miningTokens[arg1])
}
...
function sub_bc77edc2(?) {
    ...
    require address(miningTokens[arg3]) == arg1
    require arg2 < 10000
...
function addMiningToken(address arg1, uint256 arg2) {
    ...
    while uint16(idx) < miningTokens.length:
        mem[0] = 11
        if address(miningTokens[uint16(idx)]) != arg1:
            idx = idx + 1
            continue 
        emit 0x1e09227a: address(arg1), arg2
    miningTokens.length++
    address(miningTokens[miningTokens.length]) = arg1
    emit 0x1e09227a: address(arg1), arg2
}

function sub_3c4cd4b3(?) {
    require msg.sender == sub_e7331696Address
    require arg2 < miningTokens.length
    require address(miningTokens[arg2]) == arg1
    exchangeRatios[address(arg1)] = 0
    require miningTokens.length - 1 < miningTokens.length
    require arg2 < miningTokens.length
    address(miningTokens[arg2]) = address(miningTokens[miningTokens.length])
    require miningTokens.length - 1 < miningTokens.length
    address(miningTokens[miningTokens.length]) = 0
    miningTokens.length--
    if miningTokens.length > miningTokens.length - 1:
        idx = miningTokens.length - 1
        while miningTokens.length > idx:
            uint256(miningTokens[idx]) = 0
            idx = idx + 1
            continue 
}
...
function restart() {
    ...
    while uint16(idx) < miningTokens.length:
        mem[0] = 11
        require ext_code.size(address(miningTokens[uint16(idx)]))
        call address(miningTokens[uint16(idx)]).0x70a08231 with:
        ...
        require ext_code.size(address(miningTokens[uint16(idx)]))
        mem[96] = Mask(32, 224, sha3('transfer(address,uint256)'))
        mem[100] = sub_382d4fc5Address
        mem[132] = ext_call.return_data[0] / 5
        call address(miningTokens[uint16(idx)]) with:
        ...
        s = address(miningTokens[uint16(idx)])
        idx = idx + 1
        continue 
...
function swap(uint256 arg1) {
    ...
    if arg1 <= cycleMintSupply:
        ...
        idx = 0
        while uint16(idx) < miningTokens.length:
            mem[0] = address(miningTokens[uint16(idx)])
            mem[32] = 20
            if not arg1:
                ...
                require arg1
                require arg1 * swapRates[address(stor11[uint16(idx)])] / arg1 == swapRates[address(stor11[uint16(idx)])]
                require decimalOffset
                require ext_code.size(address(miningTokens[uint16(idx)]))
                mem[96] = Mask(32, 224, sha3('transfer(address,uint256)'))
                mem[100] = msg.sender
                mem[132] = arg1 * swapRates[address(stor11[uint16(idx)])] / decimalOffset
                call address(miningTokens[uint16(idx)]) with:
                   funct Mask(32, 224, sha3('transfer(address,uint256)')) >> 224
                     gas gas_remaining wei
                    args msg.sender, arg1 * swapRates[address(stor11[uint16(idx)])] / decimalOffset
            require ext_call.success
            if return_data.size:
                require return_data.size == 32
                mem[0] = ext_call.return_data[0]
        ...
        while uint16(idx) < miningTokens.length:
            ...
            if not cycleMintSupply:
                ...
                require cycleMintSupply * swapRates[address(stor11[uint16(idx)])] / cycleMintSupply == swapRates[address(stor11[uint16(idx)])]
                require decimalOffset
                require ext_code.size(address(miningTokens[uint16(idx)]))
                mem[96] = Mask(32, 224, sha3('transfer(address,uint256)'))
                mem[100] = msg.sender
                mem[132] = cycleMintSupply * swapRates[address(stor11[uint16(idx)])] / decimalOffset
                call address(miningTokens[uint16(idx)]) with:
                   funct Mask(32, 224, sha3('transfer(address,uint256)')) >> 224
                     gas gas_remaining wei
                    args msg.sender, cycleMintSupply * swapRates[address(stor11[uint16(idx)])] / decimalOffset
            require ext_call.success
            if return_data.size:
                require return_data.size == 32
                mem[0] = ext_call.return_data[0]
        ...
        while idx > 0:
            if idx < miningTokenLeftInCurrent:
                ...
                while uint16(idx) < miningTokens.length:
                    ...
                    mem[0] = address(miningTokens[uint16(idx)])
                    mem[32] = 20
                    swapRates[address(stor11[uint16(idx)])] = ext_call.return_data[0] / 21000000 * 10^18 / decimalOffset
                    s = ext_call.return_data[0]
                    s = address(miningTokens[uint16(idx)])
                    idx = idx + 1
                    continue 
                ...
                if idx - miningTokenLeftInCurrent <= 0:
                    ...
                    if not idx - miningTokenLeftInCurrent:
                        ...
                        while uint16(s) < miningTokens.length:
                            mem[100] = this.address
                            require ext_code.size(address(miningTokens[uint16(s)]))
                            call address(miningTokens[uint16(s)]).0x70a08231 with:
                                 gas gas_remaining wei
                                args this.address
                            mem[96] = ext_call.return_data[0]
                            if not ext_call.success:
                            ...
                            require decimalOffset
                            require 21000000 * 10^18 / decimalOffset
                            mem[0] = address(miningTokens[uint16(s)])
                            mem[32] = 20
                            swapRates[address(stor11[uint16(s)])] = ext_call.return_data[0] / 21000000 * 10^18 / decimalOffset
                            t = ext_call.return_data[0]
                            t = address(miningTokens[uint16(s)])
                            s = s + 1
                            continue 
                        emit SwapStarted(swapEndTime);
                        cycleMintSupply = 21000000 * 10^18
