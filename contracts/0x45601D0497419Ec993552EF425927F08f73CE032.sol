...
function _fallback() payable {
    stor5 = 100000000000000 * 10^18
    mem[96 len -1853] = code.data[2299 len -1853]
    mem[64] = -1757
...
function proofOfWork(uint256 arg1) payable {
    require Mask(64, 192, sha3(arg1, currentChallenge)) >= uint64(stor5)
    uint256(difficulty) = (60 * uint256(difficulty) / block.timestamp - timeOfLastProof) + 1
    timeOfLastProof = block.timestamp
    currentChallenge = sha3(arg1, currentChallenge, block.hash(block.number))
    balanceOf[address(msg.sender)] += block.timestamp - timeOfLastProof / 6
}
