...
uint256 sub_01e4bb29;
uint256 stor9;
array of uint256 name;
array of uint256 symbol;
uint8 decimals;

function sub_01e4bb29(?) {
    return sub_01e4bb29
}
...
function release() {
    ...
    if not unLockTime:
        require startTime >= startTime
        require block.timestamp >= startTime
    else:
        require unLockTime
        require unLockTime * sub_01e4bb29 / unLockTime == sub_01e4bb29
        require startTime + (unLockTime * sub_01e4bb29) >= startTime
        require block.timestamp >= startTime + (unLockTime * sub_01e4bb29)
    require balanceOf[address(this.address)] > 0
    require ext_code.size(tokenAddress)
    call tokenAddress.0xa9059cbb with:
         gas gas_remaining wei
        args _beneficiaryAddress, stor9
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    require ext_call.return_data[0]
    require sub_01e4bb29 + 1 >= sub_01e4bb29
    sub_01e4bb29++
}
