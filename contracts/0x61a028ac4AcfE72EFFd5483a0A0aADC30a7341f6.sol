...
address cooAddress;
uint256 sub_ab8f933a;
array of struct product;
...
function getProduct(uint256 arg1) {
    require calldata.size - 4 >= 32
    require arg1 < product.length
    return product[arg1].field_0, product[arg1].field_256
}
...
function sub_e078d8b1(?) payable {
    require calldata.size - 4 >= 64
    require msg.value < sub_ab8f933a
    revert
}
...
function tokensOfOwner(address arg1) {
    ...
    if balanceOf[address(arg1)]:
        mem[128 len 32 * balanceOf[address(arg1)]] = code.data[15184 len 32 * balanceOf[address(arg1)]]
    idx = 1
    s = 0
    while idx <= product.length - 1:
        mem[0] = idx
        mem[32] = 5
