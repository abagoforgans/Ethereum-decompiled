...
function sub_4ad2c928(?) {
    if arg1 <= block.timestamp:
        emit code.data[8874 len 32]: 0, 64, 43, 'The start date for this journey ', 'is invalid.', msg.sender
    else:
        if not arg3:
            emit code.data[8874 len 32]: 0, 64, 22, 'Insurance amount is 0.', msg.sender
        else:
            if arg2 < arg1:
                emit code.data[8874 len 32]: 0, 64, 55, 'The end date for this journey is', ' before the start date.', msg.sender
            else:
                if arg6 > 4:
                    emit code.data[8874 len 32]: 0, 64, 26, 'Insurance Type is invalid.', msg.sender
                else:
                    if arg5 > 3:
                        emit code.data[8874 len 32]: 0, 64, 23, 'Travel Type is invalid.', msg.sender
                    else:
                        if arg11 > 1:
                            emit code.data[8874 len 32]: 0, 64, 29, 'Traveller parameters invalid.', msg.sender
                        else:
                            if arg10 > 120:
                                emit code.data[8874 len 32]: 0, 64, 29, 'Traveller parameters invalid.', msg.sender
                            else:
                                if arg9 > 4:
                                    ...
                                    if arg12 > 2:
                                        emit code.data[8874 len 32]: 0, 64, 25, 'Invalid auction duration.', msg.sender
                                    else:
                                        if not arg8:
                                            emit code.data[8874 len 32]: 0, 64, 29, 'Maximum price cannot be zero.', msg.sender
                                        else:
                                            if arg7 > 4:
                                                emit code.data[8874 len 32]: 0, 64, 28, 'Invalid destination country.', msg.sender
                                            else:
                                                emit code.data[8874 len 32]: 1, 'Success', msg.sender
                                                numberOfRequests++
