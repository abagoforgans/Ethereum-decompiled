...
function sub_d8b4b6ab(?) {
    require owner == msg.sender
    require arg2 + altDeposits >= altDeposits
    altDeposits += arg2
    emit 0x1f29398a: arg3, arg1, arg2
}
...
function finishMinting() {
    require owner == msg.sender
    require altDeposits + eth.balance(multisigVaultAddress) >= eth.balance(multisigVaultAddress)
    if altDeposits + eth.balance(multisigVaultAddress):
        require altDeposits + eth.balance(multisigVaultAddress)
        require (350 * altDeposits) + (350 * eth.balance(multisigVaultAddress)) / altDeposits + eth.balance(multisigVaultAddress) == 350
    if (350 * altDeposits) + (350 * eth.balance(multisigVaultAddress)):
        require (350 * altDeposits) + (350 * eth.balance(multisigVaultAddress))
        require (42000 * altDeposits) + (42000 * eth.balance(multisigVaultAddress)) / (350 * altDeposits) + (350 * eth.balance(multisigVaultAddress)) == 120
    require ext_code.size(tokenAddress)
    call tokenAddress.0x18160ddd with:
         gas gas_remaining - 710 wei
    require ext_call.success
    require ext_call.return_data[0] <= (42000 * altDeposits) + (42000 * eth.balance(multisigVaultAddress)) / 51
    require ext_code.size(tokenAddress)
    call tokenAddress.0x40c10f19 with:
         gas gas_remaining - 710 wei
        args multisigVaultAddress, ((42000 * altDeposits) + (42000 * eth.balance(multisigVaultAddress)) / 51) - ext_call.return_data[0]
    require ext_call.success
    require ext_code.size(tokenAddress)
