...
function _fallback() payable {
    ...
    require 1510083000 >= block.timestamp
...
const PHASE1_START_TIME = 1510083000
...
function setTokensPerKEther(uint256 arg1) {
    ...
    if not pausedTime:
        require block.timestamp < endTime
    require block.timestamp < 1510083000
    require arg1 > 0
...
function unpause() {
    ...
    if block.timestamp > 1510083000:
        if pausedTime >= 1510083000:
            ...
            require 1510083000 <= block.timestamp
            require block.timestamp + endTime - 1510083000 >= endTime
            endTime = block.timestamp + endTime - 1510083000
    pausedTime = 0
    require adminAddress
    require adminAddress == msg.sender
    require uint8(stor3.field_160)
