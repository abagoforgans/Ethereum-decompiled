...
uint8 conversionsEnabled; offset 96
uint32 stor11;
...
function conversionsEnabled() {
    return bool(conversionsEnabled)
}
...
function buy(address arg1, uint256 arg2, uint256 arg3) {
    ...
    require ext_code.size(extensionsAddress)
    call extensionsAddress.gasPriceLimit() with:
         gas gas_remaining - 710 wei
    require ext_call.success
    require ext_code.size(address(ext_call.return_data[0]))
    call address(ext_call.return_data[0]).gasPrice() with:
         gas gas_remaining - 710 wei
    require ext_call.success
