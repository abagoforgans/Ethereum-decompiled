...
function participants(uint256 arg1) payable {
    ...
    return address(stor0[arg1].field_0), stor[code.data[1106 len 32] + (2 * arg1)]
}
...
function enter() payable {
    ...
    if msg.value <= 100 * 10^18:
        ...
        if not stor0.length > stor0.length + 1:
            ...
            stor[code.data[1106 len 32] + (2 * stor0.length)] = msg.value
            if not stor0.length:
                collectedFees += msg.value
            else:
                collectedFees += msg.value / 10
                balance = balance + msg.value - (msg.value / 10)
            if balance <= 180 * stor[code.data[1106 len 32] + (2 * stor1)] / 100:
        ...
        if not stor0.length > stor0.length + 1:
            require stor0.length < stor0.length
            stor0[stor0.length].field_0 = msg.sender or Mask(96, 160, stor0[stor0.length].field_0)
            stor[code.data[1106 len 32] + (2 * stor0.length)] = 100 * 10^18
            if not stor0.length:
