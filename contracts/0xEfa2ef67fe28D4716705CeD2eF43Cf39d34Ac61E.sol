...
function calcMaxWithdraw() {
    ...
    if withdrawnTokens < (block.timestamp * ext_call.return_data[0]) - (startDate * ext_call.return_data[0]) + (block.timestamp * withdrawnTokens) - (startDate * withdrawnTokens) / vestingPeriod:
        if startDate <= block.timestamp:
            return (((block.timestamp * ext_call.return_data[0]) - (startDate * ext_call.return_data[0]) + (block.timestamp * withdrawnTokens) - (startDate * withdrawnTokens) / vestingPeriod) - withdrawnTokens)
        else:
            return 0
    else:
        return 0
...
function withdraw(address arg1, uint256 arg2) {
    ...
    if withdrawnTokens >= (block.timestamp * ext_call.return_data[0]) - (startDate * ext_call.return_data[0]) + (block.timestamp * withdrawnTokens) - (startDate * withdrawnTokens) / vestingPeriod:
        ...
        if startDate <= block.timestamp:
            require arg2 <= ((block.timestamp * ext_call.return_data[0]) - (startDate * ext_call.return_data[0]) + (block.timestamp * withdrawnTokens) - (startDate * withdrawnTokens) / vestingPeriod) - withdrawnTokens
        else:
            require arg2 <= 0
