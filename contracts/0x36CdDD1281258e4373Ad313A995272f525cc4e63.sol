...
function _fallback() payable {
    ...
    stor25 = code.data[3943 len 20]
    stor0 = code.data[3975 len 20]
    stor1 = code.data[4007 len 20]
    stor2 = code.data[4039 len 20]
    stor3 = code.data[4071 len 20]
    stor22 = code.data[4091 len 32]
    stor23 = code.data[4091 len 32] + stor20
    return code.data[326 len 3605]
}
...
function init() {
    ...
    call tokenAddress.0x867904b4 with:
         gas gas_remaining - 710 wei
        args beneficiaryAddress, 49 * 10^13
    require ext_call.success
    if not ext_call.return_data[0]:
        stage = 0
    else:
            ...
            if ext_call.success:
                if not ext_call.return_data[0]:
                    ...
                    if ext_code.size(tokenAddress):
                        call tokenAddress.0x867904b4 with:
                             gas gas_remaining - 710 wei
                            args marketingAddress, 25 * 10^13
                        if ext_call.success:
                            if not ext_call.return_data[0]:
                                stage = 0
                            else:
