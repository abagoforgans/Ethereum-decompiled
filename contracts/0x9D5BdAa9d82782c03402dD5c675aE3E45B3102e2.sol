...
function persons(uint256 arg1) payable {
    ...
    return address(stor0[arg1].field_0), stor[code.data[1039 len 32] + (2 * arg1)]
}
...
function enter() payable {
    ...
    if msg.value <= 3 * 10^18:
        ...
        if not stor0.length > stor0.length + 1:
            ...
            stor[code.data[1039 len 32] + (2 * stor0.length)] = msg.value
            if not stor0.length:
                balance += msg.value
            else:
                collectedFees += msg.value / 3
                call address(owner) with:
                   value (msg.value / 3) + collectedFees wei
                     gas 0 wei
                collectedFees = 0
                balance = msg.value - (msg.value / 3) + balance
            if balance <= 133 * stor[code.data[1039 len 32] + (2 * stor1)] / 100:
        ...
        if not stor0.length > stor0.length + 1:
            require stor0.length < stor0.length
            stor0[stor0.length].field_0 = msg.sender or Mask(96, 160, stor0[stor0.length].field_0)
            stor[code.data[1039 len 32] + (2 * stor0.length)] = 3 * 10^18
            if not stor0.length:
                balance += 3 * 10^18
            else:
                collectedFees += 10^18
