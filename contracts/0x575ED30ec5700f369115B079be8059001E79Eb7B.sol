...
const getInstructions = Array(len=71, data='Allows an issuer to restrict the', ' total number of non-zero token ', 'holders' >> 256, Mask(56, -512, 'Allows an issuer to restrict the', ' total number of non-zero token ', 'holders') << 512)
...
function deploy(bytes arg1) {
    ...
    mem[96 len 4114] = code.data[6662 len 4114]
    mem[4210] = msg.sender
    mem[4242] = polyTokenAddress
    create contract with 0 wei
                    code: code.data[6662 len 4114], address(msg.sender), polyTokenAddress
    if not create.new_address:
        revert with ext_call.return_data[0 len return_data.size]
    require ext_code.size(address(create.new_address))
    call address(create.new_address).getInitFunction() with:
         gas gas_remaining wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    ...
    if arg1.length < 4:
        ...
        while idx < arg1.length:
            ...
            idx = idx + 1
            s = (Mask(32, 224, s) >> 224) + (mem[idx + 128 len 1] * 2^(8 * arg1.length + -idx - 1)) << 224
            continue 
        if 32 * Mask(32, 219, arg1.length) != Mask(32, 224, ext_call.return_data[0]):
            revert with 0, 'Provided data is not valid'
    else:
        idx = 0
        s = 0
        while idx < 4:
            ...
            continue 
        if Mask(32, 224, ext_call.return_data[0]) != 0:
            revert with 0, 'Provided data is not valid'
    call address(create.new_address) with:
         gas gas_remaining wei
        args arg1[all]
    if not ext_call.success:
        revert with 0, 'Unsuccessful call'
...
function changeSTVersionBounds(string arg1, uint8[] arg2) {
    ...
    if sha3(call.data[arg1 + 36 len floor32(arg1.length)], mem[arg1.length + floor32(arg1.length) + 170 len arg1.length % 32]) == sha3(Mask(80, 0, 'lowerBound', 0)):
        ...
        if stor9[arg1[all]].field_0 % 16777216:
            ...
            mem[arg1.length + (32 * arg2.length) + 362 len 96] = code.data[6662 len 96]
            idx = 0
            while uint8(idx) < 3:
        ...
        if not stor[sha3(mem[(2 * arg1.length) + 244 len (4 * arg1.length) + 32])].field_0 % 16777216:
            ...
            mem[(2 * arg1.length) + (32 * arg2.length) + 436 len 96] = code.data[6662 len 96]
            idx = 0
            while uint8(idx) < 3:
                require uint8(idx) < arg2.length
                require uint8(idx) < 3
