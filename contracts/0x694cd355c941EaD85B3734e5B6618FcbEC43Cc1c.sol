...
const _fallback = code.data[19 len 2684]
...
function calculateWinner(uint256 arg1, uint256 arg2) payable {
    if (10 * arg1) + arg2 + 10 <= block.number:
        ...
        while uint8(idx) < pot[arg1].field_0:
            ...
            if s + pot[arg1][4][pot[arg1][idx].field_0].field_0 > block.hash((10 * arg1) + arg2 + 10) % pot[arg1].field_512:
        ...
        if msg.value - (msg.value % 10^17) < msg.value:
            call msg.sender with:
               value msg.value % 10^17 wei
                 gas 0 wei
        if not pot[block.number / 10][4][address(msg.sender)].field_0:
            pot[block.number / 10].field_0++
            if not pot[block.number / 10].field_0 <= pot[block.number / 10].field_0 + 1:
                idx = pot[block.number / 10].field_0 + 1
                while pot[block.number / 10].field_0 > idx:
                    pot[block.number / 10][idx].field_0 = 0
                    idx = idx + 1
                    continue 
            require pot[block.number / 10].field_0 < pot[block.number / 10].field_0
            pot[block.number / 10][pot[block.number / 10].field_0].field_0 = msg.sender or Mask(96, 160, pot[block.number / 10][pot[block.number / 10].field_0].field_0)
        pot[block.number / 10][4][address(msg.sender)].field_0 += msg.value - (msg.value % 10^17) / 10^17
        pot[block.number / 10].field_512 += msg.value - (msg.value % 10^17) / 10^17
