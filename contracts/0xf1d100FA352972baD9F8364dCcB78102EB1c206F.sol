contract main {


// =======================  Init code  ======================


mapping of uint256 stor0;
array of uint256 stor2;
address stor3;

function _fallback() payable {
    bool(stor2.length) = 0
    stor2.length.field_1 = 0
    stor2.length.field_8 = mem[128 len 31]
    idx = 0
    while stor2.length + 31 / 32 > idx:
        stor2[idx].field_0 = 0
        idx = idx + 1
        continue 
    require not msg.value
    require code.data[3335 len 20]
    stor3 = code.data[3335 len 20]
    stor0[address(msg.sender)] = 27700000
    return code.data[331 len 2992]
}



// =====================  Runtime code  =====================


const name = 'Gidnist'

const totalSupply = 27700000

const decimals = 2

const version = 'v0.0.1'

const symbol = 'GDNT'


mapping of uint256 balances;
mapping of uint256 allowed;
array of uint256 migrationInfo;
address migrationInfoSetterAddress;

function migrationInfo() {
    return migrationInfo[0 len migrationInfo.length]
}

function balances(address arg1) {
    return balances[arg1]
}

function migrationInfoSetter() {
    return migrationInfoSetterAddress
}

function allowed(address arg1, address arg2) {
    return allowed[arg1][arg2]
}

function balanceOf(address arg1) {
    return balances[address(arg1)]
}

function allowance(address arg1, address arg2) {
    return allowed[address(arg1)][address(arg2)]
}

function _fallback() payable {
    revert
}

function changeMigrationInfoSetter(address arg1) {
    require migrationInfoSetterAddress == msg.sender
    migrationInfoSetterAddress = arg1
}

function approve(address arg1, uint256 arg2) {
    allowed[address(msg.sender)][address(arg1)] = arg2
    emit Approval(arg2, msg.sender, arg1);
    return 1
}

function setMigrationInfo(string arg1) {
    require migrationInfoSetterAddress == msg.sender
    migrationInfo[] = Array(len=arg1.length, data=arg1[all])
    emit MigrationInfoSet(Array(len=arg1.length, data=arg1[all]));
}

function transfer(address arg1, uint256 arg2) {
    if balances[address(msg.sender)] < arg2:
        return 0
    balances[address(msg.sender)] -= arg2
    balances[arg1] += arg2
    emit Transfer(arg2, msg.sender, arg1);
    return 1
}

function compareAndApprove(address arg1, uint256 arg2, uint256 arg3) {
    if arg2 != allowed[address(msg.sender)][address(arg1)]:
        return 0
    allowed[address(msg.sender)][address(arg1)] = arg3
    emit Approval(arg3, msg.sender, arg1);
    return 1
}

function transferFrom(address arg1, address arg2, uint256 arg3) {
    if balances[address(arg1)] < arg3:
        return 0
    if allowed[address(arg1)][address(msg.sender)] < arg3:
        return 0
    balances[address(arg1)] -= arg3
    allowed[address(arg1)][address(msg.sender)] -= arg3
    balances[address(arg2)] += arg3
    emit Transfer(arg3, arg1, arg2);
    return 1
}



}
