...
array of uint256 curves_list;
mapping of uint8 redeemed;
array of address redeemed_list;
...
function curves_list(uint256 arg1) {
    require arg1 < curves_list.length
    return curves_list[arg1]
}
...
function redeemed_list(uint256 arg1) {
    require arg1 < redeemed_list.length
    return address(redeemed_list[arg1])
}
...
function reset() {
    ...
    redeemed_list.length = 0
    idx = 0
    while redeemed_list.length > idx:
        uint256(redeemed_list[idx]) = 0
        idx = idx + 1
        continue 
    curves_list.length = 0
    idx = 0
    while curves_list.length > idx:
        curves_list[idx] = 0
        idx = idx + 1
        continue 
    status = 1
...
function toBytes(address arg1) {
    mem[128 len 640] = code.data[16043 len 640]
    idx = 0
    while idx < 20:
        require 2^(8 * -idx + 19)
        require idx < 20
...
function settle() {
    ...
    idx = 0
    s = 0
    while idx < curves_list.length:
        mem[0] = 11
        if curves_list[idx] == winner:
            idx = idx + 1
            s = s
            continue 
        require idx < curves_list.length
        require ext_code.size(address(stor5))
        call address(stor5).getDotsIssued(address arg1, bytes32 arg2) with:
             gas gas_remaining wei
            args address(this.address), curves_list[idx]
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        require idx < curves_list.length
...
function initializeCurve(bytes32 arg1, bytes32 arg2, int256[] arg3) {
    if curves[arg1]:
        revert with 0, 'Curve endpoint already exists or used in the past. Please choose new'
    require ext_code.size(coordAddress)
    call coordAddress.getContract(string arg1) with:
         gas gas_remaining wei
        args Array(len=8, data='REGISTRY')
    ...
    mem[(32 * arg3.length) + 416 len 640] = code.data[16043 len 640]
    idx = 0
    while idx < 20:
        require 2^(8 * -idx + 19)
        require idx < 20
