...
function _fallback() payable {
    ...
    stor23 = 0
    stor11 = 0
    return code.data[1013 len 7471]
}
...
array of uint256 stor71709625427275648070435571067103625102151317872053819652964380875187535702997;
...
function tickets(uint256 arg1) payable {
    ...
    return stor9E8A[arg1], stor8A35[arg1], uint8(stor568A[arg1].field_0), uint256(stor[(5 * arg1) + code.data[7439 len 32]])
}
...
function sub_ca34f1f2(?) payable {
    ...
    while idx < stor4.length:
        mem[0] = 4
        if uint8(stor568A[idx].field_0) != arg2:
            idx = idx + 1
            s = s
            continue 
    ...
    while idx < stor4.length:
        ...
        if not address(stor5[stor9E8A[idx]]):
            uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 80 * arg1 / s * stor9E8A[idx] / 100
        else:
            uint256(stor[code.data[7439 len 32] + (5 * idx)]) += arg1 / s * stor9E8A[idx]
...
function sub_79747295(?) payable {
    mem[96] = 0
    mem[128] = 0
    mem[160] = uint8(maxNumber + 1)
    mem[64] = (32 * uint8(maxNumber + 1)) + 192
    idx = 0
    s = 0
    t = 0
    while idx < 123456789:
        ...
        if 0 == uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber):
            ...
            continue 
        require uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber) < mem[160]
        if 1 == mem[(32 * uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)) + 223 len 1]:
            idx = idx + 1
            s = s
            t = sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber
            continue 
        require uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber) < mem[160]
        mem[(32 * uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)) + 192] = 1
        require uint8(s) < mem[96]
        mem[(32 * uint8(s)) + 128] = uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)
        if uint8(s + 1) != 6:
            idx = idx + 1
            s = s + 1
            t = sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber
        ...
        while uint8(idx) < 5:
            ...
            while uint8(t) < 6:
                ...
                if mem[(32 * uint8(idx)) + 159 len 1] <= mem[(32 * uint8(t)) + 159 len 1]:
                    ...
                    continue 
                require uint8(idx) < mem[96]
                _206 = mem[(32 * uint8(idx)) + 128]
                require uint8(t) < mem[96]
                require uint8(idx) < mem[96]
                mem[(32 * uint8(idx)) + 128] = mem[(32 * uint8(t)) + 159 len 1]
                require uint8(t) < mem[96]
                mem[(32 * uint8(t)) + 128] = uint8(_206)
                u = _206
                t = t + 1
                continue 
            s = u
    ...
    while uint8(t) < 5:
        ...
        while uint8(idx) < 6:
            require uint8(idx) < mem[96]
            require uint8(t) < mem[96]
            if mem[(32 * uint8(t)) + 159 len 1] <= mem[(32 * uint8(idx)) + 159 len 1]:
                u = u
                idx = idx + 1
                continue 
            require uint8(t) < mem[96]
            _173 = mem[(32 * uint8(t)) + 128]
            require uint8(idx) < mem[96]
            require uint8(t) < mem[96]
            mem[(32 * uint8(t)) + 128] = mem[(32 * uint8(idx)) + 159 len 1]
            require uint8(idx) < mem[96]
            mem[(32 * uint8(idx)) + 128] = uint8(_173)
            u = _173
            idx = idx + 1
            continue 
        s = u
...
function draw() payable {
    mem[96] = 0
    require block.timestamp >= stor1.length + stor22
    require bool(stor23) != 1
    stor23 = 1
    mem[128] = 0
    mem[160] = 0
    mem[192] = uint8(maxNumber + 1)
    mem[64] = (32 * uint8(maxNumber + 1)) + 224
    idx = 0
    s = 0
    t = 0
    while idx < 123456789:
        ...
        if 0 == uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber):
            ...
            t = sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber
            continue 
        require uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber) < mem[192]
        if 1 == mem[(32 * uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)) + 255 len 1]:
            idx = idx + 1
            s = s
            t = sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber
            continue 
        require uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber) < mem[192]
        mem[(32 * uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)) + 224] = 1
        require uint8(s) < mem[128]
        mem[(32 * uint8(s)) + 160] = uint8(sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber)
        if uint8(s + 1) != 6:
            idx = idx + 1
            s = s + 1
            t = sha3(block.difficulty, block.coinbase, block.timestamp, msg.sender, idx) % maxNumber
        ...
        while uint8(idx) < 5:
            ...
            t = idx + 1
            while uint8(t) < 6:
                require uint8(t) < mem[128]
                require uint8(idx) < mem[128]
                if mem[(32 * uint8(idx)) + 191 len 1] <= mem[(32 * uint8(t)) + 191 len 1]:
                    u = u
                    t = t + 1
                    continue 
                require uint8(idx) < mem[128]
                _517 = mem[(32 * uint8(idx)) + 160]
                require uint8(t) < mem[128]
                require uint8(idx) < mem[128]
                mem[(32 * uint8(idx)) + 160] = mem[(32 * uint8(t)) + 191 len 1]
                require uint8(t) < mem[128]
                mem[(32 * uint8(t)) + 160] = uint8(_517)
                u = _517
                t = t + 1
                continue 
            s = u
        ...
        while idx < stor4.length:
            ...
            while uint8(s) < 6:
                ...
                while uint8(t) < 6:
                    require idx < stor4.length
                    require uint8(t) < stor8A35[idx]
                    mem[0] = (5 * idx) - 0x75ca53043ea007e5c65182cbb028f60d7179ff4b55739a3949b401801c942e64
                    require uint8(s) < mem[128]
                    if mem[(32 * uint8(s)) + 191 len 1] != stor((Mask(3, 5, t) >> 5) + sha3((5 * idx) - 0x75ca53043ea007e5c65182cbb028f60d7179ff4b55739a3949b401801c942e64))[uint8(t)]:
                        t = t + 1
                        continue 
                    require idx < stor4.length
                    mem[0] = 4
                    uint256(stor568A[idx].field_0) = Mask(248, 0, stor568A[idx].field_8)
                    s = s + 1
                    continue 
                s = s + 1
                continue 
        ...
        if not mem[128]:
            ...
            while (32 * mem[128]) + 160 > idx:
                stor19 = mem[idx] * 256^s or !(255 * 256^s) and stor19
                s = s + -(s + 1 / 32) + (-1 * s * s + 1 / 32) + 1
                idx = idx + 32
                continue 
            idx = Mask(251, 0, (32 * mem[128]) + 31) >> 5
            s = 0x66de8ffda797e3de9c05e8fc57b3bf0ec28a930d40b0d285d93c06501cf6a090
            while idx:
                ...
                s = (idx + 1 / 32) + s
                continue 
            idx = (floor32(None + 3) >> 4) + (None * None + 3 / 32) + (-1 * Mask(251, 0, (32 * mem[128]) + 31) >> 5 * None + 3 / 32) + 0x66de8ffda797e3de9c05e8fc57b3bf0ec28a930d40b0d285d93c06501cf6a090
            while (stor19.length + 31 / 32) + 0x66de8ffda797e3de9c05e8fc57b3bf0ec28a930d40b0d285d93c06501cf6a090 > idx:
                uint8(stor[idx]) = 0
                idx = idx + 1
                continue 
        ...
        while idx < stor4.length:
            mem[0] = 4
            if uint8(stor568A[idx].field_0) != 6:
                idx = idx + 1
                s = s
                continue 
            require idx < stor4.length
            ...
            continue 
        if s <= 0:
            stor13 += 60 * stor12 / 100
            idx = 0
            s = 0
            while idx < stor4.length:
                mem[0] = 4
                if uint8(stor568A[idx].field_0) != 5:
                    idx = idx + 1
                    s = s
                    continue 
                require idx < stor4.length
                ...
                continue 
            if s <= 0:
                stor13 += 30 * stor12 / 100
                idx = 0
                s = 0
                while idx < stor4.length:
                    mem[0] = 4
                    if uint8(stor568A[idx].field_0) != 4:
                        idx = idx + 1
                        s = s
                        continue 
                    require idx < stor4.length
                    ...
                    continue 
                if s <= 0:
                    stor13 += 10 * stor12 / 100
                    s = 0
                    idx = 0
                    while idx < stor4.length:
                        mem[0] = 4
                        if uint256(stor[code.data[7439 len 32] + (5 * idx)]) <= 0:
                            s = s
                            idx = idx + 1
                            continue 
                        require idx < stor4.length
                        mem[0] = 4
                        uint256(stor[code.data[7439 len 32] + (5 * idx)]) = 0
                        if stor9E8A[idx] > 0:
                            require idx < stor4.length
                            mem[0] = 4
                            call stor9E8A[idx] with:
                               value stor9E8A[idx] wei
                                 gas 0 wei
                            if ext_call.success:
                                stor13 += stor9E8A[idx]
                        s = stor9E8A[idx]
                    ...
                    if stor17 > 0:
                        call address(stor0.field_0) with:
                           value 80 * stor17 / 100 wei
                             gas 0 wei
                        if not ext_call.success:
                            stor13 += 80 * stor17 / 100
                        if stor17 > 0:
                            call msg.sender with:
                               value 20 * stor17 / 100 wei
                                 gas 0 wei
                            if not ext_call.success:
                                stor13 += 20 * stor17 / 100
                else:
                    if 10 * stor12 / 100 / s < stor3.length:
                        stor13 += 10 * stor12 / 100
                    else:
                        t = 0
                        idx = 0
                        while idx < stor4.length:
                            ...
                            mem[0] = 4
                            if not address(stor5[stor9E8A[idx]]):
                                uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 80 * 10 * stor12 / 100 / s * stor9E8A[idx] / 100
                            else:
                                uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 10 * stor12 / 100 / s * stor9E8A[idx]
                            t = 10 * stor12 / 100 / s * stor9E8A[idx]
                            idx = idx + 1
                            continue 
                    s = 0
                    idx = 0
                ...
                if 30 * stor12 / 100 / s < stor3.length:
                    stor13 += 30 * stor12 / 100
                else:
                    t = 0
                    idx = 0
                    while idx < stor4.length:
                        ...
                        mem[0] = 4
                        if not address(stor5[stor9E8A[idx]]):
                            uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 80 * 30 * stor12 / 100 / s * stor9E8A[idx] / 100
                        else:
                            uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 30 * stor12 / 100 / s * stor9E8A[idx]
                        t = 30 * stor12 / 100 / s * stor9E8A[idx]
                        idx = idx + 1
                        continue 
                idx = 0
                s = 0
            ...
            if 60 * stor12 / 100 / s < stor3.length:
                stor13 += 60 * stor12 / 100
            else:
                t = 0
                idx = 0
                while idx < stor4.length:
                    ...
                    mem[0] = 4
                    if not address(stor5[stor9E8A[idx]]):
                        uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 80 * 60 * stor12 / 100 / s * stor9E8A[idx] / 100
                    else:
                        uint256(stor[code.data[7439 len 32] + (5 * idx)]) += 60 * stor12 / 100 / s * stor9E8A[idx]
                    t = 60 * stor12 / 100 / s * stor9E8A[idx]
                    idx = idx + 1
                    continue 
            idx = 0
            s = 0
    ...
    while uint8(t) < 5:
        ...
        while uint8(idx) < 6:
            require uint8(idx) < mem[128]
            require uint8(t) < mem[128]
            if mem[(32 * uint8(t)) + 191 len 1] <= mem[(32 * uint8(idx)) + 191 len 1]:
                u = u
                idx = idx + 1
                continue 
            require uint8(t) < mem[128]
            _271 = mem[(32 * uint8(t)) + 160]
            require uint8(idx) < mem[128]
            require uint8(t) < mem[128]
            mem[(32 * uint8(t)) + 160] = mem[(32 * uint8(idx)) + 191 len 1]
            require uint8(idx) < mem[128]
            mem[(32 * uint8(idx)) + 160] = uint8(_271)
            u = _271
            idx = idx + 1
            continue 
        s = u
