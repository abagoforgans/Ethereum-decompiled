...
function voters(address arg1) payable {
    return voters[arg1].field_0, uint8(voters[arg1].field_256), address(voters[arg1].field_256), voters[arg1].field_512
}
...
function proposals(uint256 arg1) payable {
    ...
    return stor2[arg1], stor[code.data[1094 len 32] + (2 * arg1)]
}

function giveRightToVote(address arg1) payable {
    require chairpersonAddress == msg.sender
    require not uint8(voters[address(arg1)].field_256)
    voters[address(arg1)].field_0 = 1
}

function vote(uint256 arg1) payable {
    require not uint8(voters[address(msg.sender)].field_256)
    uint8(voters[address(msg.sender)].field_256) = 1
    voters[address(msg.sender)].field_512 = arg1
    require arg1 < stor2.length
    stor[code.data[1094 len 32] + (2 * arg1)] += voters[address(msg.sender)].field_0
}
...
function winningProposal() payable {
    ...
    while idx < stor2.length:
        mem[0] = 2
        if stor[code.data[1094 len 32] + (2 * idx)] <= s:
            idx = idx + 1
            s = s
            continue 
        ...
        mem[0] = 2
        idx = idx + 1
        s = stor[code.data[1094 len 32] + (2 * idx)]
        continue 
    return 0
}
