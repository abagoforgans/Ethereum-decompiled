...
function _fallback() payable {
    if calldata.size <= 0:
        if sub_2e5833ea[address(msg.sender)]:
            ...
            call seriesFactoryAddress.0x95f770fd with:
               value msg.value wei
                 gas gas_remaining - 9050 wei
                args 0, uint32(series.length), 0, 0, 0, 256, 288, msg.sender, 0, 0, 0, 0
            ...
            emit 0x51b48d0e: address(ext_call.return_data[0]), series.length
    else:
        ...
        if calldata.size:
            ...
            if not calldata.size % 32:
                call seriesFactoryAddress.0x95f770fd with:
                   value msg.value wei
                     gas gas_remaining - 9050 wei
                    args series.length, 224, 0, calldata.size + 256, calldata.size + 288, msg.sender, 0, calldata.size, mem[128], mem[160 len floor32(calldata.size - 1)], Mask(8 * -floor32(calldata.size - 1) + calldata.size, 0, 0), 0
            else:
                call seriesFactoryAddress.0x95f770fd with:
                   value msg.value wei
                     gas gas_remaining - 9050 wei
                    args series.length, 224, 0, floor32(calldata.size) + 288, floor32(calldata.size) + 320, msg.sender, 0, calldata.size, mem[128], mem[160 len floor32(calldata.size - 1)], Mask(8 * -floor32(calldata.size - 1) + floor32(calldata.size) + 32, 0, 0), 0
        else:
            if not calldata.size % 32:
                require ext_code.size(seriesFactoryAddress)
                call seriesFactoryAddress.0x95f770fd with:
                   value msg.value wei
                     gas gas_remaining - 9050 wei
                    args series.length, 224, 0, calldata.size + 256, calldata.size + 288, msg.sender, 0, calldata.size, mem[ceil32(calldata.size) + 452 len calldata.size], 0, 0
            else:
                mem[floor32(calldata.size) + ceil32(calldata.size) + 452] = mem[floor32(calldata.size) + ceil32(calldata.size) + -(calldata.size % 32) + 484 len calldata.size % 32]
                require ext_code.size(seriesFactoryAddress)
                call seriesFactoryAddress.0x95f770fd with:
                   value msg.value wei
                     gas gas_remaining - 9050 wei
                    args series.length, 224, 0, floor32(calldata.size) + 288, floor32(calldata.size) + 320, msg.sender, 0, calldata.size, mem[ceil32(calldata.size) + 452 len floor32(calldata.size) + 32], 0, 0
        require ext_call.success
        require ext_call.return_data[12 len 20]
        if ext_call.return_data[32] > 0:
            require not addr[ext_call.return_data[32]]
