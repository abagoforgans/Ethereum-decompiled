...
function claimDividends() {
    if not pendingWithdrawals[address(msg.sender)]:
        revert with 0, 'Your withdrawal balance is zero.'
    require claimId + 1 >= claimId
    claimId++
    claims.length++
    claims[claims.length].field_0 = claimId
    claims[claims.length].field_256 = msg.sender
    claims[claims.length].field_416 = 0
    claims[claims.length].field_512 = block.timestamp
    emit 0x4564c5b0: msg.sender, claimId, block.timestamp
}

function approveClaim(uint256 arg1) {
    if bool(stor67[address(msg.sender)]) != 1:
        revert with 0, 'You don't have admin rights.'
    require arg1 < claims.length
    require not claims[arg1].field_416
    pendingWithdrawals[stor64[arg1].field_256] = 0
    call claims[arg1].field_256 with:
       value pendingWithdrawals[stor64[arg1].field_256] wei
         gas 2300 * is_zero(value) wei
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    sub_e01a7739[stor64[arg1].field_256] = block.timestamp
    emit 0x1a983dd8: claims[arg1].field_256, arg1, pendingWithdrawals[stor64[arg1].field_256]
    claims[arg1].field_416 = 1
}
