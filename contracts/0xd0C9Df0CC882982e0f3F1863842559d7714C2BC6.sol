...
function _fallback() payable {
    mem[96 len -1784] = code.data[2353 len -1784]
    mem[64] = -1688
...
function vote(string arg1) payable {
    ...
    if not votes.length <= votes.length + 1:
        mem[0] = 0
        idx = votes.length + 1
        while sha3(0) + votes.length > idx + sha3(mem[0]):
    ...
    votes[votes.length][] = Array(len=arg1.length, data=arg1[all])
    stor7++
    if stor3 > 0:
        if stor7 >= stor3:
            if msg.sender == stor1:
