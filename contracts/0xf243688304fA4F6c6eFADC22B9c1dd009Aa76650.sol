...
function enter() payable {
    ...
    if msg.value <= 10^18:
        ...
        if not stor0.length > stor0.length + 1:
            ...
            if balance <= 200 * stor[code.data[1047 len 32] + (2 * stor1)] / 100:
        ...
        if not stor0.length > stor0.length + 1:
            require stor0.length < stor0.length
            stor0[stor0.length].field_0 = msg.sender or Mask(96, 160, stor0[stor0.length].field_0)
            stor[code.data[1047 len 32] + (2 * stor0.length)] = 10^18
            if not stor0.length:
                balance += 10^18
            else:
                collectedFees += 10^17
                call address(owner) with:
                   value collectedFees + 10^17 wei
                     gas 0 wei
                collectedFees = 0
                balance += 25 * 10^13 * 3600
            if balance <= 200 * stor[code.data[1047 len 32] + (2 * stor1)] / 100:
            require payoutIdx < stor0.length
            call address(stor0[stor1].field_0) with:
               value var24003 wei
