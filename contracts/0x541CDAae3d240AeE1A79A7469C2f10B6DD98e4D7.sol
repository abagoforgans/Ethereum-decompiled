...
function _fallback() payable {
    ...
    stor0 = msg.sender
    stor4 = 2 * 10^18
    stor5[address(this.address)] = 0
    emit code.data[4577 len 32]: 0, 0, this.address
    stor5[address(msg.sender)] = 2 * 10^18
    emit code.data[4577 len 32]: 2 * 10^18, 0, msg.sender
    bool(stor1.length) = 0
    stor1.length.field_1 = 9
    stor1.length.field_8 = 'Coindaily' / 256
    idx = 0
    ...
    stor2.length.field_8 = 'DAILY' / 256
    idx = 0
    while stor2.length + 31 / 32 > idx:
        stor2[idx].field_0 = 0
        idx = idx + 1
        continue 
    return code.data[583 len 3994]
}
...
function buy() payable {
    ...
    require 0 >= saleQuantity
    require saleQuantity <= 0
    if msg.value * buyPrice / 10^10 <= -saleQuantity:
        ...
        require msg.value + ethQuantity >= msg.value
        ethQuantity += msg.value
        balanceOf[address(this.address)] += saleQuantity
        balanceOf[msg.sender] -= saleQuantity
        emit Transfer(-saleQuantity, this.address, msg.sender);
        require msg.sender
        require -saleQuantity > 0
        require -saleQuantity + lockInfo[address(msg.sender)] <= balanceOf[address(msg.sender)]
        lockInfo[address(msg.sender)] -= saleQuantity
        emit Buy(msg.value, -saleQuantity, msg.sender);
}
