...
function buy() payable {
    ...
    if block.timestamp > stor7:
        ...
        stor11 += msg.value
        emit 0xfeddf252: msg.value, 0, msg.sender
    else:
        if msg.value:
            require msg.value
            require 2 * msg.value / msg.value == 2
        stor14[msg.sender] = 1
        require (2 * msg.value) + balanceOf[msg.sender] >= balanceOf[msg.sender]
        balanceOf[msg.sender] += 2 * msg.value
        require (2 * msg.value) + stor11 >= stor11
        stor11 += 2 * msg.value
        emit 0xfeddf252: (2 * msg.value), 0, msg.sender
    call _owner with:
