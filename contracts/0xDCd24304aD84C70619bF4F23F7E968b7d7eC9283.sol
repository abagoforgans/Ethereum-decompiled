...
function _fallback() payable {
    ...
    stor0 = msg.sender
    stor2[address(this.address)] = 40000000 * 10^18
    stor1 = 40000000 * 10^18
    emit Transfer(stor1, 0, this.address);
    return code.data[312 len 9203]
}
...
const name = 'e-balance coin (EBCoin)'
...
function DailyMint() {
    require 432480 * 24 * 3600 <= block.timestamp
    require block.timestamp - (432480 * 24 * 3600) / 24 * 3600 >= lastDayPaid
    if lastDayPaid / 30 > stor6:
        stor6++
        stor7 = 99 * stor7 / 100
    balanceOf[address(this.address)] += 10^18 * 95 * 10^6 * stor7 / 10 * 10^6 / 30
    totalSupply += 10^18 * 95 * 10^6 * stor7 / 10 * 10^6 / 30
    emit Transfer((10^18 * 95 * 10^6 * stor7 / 10 * 10^6 / 30), 0, this.address);
    emit DayMinted(lastDayPaid, 10^18 * 95 * 10^6 * stor7 / 10 * 10^6 / 30, block.timestamp);
    lastDayPaid++
}
