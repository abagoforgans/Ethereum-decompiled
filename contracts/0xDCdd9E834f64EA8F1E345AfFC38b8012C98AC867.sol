...
function discontinue() payable {
    if msg.sender == address(registrarAddress):
        require msg.value <= 0
        selfdestruct(msg.sender)
    emit code.data[2688 len 32]: 1
}
...
function edit(address arg1, bytes arg2, bool arg3) payable {
    if address(registrarAddress) != msg.sender:
        emit code.data[2688 len 32]: 1
        return 0
    require msg.value <= 0
    if not registrantIndex[address(arg1)]:
        emit code.data[2688 len 32]: 3
        return 0
    require registrantIndex[address(arg1)] < stor2.length
    ...
    emit Updated(msg.sender, Array(len=arg2.length, data=arg2[all]), arg3, arg1);
    return 1
}
...
function add(address arg1, bytes arg2) payable {
    if address(registrarAddress) != msg.sender:
        emit code.data[2688 len 32]: 1
        return 0
    require msg.value <= 0
    if registrantIndex[address(arg1)] > 0:
        emit code.data[2688 len 32]: 2
        return 0
    stor2.length++
    ...
    emit Created(msg.sender, Array(len=arg2.length, data=arg2[all]), arg1);
    return 1
}
