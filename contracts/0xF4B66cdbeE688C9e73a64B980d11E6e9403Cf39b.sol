...
function _fallback() payable {
    mem[96 len -1320] = code.data[1559 len -1320]
    mem[64] = -1224
    ...
    while idx < stor1:
        ...
        if not stor2 <= stor2 + 1:
            s = stor2 + code.data[1527 len 32] + 1
            while code.data[1527 len 32] + stor2 > s:
                stor[s] = 0
                s = s + 1
                continue 
        ...
        require idx < stor2
        mem[0] = 2
        stor[code.data[1527 len 32] + idx] = mem[(32 * idx) + mem[96] + 128]
        idx = idx + 1
        continue 
    return code.data[239 len 1288]
}
...
function getRandom() payable {
    ...
    while idx < stor2.length:
        ...
        continue 
    mem[mem[64]] = sha3(mem[mem[64] len -mem[64] + 180]) + (_21 * stor2.length)
    return memory
      from mem[64]
       len 32
}
