...
function _fallback() payable {
    require eth.balance(multisigVaultAddress) + altDeposits <= stor5
    require block.timestamp > start
    require block.timestamp < start + (288 * 24 * 3600)
    ...
    emit TokenSold(address(msg.sender), msg.value, ext_call.return_data[0] * msg.value / 10^18, ext_call.return_data[0]);
}
...
function createTokens(address arg1) payable {
    ...
    call tokenAddress.0x40c10f19 with:
         gas gas_remaining - 710 wei
        args address(arg1), ext_call.return_data[0] * msg.value / 10^18
    require ext_call.success
    call multisigVaultAddress with:
       value msg.value wei
         gas 2300 * is_zero(value) wei
    emit TokenSold(address(arg1), msg.value, ext_call.return_data[0] * msg.value / 10^18, ext_call.return_data[0]);
}
