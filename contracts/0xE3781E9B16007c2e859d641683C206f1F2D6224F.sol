...
const stage_3_add = 18

const decimals = 18

const stage_2_add = 33

const bonusAdd = 99

const stage_1_add = 50

const bonusTokenThreshold = 2000000 * 10^18

const minContribution = 10^16

const bonusMinContribution = 10^17
...
function grant_token_from_offchain(address arg1, uint256 arg2, string arg3) {
    ...
    call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
         gas gas_remaining - 710 wei
...
function _fallback() payable {
    ...
    if block.timestamp < tCampaignStart:
        ...
        if block.timestamp > tCampaignEnd:
            ...
            if block.timestamp <= tBonusStageEnd:
                require 429 * 10^18 * msg.value / 429 * 10^18 == msg.value
                require 10 * msg.value / 10 == msg.value
                call opVaultAddr with:
                   value 10 * msg.value / 100 wei
                     gas 2300 * is_zero(value) wei
                ...
                call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                     gas gas_remaining - 710 wei
                    args msg.sender, 429 * 10^18 * msg.value / 10^18
                require ext_call.success
                require ext_call.return_data[0]
                require (429 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                tokensGenerated += 429 * 10^18 * msg.value / 10^18
                require msg.value + amountRaised >= amountRaised
                amountRaised += msg.value
                emit TokenGranted((429 * 10^18 * msg.value / 10^18), msg.sender);
            else:
                if block.timestamp <= t_1st_StageEnd:
                    require 380 * 10^18 * msg.value / 380 * 10^18 == msg.value
                    require 10 * msg.value / 10 == msg.value
                    call opVaultAddr with:
                       value 10 * msg.value / 100 wei
                         gas 2300 * is_zero(value) wei
                    ...
                    call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                         gas gas_remaining - 710 wei
                        args msg.sender, 380 * 10^18 * msg.value / 10^18
                    require ext_call.success
                    require ext_call.return_data[0]
                    require (380 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                    tokensGenerated += 380 * 10^18 * msg.value / 10^18
                    require msg.value + amountRaised >= amountRaised
                    amountRaised += msg.value
                    emit TokenGranted((380 * 10^18 * msg.value / 10^18), msg.sender);
                else:
                    if block.timestamp <= t_2nd_StageEnd:
                        require 363 * 10^18 * msg.value / 363 * 10^18 == msg.value
                        require 10 * msg.value / 10 == msg.value
                        call opVaultAddr with:
                           value 10 * msg.value / 100 wei
                             gas 2300 * is_zero(value) wei
                        ...
                        call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                             gas gas_remaining - 710 wei
                            args msg.sender, 363 * 10^18 * msg.value / 10^18
                        require ext_call.success
                        require ext_call.return_data[0]
                        require (363 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                        tokensGenerated += 363 * 10^18 * msg.value / 10^18
                        require msg.value + amountRaised >= amountRaised
                        amountRaised += msg.value
                        emit TokenGranted((363 * 10^18 * msg.value / 10^18), msg.sender);
                    else:
                        if block.timestamp > t_3rd_StageEnd:
                            ...
                            require 348 * 10^18 * msg.value / 348 * 10^18 == msg.value
                            require 10 * msg.value / 10 == msg.value
                            call opVaultAddr with:
                               value 10 * msg.value / 100 wei
                                 gas 2300 * is_zero(value) wei
                            ...
                            call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                                 gas gas_remaining - 710 wei
                                args msg.sender, 348 * 10^18 * msg.value / 10^18
                            require ext_call.success
                            require ext_call.return_data[0]
                            require (348 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                            tokensGenerated += 348 * 10^18 * msg.value / 10^18
                            require msg.value + amountRaised >= amountRaised
                            amountRaised += msg.value
                            emit TokenGranted((348 * 10^18 * msg.value / 10^18), msg.sender);
    ...
    if block.timestamp < tCampaignStart:
        ...
        if block.timestamp > tCampaignEnd:
            ...
            if block.timestamp <= tBonusStageEnd:
                ...
                call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                     gas gas_remaining - 710 wei
                    args address(arg1), 429 * 10^18 * msg.value / 10^18
                require ext_call.success
                require ext_call.return_data[0]
                require (429 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                tokensGenerated += 429 * 10^18 * msg.value / 10^18
                require msg.value + amountRaised >= amountRaised
                amountRaised += msg.value
                emit TokenGranted((429 * 10^18 * msg.value / 10^18), arg1);
            else:
                if block.timestamp <= t_1st_StageEnd:
                    require 380 * 10^18 * msg.value / 380 * 10^18 == msg.value
                    require 10 * msg.value / 10 == msg.value
                    ...
                    call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                         gas gas_remaining - 710 wei
                        args address(arg1), 380 * 10^18 * msg.value / 10^18
                    require ext_call.success
                    require ext_call.return_data[0]
                    require (380 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                    tokensGenerated += 380 * 10^18 * msg.value / 10^18
                    require msg.value + amountRaised >= amountRaised
                    amountRaised += msg.value
                    emit TokenGranted((380 * 10^18 * msg.value / 10^18), arg1);
                else:
                    if block.timestamp <= t_2nd_StageEnd:
                        require 363 * 10^18 * msg.value / 363 * 10^18 == msg.value
                        require 10 * msg.value / 10 == msg.value
                        ...
                        call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                             gas gas_remaining - 710 wei
                            args address(arg1), 363 * 10^18 * msg.value / 10^18
                        require ext_call.success
                        require ext_call.return_data[0]
                        require (363 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                        tokensGenerated += 363 * 10^18 * msg.value / 10^18
                        require msg.value + amountRaised >= amountRaised
                        amountRaised += msg.value
                        emit TokenGranted((363 * 10^18 * msg.value / 10^18), arg1);
                    else:
                        if block.timestamp > t_3rd_StageEnd:
                            require 330 * 10^18 * msg.value / 330 * 10^18 == msg.value
                            require 10 * msg.value / 10 == msg.value
                            ...
                            call tokenAddress.generate_token_for(address rg1, uint256 rg2) with:
                                 gas gas_remaining - 710 wei
                                args address(arg1), 348 * 10^18 * msg.value / 10^18
                            require ext_call.success
                            require ext_call.return_data[0]
                            require (348 * 10^18 * msg.value / 10^18) + tokensGenerated >= tokensGenerated
                            tokensGenerated += 348 * 10^18 * msg.value / 10^18
                            require msg.value + amountRaised >= amountRaised
                            amountRaised += msg.value
                            emit TokenGranted((348 * 10^18 * msg.value / 10^18), arg1);
    emit TotalRaised(amountRaised);
}
