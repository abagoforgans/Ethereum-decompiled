...
function sub_7e4620a8(?) payable {
    mem[352] = mem[372 len 12]
    return stor7.length, Array(len=44, data='Number of participations since t', 'he beginning', mem[352])
}

function sub_936e75b2(?) payable {
    mem[288] = Mask(144, 56, 'Number of deposit waiting') >> 56, mem[313 len 7]
    return stor7.length - stor5, Array(len=25, data=mem[288])
}
...
function CollectAllFees() payable {
    if msg.sender == address(stor6):
        require sub_00bfee66 != 0
        call address(stor6) with:
           value sub_00bfee66 wei
             gas 0 wei
        sub_00bfee66 = 0
}
...
function PlayerInfo(uint256 arg1) payable {
    ...
    return address(stor[code.data[1840 len 32] + (3 * arg1)]), uint256(stor7[arg1].field_256) / 10^15, storA66C[arg1]
}
...
function GetAndReduceFeesByFraction(uint256 arg1) payable {
    if msg.sender == address(stor6):
        if 0 == sub_00bfee66:
            stor3 = 50 * stor3 / 100
        call address(stor6) with:
           value arg1 * sub_00bfee66 / 1000 wei
             gas 0 wei
        sub_00bfee66 += -1 * arg1 * sub_00bfee66 / 1000
}
...
function _fallback() payable {
    ...
    if msg.value <= stor4:
        ...
        if not stor7.length > stor7.length + 1:
            ...
            require stor5 < stor7.length
            call var29004 with:
               value uint256(stor[code.data[1840 len 32] + (3 * stor5) + 1]) wei
                 gas 0 wei
            stor0 -= storA66C[stor5]
            uint8(stor[code.data[1840 len 32] + (3 * stor5) + 2]) = 1
            stor5++
            if stor0 < storA66C[stor5]:
            require stor5 < stor7.length
            if 0 == storA66C[stor5]:
