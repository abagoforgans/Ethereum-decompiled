...
function _fallback() payable {
    ...
    stor1.length = code.data[5229 len 32]
    stor2.length = code.data[5261 len 32]
    stor3 = code.data[5293 len 32]
    stor7++
    if not stor7 <= stor7 + 1:
        idx = (3 * stor7) + code.data[5197 len 32] + 3
        while code.data[5197 len 32] + (3 * stor7) > idx:
            ...
            continue 
    require 0 < stor7
    Mask(168, 0, stor[code.data[5197 len 32]].field_0) = 0
    stor[code.data[5197 len 32]].field_256 % 1 = 0
    stor[code.data[5197 len 32]].field_256 % 1 = 0
    stor[code.data[5197 len 32]].field_256 % 1 = 0
    bool(stor1[code.data[5197 len 32]].field_0) = 0
    uint255(stor1[code.data[5197 len 32]].field_1) = 0
    Mask(248, 0, stor1[code.data[5197 len 32]].field_8) = mem[384 len 31]
    idx = 0
    while stor[code.data[5197 len 32] + 1].length + 31 / 32 > idx:
        uint256(stor[idx + sha3(code.data[5197 len 32] + 1)].field_0) = 0
        idx = idx + 1
        continue 
    stor2[code.data[5197 len 32]] = block.timestamp
    if address(code.data[5325 len 32]) != 0:
        stor0 = code.data[5325 len 32] or Mask(96, 160, stor0)
    return code.data[615 len 4582]
}
