...
function donateEth() payable {
    ...
    emit Donation(0, msg.value, block.timestamp, sha3(block.timestamp, block.difficulty) / uint8(msg.value / supportedTokens[0]));
}
...
function donateToken(address arg1, uint256 arg2) {
    ...
    emit Donation(address(arg1), arg2, block.timestamp, sha3(block.timestamp, block.difficulty) / uint8(arg2 / supportedTokens[address(arg1)]));
}
