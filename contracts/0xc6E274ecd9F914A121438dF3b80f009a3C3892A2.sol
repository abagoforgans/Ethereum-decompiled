...
function contestant(uint256 arg1) payable {
    ...
    return address(stor[code.data[942 len 32] + arg1])
}
...
function enter() payable {
    if 10^18 != msg.value:
        ...
        if not idx:
            ...
            uint256(stor[code.data[942 len 32] + stor6]) = msg.sender or Mask(96, 160, uint256(stor[code.data[942 len 32] + stor6]))
        else:
            if stor2 - stor1 <= 180:
                stor1 = stor2
            else:
                require idx < stor0.length
                call address(stor[code.data[942 len 32] + stor6]) with:
                ...
                if not stor0.length <= 0:
                    ...
                    while stor0.length > idx:
                        ...
                        continue 
                stor0.length = 0
                idx = code.data[942 len 32]
                while code.data[942 len 32] + stor0.length > idx:
                    address(stor[idx]) = 0
                    idx = idx + 1
                    continue 
            stor0.length++
