...
function sub_7056074a(?) {
    ...
    staticcall stor2.0x217fe6c6 with:
            gas gas_remaining wei
           args msg.sender, 'SUDO'
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    require return_data.size >= 32
    if address(ext_call.return_data[0]) != msg.sender:
        if not ext_call.return_data[0]:
            revert with 0, 'Only a Sudo Member or Owner can execute this function.'
    mem[260] = uint256(stor3.field_0)
    idx = 260
    s = 0
    while stor3.length + 260 > idx + 32:
        mem[idx + 32] = stor3[s].field_256
        idx = idx + 32
        s = s + 1
        continue 
    ...
    call stor2.0x7d72aa65 with:
         gas gas_remaining wei
        args address(arg1), Array(len=stor3.length, data=mem[260 len stor3.length + (floor32(stor3.length - 1) + -stor3.length + 32 % 32)])
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
    mem[260] = uint256(stor3.field_0)
    idx = 260
