...
function niceGuys(uint256 arg1) payable {
    require arg1 < stor1
    return address(stor[code.data[932 len 32] + arg1])
}

function cNiceGuyIdx() payable {
    return cNiceGuyIdx
}
...
function cNiceGuy() payable {
    return cNiceGuyAddress
}
...
function enter() payable {
    ...
    if investor >= 9:
        ...
        if not stor1 <= stor1 + 1:
            idx = stor1 + code.data[932 len 32] + 1
            while code.data[932 len 32] + stor1 > idx:
                address(stor[idx]) = 0
                idx = idx + 1
                continue 
        require stor1 < stor1
        uint256(stor[code.data[932 len 32] + stor1]) = msg.sender or Mask(96, 160, uint256(stor[code.data[932 len 32] + stor1]))
        investor++
        if investor != 10:
            # nil
        else:
            require cNiceGuyIdx < stor1
            cNiceGuyAddress = address(stor[code.data[932 len 32] + stor3])
            investor = 0
            cNiceGuyIdx++
            # nil
    else:
