...
function _fallback() payable {
    ...
    if not stor6 <= stor6 + 1:
        mem[0] = 6
        idx = stor6 + 1
        while sha3(6) + stor6 > idx + sha3(mem[0]):
...
function owners(uint256 arg1) payable {
    ...
    return address(stor[code.data[2929 len 32] + arg1])
}
...
function sub_f58237da(?) payable {
    ...
    if address(stor[stor1[address(arg1)] + code.data[2929 len 32]]) != tx.origin:
        if owner != tx.origin:
            return 0
    ...
    while sub_8205090e[stor1[address(arg1)]].length + 31 / 32 > idx:
        ...
        continue 
    require sub_f85e610e[address(arg1)] < stor4.length
    address(stor[stor1[address(arg1)] + code.data[2929 len 32]]) = 0
...
function sub_f0bbba87(?) payable {
    ...
    if not sub_a2e28f26.length <= sub_a2e28f26.length + 1:
        ...
        while sha3(6) + sub_a2e28f26.length > idx + sha3(mem[0]):
...
function sub_fc000500(?) payable {
    ...
    while idx < stor2:
        ...
        if idx != 0:
            ...
            uint256(stor[idx + code.data[2929 len 32]]) = tx.origin or Mask(96, 160, uint256(stor[idx + code.data[2929 len 32]]))
            ...
            uint256(stor[stor2 + code.data[2929 len 32]]) = tx.origin or Mask(96, 160, uint256(stor[stor2 + code.data[2929 len 32]]))
