...
function sub_a2d92d66(?) payable {
    return address(sub_a2d92d66Address)
}
...
function sub_b7630118(?) payable {
    require msg.value <= 0
    if msg.sender == address(sub_a2d92d66Address):
...
function sub_f5a1abef(?) payable {
    require msg.value <= 0
    if address(sub_a2d92d66Address) != msg.sender:
        emit Error(1);
        return 0
    ...
    emit 0x9d8c5bb9: msg.sender, Array(len=arg2.length, data=arg2[all]), bool(arg3), arg1
    return 1
}
...
function sub_36d6da55(?) payable {
    ...
    emit 0x8128eaba: msg.sender, Array(len=arg2.length, data=arg2[all]), arg1
    return 1
}
