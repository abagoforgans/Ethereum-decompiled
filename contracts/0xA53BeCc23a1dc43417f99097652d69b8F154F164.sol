...
const start_timestamp = (421754 * 3600)

const sub_876ac3eb(?) = 1519574399
...
function check() {
    if block.timestamp < 421754 * 3600:
        state = 0
    else:
        if block.timestamp >= 1519574399:
...
function _fallback() payable {
    ...
    require sub_1e243377 - sub_4cf59e13 >= (3000 * msg.value / 10^18) + (30 * 3000 * msg.value / 10^18 / 100)
    sub_27cb9a16 += msg.value
    sub_c9c23038 += 3000 * msg.value / 10^18
    sub_4cf59e13 = sub_4cf59e13 + (3000 * msg.value / 10^18) + (30 * 3000 * msg.value / 10^18 / 100)
    balances[address(msg.sender)] = balances[address(msg.sender)] + (3000 * msg.value / 10^18) + (30 * 3000 * msg.value / 10^18 / 100)
    sub_31b22544[address(msg.sender)] += msg.value
    emit 0x5a940f0f: address(msg.sender), (3000 * msg.value / 10^18) + (30 * 3000 * msg.value / 10^18 / 100), msg.value, 30 * 3000 * msg.value / 10^18 / 100
}
