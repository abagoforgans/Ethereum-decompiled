...
const _fallback = code.data[18 len 1686]
...
function getAccountBalance(address arg1) payable {
    call address(index['elcoinDb']).getBalance(address rg1) with:
...
function _fallback() payable {
    if calldata.size <= 0:
        return 0
    call address(index['elcoin']) with:
...
function sub_3ed48679(?) payable {
    call address(index['elcoin']).transferPool(address rg1, address rg2, uint256 rg3) with:
         gas gas_remaining - 25050 wei
