...
const PRICE_STAGE_TWO = 10^6

const MAX_SUPPLY = 32 * 10^10

const ALLOC_BOUNTIES = 16 * 10^9

const STAGE_TWO_TIME_END = (216 * 24 * 3600)

const STAGE_ONE_TIME_END = (96 * 24 * 3600)

const ALLOC_CROWDSALE = 128 * 10^9

const PRICE_STAGE_ONE = 1500000

const PRICE_PRESALE = 1500000

const ALLOC_TEAM = 48 * 10^9

const PRICE_STAGE_THREE = 600000
...
function getPriceRate() {
    if block.timestamp <= publicStartTime:
        return 1500000
    if block.timestamp <= publicStartTime + (96 * 24 * 3600):
        return 1500000
    if block.timestamp <= publicStartTime + (216 * 24 * 3600):
        return 10^6
...
function _fallback() payable {
    ...
    if block.timestamp <= publicStartTime:
        ...
        require 1500000 * msg.value / 10^18 <= -sub_d16825c8 + 128 * 10^9
        if msg.value:
            require msg.value
        ...
        sub_d16825c8 += 1500000 * msg.value / 10^18
        ...
        if block.timestamp <= publicStartTime + (96 * 24 * 3600):
            ...
            if block.timestamp <= publicStartTime + (216 * 24 * 3600):
                ...
                if block.timestamp > publicStartTime + (336 * 24 * 3600):
                    ...
                    if msg.value:
                        require msg.value
                        require 600000 * msg.value / msg.value == 600000
                    require 600000 * msg.value == (10^18 * 600000 * msg.value / 10^18) + (600000 * msg.value % 10^18)
                    require 600000 * msg.value / 10^18 <= -sub_d16825c8 + 128 * 10^9
                    if msg.value:
                        require msg.value
                        require 15 * msg.value / msg.value == 15
                    require 15 * msg.value == (1000 * 15 * msg.value / 1000) + (15 * msg.value % 1000)
                    ...
                    call sub_1188aedaAddress.createIlliquidToken(address rg1, uint256 rg2) with:
                         gas gas_remaining - 710 wei
                        args msg.sender, 600000 * msg.value / 10^18
                    require ext_call.success
                    require ext_call.return_data[0]
                    sub_d16825c8 += 600000 * msg.value / 10^18
                    totalEtherRaised += msg.value
                    etherContributions[address(msg.sender)] += msg.value
                    emit Buy((600000 * msg.value / 10^18), msg.sender);
}
