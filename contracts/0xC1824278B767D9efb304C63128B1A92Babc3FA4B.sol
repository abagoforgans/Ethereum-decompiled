...
function participants(uint256 arg1) payable {
    ...
    return address(stor5629[arg1]), stor[code.data[1105 len 32] + (2 * arg1)]
}
...
function enter() payable {
    if msg.value < 10^18:
        ...
        stor[code.data[1105 len 32] + (2 * stor0.length)] = msg.value
        if not stor0.length:
        ...
        if balance > 2 * stor[code.data[1105 len 32] + (2 * stor1)]:
            require payoutIdx < stor0.length
            call address(stor5629[stor1]) with:
               value 2 * stor5629[stor1] - (stor5629[stor1] / 10) wei
                 gas 0 wei
            balance -= 2 * stor[code.data[1105 len 32] + (2 * stor1)]
            payoutIdx++
}
