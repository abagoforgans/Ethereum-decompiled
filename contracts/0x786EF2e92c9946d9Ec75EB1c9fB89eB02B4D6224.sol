...
function createSale(address arg1, uint256 arg2) payable {
    ...
    tokenSalesByAsset[address(msg.sender)]++
    if not tokenSalesByAsset[address(msg.sender)] <= tokenSalesByAsset[address(msg.sender)] + 1:
        idx = tokenSalesByAsset[address(msg.sender)] + 1
        while tokenSalesByAsset[address(msg.sender)] > idx:
            tokenSalesByAsset[address(msg.sender)][idx] = 0
            idx = idx + 1
            continue 
    tokenSalesByAsset[address(msg.sender)][tokenSalesByAsset[address(msg.sender)]] = tokenSalesAll.length
