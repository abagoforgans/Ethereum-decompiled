...
const OWNER = 0x29709eab90799566053a251fedbb205c112f16e7

const MIN_ACCEPTED_AMOUNT_FINNEY = 1

const WITHDRAWAL_END = 4780000

const MAX_TOTAL_AMOUNT_TO_RECEIVE_ETH = 2000

const PRESALE_START = 4262000
...
function abort() {
    ...
    if block.number >= 4262000:
        if block.number > 4400000:
            require eth.balance(this.address)
            require block.number <= 4780000
            require total_received_amount >= 200 * 10^18
        else:
            if total_received_amount >= 2000 * 10^18:
                require eth.balance(this.address)
                require block.number <= 4780000
                require total_received_amount >= 200 * 10^18
    require 0x29709eab90799566053a251fedbb205c112f16e7 == msg.sender
    uint8(stor7.field_0) = 1
}

function withdrawFunds() {
    require not uint8(stor7.field_0)
    require block.number >= 4262000
    if block.number <= 4400000:
        require total_received_amount >= 2000 * 10^18
    require eth.balance(this.address)
    require block.number <= 4780000
    require total_received_amount >= 200 * 10^18
    require 0x29709eab90799566053a251fedbb205c112f16e7 == msg.sender
    ...
    call 0x29709eab90799566053a251fedbb205c112f16e7 with:
    ...
    if uint8(stor7.field_0):
        ...
        if block.number > 4400000:
            ...
            if total_received_amount < 2000 * 10^18:
                ...
                if total_received_amount + msg.value <= 2000 * 10^18:
                    balances[address(msg.sender)] += msg.value
                    total_received_amount += msg.value
                else:
                    call msg.sender with:
                       value msg.value + total_received_amount - 2000 * 10^18 wei
                         gas 2300 * is_zero(value) wei
                    require ext_call.success
                    balances[address(msg.sender)] = -total_received_amount + balances[address(msg.sender)] + 2000 * 10^18
                    total_received_amount = 2000 * 10^18
            else:
                require eth.balance(this.address)
                if block.number <= 4780000:
                    require total_received_amount < 200 * 10^18
