...
function Invest() payable {
    require stor0 == msg.sender
    if 256 * 10^15 * eth.balance(this.address) / 256 / 256 * 10^15 != uint256(stor2.field_0):
        uint256(stor2.field_0) = 256 * 10^15 * eth.balance(this.address) / 256 / 256 * 10^15
        emit SettingsChanged(256 * 10^15 * eth.balance(this.address) / 256 / 256 * 10^15, Mask(248, 8, 256 * 10^15 * eth.balance(this.address) / 256 / 256 * 10^15), stor7, storFF55, stor9EBB, uint8(stor1.field_0), bool(uint8(stor3.field_128)));
}
...
function sub_7fa94d65(?) {
    ...
    if uint8(stor9[arg1].field_768) != 37:
        ...
        if stor10[stor9[arg1].field_2048] != sha3(arg2):
            emit ErrorLog(address(msg.sender), Array(len=12, data='Hash differs'));
        else:
            require arg1 < stor9.length
            require arg1 < stor9.length
            uint8(stor9[arg1].field_768) = uint8(sha3(arg2, stor9[arg1].field_1792) % 37)
            stor9[arg1].field_512 = block.number
            s = 0
            idx = 0
            t = 0
