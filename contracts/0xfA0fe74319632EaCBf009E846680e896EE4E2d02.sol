...
function getGameBlockNumber(uint256 arg1) {
    require arg1 < games.length
...
function placeBet(uint8 arg1, uint8 arg2) payable {
    ...
    games[games.length].field_1280 = 0
    games[games.length].field_1536 = block.number
    if games.length <= 0:
        return 0
    require games.length < games.length
    require games.length - 1 < games.length
    if games[games.length].field_0 == games[games.length].field_1536:
        return 0
    require games.length - 1 < games.length
    games[games.length].field_0 = block.hash(block.number - 1)
    require tx.origin == msg.sender
    stor7++
    stor6 = sha3(block.timestamp, stor7 + 1)
    require games.length - 1 < games.length
    games[games.length].field_0 = uint8(uint8(sha3(games[games.length].field_0, sha3(block.timestamp, stor7 + 1))) % 9)
    require games.length - 1 < games.length
    ...
    call games[games.length].field_0 with:
       value games[games.length].field_0 wei
         gas 2300 * is_zero(value) wei
    require ext_call.success
    require games.length - 1 < games.length
