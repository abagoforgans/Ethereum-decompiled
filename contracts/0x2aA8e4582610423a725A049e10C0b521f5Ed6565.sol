...
function _fallback() payable {
    ...
    if not msg.value % price:
        if msg.value / price <= ext_call.return_data[0]:
            ...
            saleState = 3
            if (msg.value % price) + msg.value - (ext_call.return_data[0] * price) > 0:
                call msg.sender with:
                   value (msg.value % price) + msg.value - (ext_call.return_data[0] * price) wei
                     gas 2300 * is_zero(value) wei
                require ext_call.success
            require ext_code.size(assetAddress)
            call assetAddress.0xa9059cbb with:
        ...
        if msg.value / price <= ext_call.return_data[0]:
            ...
            saleState = 3
            if msg.value - (ext_call.return_data[0] * price) > 0:
                call msg.sender with:
