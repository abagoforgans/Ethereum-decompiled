...
function _fallback() payable {
    mem[96 len -2573] = code.data[3096 len -2573]
    mem[64] = -2477
...
function sell(uint256 arg1) payable {
    sub_bc094049 = eth.balance(this.address) / totalSupply
    require balanceOf[address(msg.sender)] >= arg1
    balanceOf[address(this.address)] += arg1
    balanceOf[msg.sender] -= arg1
    call msg.sender with:
       value arg1 / 10^11 * sub_bc094049 wei
         gas 0 wei
    emit 0x56ddf252: arg1, msg.sender, this.address
    return (arg1 / 10^11 * sub_bc094049)
}
...
function buy() payable {
    ...
    if s >= uint32(odds):
        ...
        balanceOf[address(stor5)] += msg.value / 10^11 / 999
        balanceOf[this.address] = balanceOf[this.address] - (2 * msg.value / 10^11) - (msg.value / 10^11 / 999)
