...
function slice(bytes arg1, uint256 arg2, uint256 arg3) {
    ...
    if arg3:
        mem[ceil32(arg1.length) + 160 len 32 * arg3] = code.data[7737 len 32 * arg3]
    idx = 0
    while idx < arg3:
...
function getGameInfo() {
    ...
    call mConfigAddress.getWinRate(uint8 arg1) with:
    ...
    require ext_code.size(mConfigAddress)
    call mConfigAddress.getOverRate(uint8 arg1) with:
         gas gas_remaining wei
        args uint8(stor1.field_8)
    if not ext_call.success:
        revert with ext_call.return_data[0 len return_data.size]
...
function receiveApproval(address arg1, uint256 arg2, address arg3, bytes arg4) {
    ...
    if arg4.length - 1:
        mem[ceil32(arg4.length) + 160 len 32 * arg4.length - 1] = code.data[7737 len 32 * arg4.length - 1]
    idx = 0
    ...
    mem[ceil32(arg4.length) + floor32(arg4.length + 30) + 192] = code.data[7737 len 32]
    idx = 0
    while idx < 1:
    ...
    require return_data.size >= 32
    require ext_code.size(mConfigAddress)
    call mConfigAddress.getRandom(bytes32 arg1) with:
         gas gas_remaining wei
    ...
    if mem[ceil32(arg4.length) + 128] != 0:
        if mConfigAddress:
            require ext_code.size(mConfigAddress)
            call mConfigAddress.GetAddressByName(bytes32 arg1) with:
                 gas gas_remaining wei
            ...
            if address(ext_call.return_data[0]):
                require ext_code.size(mConfigAddress)
                call mConfigAddress.getInviteRate() with:
                     gas gas_remaining wei
                if not ext_call.success:
                    revert with ext_call.return_data[0 len return_data.size]
                require return_data.size >= 32
    ...
    if address(stor1.field_16):
        if sha3(msg.sender, sha3(msg.sender, Mask(96, 0, stor2.field_160), address(stor2.field_0)), sha3(msg.sender, Mask(96, 0, stor2.field_160), address(stor2.field_0)) % 100) % 100 < ext_call.return_data[0]:
            ...
            require ext_code.size(mConfigAddress)
            call mConfigAddress.getPumpRate() with:
                 gas gas_remaining wei
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            require return_data.size >= 32
