...
function buy() {
    ...
    if ext_call.return_data[0] + 1 < ext_call.return_data[0]:
        ...
        if ext_code.size(0 or saleAddress):
            call 0 or saleAddress.0xf48c3054 with:
               value eth.balance(this.address) - bounty wei
                 gas gas_remaining - 9710 wei
                args this.address
            if ext_call.success:
                require eth.balance(this.address) <= eth.balance(this.address)
                if eth.balance(this.address) - bounty:
                    bounty -= 0 / eth.balance(this.address) - bounty
                    call msg.sender with:
                       value 0 / eth.balance(this.address) - bounty wei
