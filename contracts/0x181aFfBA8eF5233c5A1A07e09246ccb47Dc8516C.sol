    ...
    while idx <= arg1:
        ...
        while s < sub_e512e0ac:
            ...
            while t >= 32:
                ...
                t = t - 32
                continue 
            mem[mem[64] + floor32(mem[_116])] = mem[mem[64] + floor32(mem[_116])] and 256^(-(mem[_116] % 32) + 32) - 1 or mem[_116 + floor32(mem[_116]) + 32] and !(256^(-(mem[_116] % 32) + 32) - 1)
            _164 = sha3(mem[mem[64] len _115 + _118 + -mem[64] + 52])
            ...
            while t >= 32:
                ...
                t = t - 32
                continue 
            mem[mem[64] + floor32(mem[_167])] = mem[mem[64] + floor32(mem[_167])] and 256^(-(mem[_167] % 32) + 32) - 1 or mem[_167 + floor32(mem[_167]) + 32] and !(256^(-(mem[_167] % 32) + 32) - 1)
            require block.timestamp
            if block.difficulty + block.timestamp < block.timestamp:
                revert with 0, 'SafeMath add failed'
            if sha3(mem[mem[64] len _166 + _169 + -mem[64] + 52]) / block.timestamp < 0:
                revert with 0, 'SafeMath add failed'
            if block.gas_limit < 0:
                revert with 0, 'SafeMath add failed'
            if _164 / block.timestamp < 0:
                revert with 0, 'SafeMath add failed'
            if block.number < 0:
                revert with 0, 'SafeMath add failed'
            _218 = mem[64]
            mem[mem[64] + 32] = block.number + (_164 / block.timestamp) + block.gas_limit + (sha3(mem[mem[64] len _166 + _169 + -mem[64] + 52]) / block.timestamp) + block.difficulty + block.timestamp
            _219 = mem[64]
            mem[mem[64]] = 32
            mem[64] = mem[64] + 64
            _221 = mem[_219]
            ...
            while t >= 32:
                ...
                t = t - 32
                continue 
            mem[_218 + floor32(mem[_219]) + 64] = mem[_218 + floor32(mem[_219]) + 64] and 256^(-(mem[_219] % 32) + 32) - 1 or mem[_219 + floor32(mem[_219]) + 32] and !(256^(-(mem[_219] % 32) + 32) - 1)
            if sha3(mem[_218 + 64 len _221]) - (1000 * sha3(mem[_218 + 64 len _221]) / 1000) >= ext_call.return_data[0]:
                t = ('signextend', 1, ('signextend', 1, ('field', 0, ('stor', ('map', ('var', 1), ('name', 'sub_b3e0cf7e', 1))))))
                s = s + 1
                continue 
            require ext_code.size(sub_b3e0cf7e[s].field_16)
            if sha3(mem[_218 + 64 len _221]) - (1000 * sha3(mem[_218 + 64 len _221]) / 1000) >= 5:
                call sub_b3e0cf7e[s].field_16.0x50a11a53 with:
                   value msg.value wei
                     gas gas_remaining wei
                    args idx, 1
            else:
                call sub_b3e0cf7e[s].field_16.0x50a11a53 with:
                   value msg.value wei
                     gas gas_remaining wei
                    args idx, ('signextend', 0, ('signextend', 0, ('signextend', 0, ('type', 256, ('stor', ('name', 'stor3', 3))))))
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
            sub_b3e0cf7e[s].field_0 = uint16(('signextend', 1, ('add', 1, ('var', 0), ('signextend', 1, ('field', 0, ('stor', ('map', ('var', 1), ('name', 'sub_b3e0cf7e', 1))))))))
