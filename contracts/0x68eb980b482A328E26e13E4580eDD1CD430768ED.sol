...
function sub_4d92df14(?) {
    ...
    while uint8(idx) < arg1.length:
        ...
        while uint8(s) < 9:
            ...
            require uint8(idx) < arg2.length
            require uint8(s) < 9
            if mem[(32 * uint8(idx)) + (32 * arg1.length) + 160] < 0:
                stor4[uint8(s)] += test266151307() * mem[(32 * uint8(idx)) + (32 * arg1.length) + 160]
            else:
                stor4[uint8(s)] += mem[(32 * uint8(idx)) + (32 * arg1.length) + 160]
            idx = idx + 1
            continue 
        idx = idx + 1
        continue 
...
function _fallback() payable {
    ...
    while uint8(idx) < 9:
        if stor5[uint8(idx)].field_160:
            ...
            emit 0xf2f3fe78: msg.sender, msg.value
        else:
            require uint8(idx) != 9
            stor31 += stor30
            require uint8(idx) < 9
