...
function _fallback() {
    mem[96 len -8276] = code.data[8623 len -8276]
    mem[64] = -8180
...
function kill(address arg1) {
    ...
    call tokenCtrAddress.transferStop() with:
...
function withdrawReserve(address arg1) {
    ...
    call tokenCtrAddress.creationTime() with:
         gas gas_remaining - 710 wei
    require ext_call.success
    if ext_call.return_data[0] + (1227 * 3600) >= block.timestamp:
    require ext_code.size(tokenCtrAddress)
    call tokenCtrAddress.mintReserve(address rg1) with:
         gas gas_remaining - 710 wei
        args arg1
    require ext_call.success
}
...
function _fallback() payable {
    ...
    require ext_code.size(tokenCtrAddress)
    call tokenCtrAddress.currentSwapRate() with:
         gas gas_remaining - 710 wei
    require ext_call.success
    if msg.value:
        require msg.value
    ...
    require ext_code.size(tokenCtrAddress)
    call tokenCtrAddress.tokenCap() with:
         gas gas_remaining - 710 wei
    require ext_call.success
    require ext_code.size(tokenCtrAddress)
    call tokenCtrAddress.0x18160ddd with:
