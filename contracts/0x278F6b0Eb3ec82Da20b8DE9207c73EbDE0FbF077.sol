...
function _fallback() {
    mem[96 len -6188] = code.data[6515 len -6188]
    mem[64] = -6092
...
const SWAP_LENGTH = (1392 * 24 * 3600)

const MAX_ETH = 800000 * 10^18
...
function _fallback() payable {
    require msg.value
    require amountRaised + msg.value <= 800000 * 10^18
    require tokenSwap
    require ext_code.size(tokenCtrAddress)
    call tokenCtrAddress.creationTime() with:
         gas gas_remaining - 50 wei
    require ext_call.success
    if ext_call.return_data[0] + (1392 * 24 * 3600) < block.timestamp:
        require ext_code.size(tokenCtrAddress)
