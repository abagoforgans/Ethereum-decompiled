...
function claim() payable {
    require block.number >= 3910000
    require ext_code.size(tokenAddress)
    call tokenAddress.0x70a08231 with:
         gas gas_remaining - 710 wei
        args this.address
    require ext_call.success
    require ext_call.return_data[0] - total_snt_claimed >= 2 * (-3910000 * msg.value) + (block.number * msg.value)
    snt_claimed[address(msg.sender)] += 2 * (-3910000 * msg.value) + (block.number * msg.value)
    total_snt_claimed += 2 * (-3910000 * msg.value) + (block.number * msg.value)
    call stor3 with:
       value msg.value wei
