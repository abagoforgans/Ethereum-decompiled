...
function _fallback() payable {
    ...
    stor1.length = code.data[4078 len 32]
    stor2.length = code.data[4110 len 32]
    stor3 = code.data[4142 len 32]
    emit ChangeOfRules(code.data[4078 len 32], code.data[4110 len 32], code.data[4142 len 32]);
    stor1.length = code.data[4078 len 32]
    stor2.length = code.data[4110 len 32]
    stor3 = code.data[4142 len 32]
    stor7++
    if not stor7 <= stor7 + 1:
        idx = (3 * stor7) + code.data[4046 len 32] + 3
        while code.data[4046 len 32] + (3 * stor7) > idx:
            address(stor[idx]) = 0
            ...
            continue 
    require 0 < stor7
    address(stor[code.data[4046 len 32]]) = 0
    bool(stor1[code.data[4046 len 32]].field_0) = 0
    uint255(stor1[code.data[4046 len 32]].field_1) = 0
    Mask(248, 0, stor1[code.data[4046 len 32]].field_8) = mem[320 len 31]
    idx = 0
    while stor[code.data[4046 len 32] + 1].length + 31 / 32 > idx:
        uint256(stor[idx + sha3(code.data[4046 len 32] + 1)]) = 0
        idx = idx + 1
        continue 
    stor2[code.data[4046 len 32]] = block.timestamp
    uint256(stor0) = msg.sender or Mask(96, 160, uint256(stor0))
    return code.data[661 len 3385]
}
