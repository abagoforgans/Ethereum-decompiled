...
const _fallback = code.data[18 len 695]
...
function _fallback() payable {
    ...
    emit code.data[663 len 32]: uint8(cd[1])
    emit code.data[663 len 32]: 0
    require 0 < calldata.size
    emit code.data[663 len 32]: 1
    idx = 0
    while idx < 32:
        require idx + 1 < calldata.size
        require idx < 32
        mem[(32 * idx) + 96] = Mask(8, 248, cd[(idx + 1)])
        require idx + 1 < calldata.size
        require idx < 32
        mem[(32 * idx) + 1120] = Mask(8, 248, cd[(idx + 1)])
        idx = idx + 1
        continue 
    emit code.data[663 len 32]: 2
}
