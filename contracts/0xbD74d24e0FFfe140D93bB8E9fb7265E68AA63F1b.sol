...
function uintToBytesForAddress(uint256 arg1) {
    mem[128 len 640] = code.data[5566 len 640]
    idx = 0
    while idx < 20:
        require 2^(8 * -idx + 31)
        require idx < 20
...
function popFirstFromArray(address[] arg1) {
    ...
    if arg1.length - 1:
        mem[(32 * arg1.length) + 160 len 32 * arg1.length - 1] = code.data[5566 len 32 * arg1.length - 1]
    idx = 0
...
function mintForwarder(uint256 arg1, bytes32 arg2, address[] arg3) {
    ...
    if arg3.length == 1:
        ...
        require ext_code.size(mem[140 len 20])
        call mem[140 len 20].0x8da5cb5b with:
             gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        ...
        mem[(32 * arg3.length) + 160 len 640] = code.data[5566 len 640]
        idx = 0
        while idx < 20:
            require 2^(8 * -idx + 31)
            require idx < 20
        ...
        if arg3.length - 1:
            mem[(32 * arg3.length) + 160 len 32 * arg3.length - 1] = code.data[5566 len 32 * arg3.length - 1]
        idx = 0
        ...
        require ext_call.return_data[0] == ext_call.return_data[0] + 1
        require ext_code.size(address(_8))
        call address(_8).0x8da5cb5b with:
             gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        require address(ext_call.return_data[0]) == owner
        require ext_code.size(kingContractAddress)
        call kingContractAddress.getKing() with:
             gas gas_remaining wei
        if not ext_call.success:
        ...
        mem[(32 * arg3.length) + (32 * arg3.length - 1) + 192 len 640] = code.data[5566 len 640]
        idx = 0
        while idx < 20:
            require 2^(8 * -idx + 31)
            require idx < 20
