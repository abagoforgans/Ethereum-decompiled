...
const identifier = 'Asset whitelist'
...
function sub_08a6d790(?) {
    ...
    return bool(stor2[address(arg3)])
}

function sub_262ad50d(?) {
    idx = 0
...
function removeFromWhitelist(address arg1) {
    ...
    if not stor2[address(arg1)]:
        revert with 0, 'Asset not in whitelist'
    ...
    while idx < stor3.length - 1:
        ...
        mem[0] = 3
        address(stor3[idx].field_0) = address(stor3[idx].field_256)
        idx = idx + 1
        continue 
    stor3.length--
    if stor3.length > stor3.length - 1:
