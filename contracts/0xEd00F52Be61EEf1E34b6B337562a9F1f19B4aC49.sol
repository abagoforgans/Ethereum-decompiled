...
function participants(uint256 arg1) payable {
    ...
    return address(stor5629[arg1]), stor[code.data[1122 len 32] + (2 * arg1)]
}
...
function enter() payable {
    if msg.value < 21 * 10^15:
        call msg.sender with:
           value msg.value wei
             gas 0 wei
    if msg.value <= 88 * 10^18:
        stor0.length++
        if not stor0.length > stor0.length + 1:
            ...
            stor[code.data[1122 len 32] + (2 * stor0.length)] = msg.value
            if not stor0.length:
                collectedFees += msg.value
            else:
                collectedFees += msg.value / 100
                balance += msg.value
            if balance <= 133 * stor5629[stor1] / 100:
            require payoutIdx < stor0.length
            call address(stor5629[stor1]) with:
               value var25003 wei
                 gas 0 wei
            balance += -133 * stor[code.data[1122 len 32] + (2 * stor1)] / 100
            payoutIdx++
            if balance <= 133 * stor5629[stor1] / 100:
            require payoutIdx < stor0.length
            call address(stor5629[stor1]) with:
        ...
        call msg.sender with:
           value msg.value - 88 * 10^18 wei
             gas 0 wei
        stor0.length++
        if not stor0.length > stor0.length + 1:
            require stor0.length < stor0.length
            uint256(stor5629[stor0.length]) = msg.sender or Mask(96, 160, uint256(stor5629[stor0.length]))
            stor[code.data[1122 len 32] + (2 * stor0.length)] = 88 * 10^18
            if not stor0.length:
                collectedFees += 88 * 10^18
            else:
                collectedFees += 88 * 10^16
                balance += 88 * 10^18
            if balance <= 133 * stor5629[stor1] / 100:
            require payoutIdx < stor0.length
            call address(stor5629[stor1]) with:
