...
function mintInfo(uint256 arg1) {
    if arg1 >= stor21:
        revert with 0, 'categoryIndex does not exist'
    return stor29[msg.sender], stor30[arg1]
}

function sub_cf153e5c(?) {
    if arg1 >= stor21:
        revert with 0, 'categoryIndex does not exist'
    if stor29[msg.sender] < stor30[arg1]:
        return (stor30[arg1] - stor29[msg.sender])
    else:
        return 0
}

function sub_55f69442(?) {
    if not stor22[arg1]:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'No category exists with this name.'
    return stor29[msg.sender], stor30[stor22[arg1] - 1]
}

function sub_73569f62(?) {
    if not stor22[arg1]:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'No category exists with this name.'
    if stor29[msg.sender] < stor30[stor22[arg1] - 1]:
        return (stor30[stor22[arg1] - 1] - stor29[msg.sender])
    else:
        return 0
}
...
function sub_a7999238(?) {
    if stor27[msg.sender] != 1:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'Only a minter can call this function.'
    if not address(cd[4]):
        revert with 0, 'Owner cannot be the 0 address.'
    uint256(stor6)++
    if uint256(stor6) >= 4294967296:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'Only 4,294,967,296 mokens can be created.'
    if not stor22[cd[68]]:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'No category exists with this name.'
    if stor29[address(cd[4])] < stor30[stor22[cd[68]] - 1]:
        revert with 0x8c379a0000000000000000000000000000000000000000000000000000000, 'Paid ether is lower than mint price.'
    mem[0] = address(cd[4])
    mem[32] = 29
    ...
    while idx < ('cd', 36).length + 128:
        ...
        s = Mask(8, -(('mask_shl', 256, 0, -3, ('var', '_97')), 0) + 256, 0) << (('mask_shl', 256, 0, -3, ('var', '_97')), 0) - 256
        idx = idx + 1
        continue 
    _99 = mem[96]
    ...
    mem[mem[64] + _99] = 28
    if stor[sha3(mem[mem[64] len _99 + 32])]:
    ...
    mem[_302 + mem[64]] = 28
    stor[sha3(mem[mem[64] len _302 + _301 + -mem[64] + 32])] = uint256(stor6) + 1
    ...
    mem[mem[64]] = 160
    mem[_324 + 160] = ('cd', 36).length
    mem[_324 + 192 len ('cd', 36).length] = call.data[cd[36] + 36 len ('cd', 36).length]
    emit Mint(address arg1, address arg2, bytes32 arg3, string arg4, bytes32 arg5, uint256 arg6, bytes32 arg7, uint256 arg8):
              mem[mem[64] len _324 + ('cd', 36).length + -mem[64] + 192],
              this.address,
              address(cd[4]),
              cd[68],
