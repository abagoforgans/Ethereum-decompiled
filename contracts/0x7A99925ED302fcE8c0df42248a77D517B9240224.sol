...
function _fallback() {
    stor0 = msg.sender
    stor1 = code.data[2626 len 20]
    stor2 = code.data[2658 len 20]
    stor3 = code.data[2678 len 32]
    stor4 = code.data[2710 len 32]
    stor5 = code.data[2742 len 32]
    uint8(stor6.field_0) = uint8(bool(code.data[2774 len 32]))
    Mask(248, 0, stor6.field_8) = Mask(248, 0, bool(code.data[2806 len 32]))
    Mask(240, 0, stor6.field_16) = Mask(240, 16, bool(code.data[2774 len 32])) >> 16
    require stor0 == msg.sender
    emit OwnershipTransferred(stor0, code.data[2626 len 20]);
    stor0 = code.data[2626 len 20]
    return code.data[370 len 2244]
}
    ...
    if uint8(stor6.field_0):
        if owner != msg.sender:
            ...
            if arg1 / units <= eth.balance(this.address) / buyPrice:
                require arg1 / units > 0
                require ext_code.size(assetAddress)
                call assetAddress.0x23b872dd with:
                     gas gas_remaining - 710 wei
                    args msg.sender, owner, arg1 / units * units
                require ext_call.success
                ...
                require eth.balance(this.address) / buyPrice > 0
                require ext_code.size(assetAddress)
                call assetAddress.0x23b872dd with:
                     gas gas_remaining - 710 wei
                    args msg.sender, owner, eth.balance(this.address) / buyPrice * units
                require ext_call.success
                require ext_call.return_data[0]
                call msg.sender with:
        ...
        if owner == msg.sender:
            ...
            if msg.value <= ext_call.return_data[0] / units * sellPrice:
                ...
                if ext_call.return_data[0] / units <= 0:
                    call msg.sender with:
                       value ext_call.return_data[0] / units * sellPrice wei
                         gas 2300 * is_zero(value) wei
                else:
                    require ext_code.size(assetAddress)
                    call assetAddress.0xa9059cbb with:
