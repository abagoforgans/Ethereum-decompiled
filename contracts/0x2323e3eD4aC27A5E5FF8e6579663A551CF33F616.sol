...
function sub_a1a2385c(?) {
    totalSupply = 100 * 10^6 * 10^decimals
    balanceOf[msg.sender] = 100 * 10^6 * 10^decimals
    uint8(name.length) = 26
    name.length.field_8 = 'tokenPrcctest' / 256
    idx = 0
    while name.length + 31 / 32 > idx:
        name[idx].field_0 = 0
        idx = idx + 1
        continue 
    uint8(symbol.length) = 14
    symbol.length.field_8 = 'Prctest' / 256
    idx = 0
