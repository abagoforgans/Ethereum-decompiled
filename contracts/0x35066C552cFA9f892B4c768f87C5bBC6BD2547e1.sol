...
function sub_2c766cc6(?) {
    ...
    while idx < arg2 + tokenByIndex.length:
        require arg1
        require not ownerOf[idx]
        ownerOf[idx] = arg1
        require balanceOf[address(arg1)] + 1 >= balanceOf[address(arg1)]
        balanceOf[address(arg1)]++
        tokenOfOwnerByIndex[address(arg1)]++
        tokenOfOwnerByIndex[address(arg1)][tokenOfOwnerByIndex[address(arg1)]] = idx
        stor6[idx] = tokenOfOwnerByIndex[address(arg1)]
        emit Transfer(0, arg1, idx);
        stor8[idx] = tokenByIndex.length
        tokenByIndex.length++
        tokenByIndex[tokenByIndex.length] = idx
        require ownerOf[idx]
        mem[32] = 11
        mem[0] = sha3(idx, 11)
        uint256(tokenURI[idx]) = (2 * arg3.length) + 1
        t = sha3(sha3(idx, 11))
        s = 128
        while arg3.length + 128 > s:
            ...
            s = s + 32
            continue 
        s = sha3(sha3(s, 11)) + (Mask(251, 0, arg3.length + 31) >> 5)
        while sha3(sha3(s, 11)) + (tokenURI[s].length + 31 / 32) > s:
            stor[s] = 0
            s = s + 1
            continue 
        s = sha3(sha3(s, 11)) + (tokenURI[s].length + 31 / 32) + 1
        continue 
    emit 0xfc0b4f7b: arg2, tokenByIndex.length, idx, arg1
    return 1
}
