...
function participants(uint256 arg1) payable {
    ...
    return address(stor[(2 * arg1) + code.data[1065 len 32]]), uint256(stor290D[arg1].field_0)
}
...
function NextPayout() payable {
    ...
    if balance >= uint256(stor290D[stor1].field_0):
        require payoutIdx < stor0.length
        call address(stor[code.data[1065 len 32] + (2 * stor1)]) with:
...
function enter() payable {
    if msg.value < 10^17:
        ...
        uint256(stor[code.data[1065 len 32] + (2 * stor0.length)]) = msg.sender or Mask(96, 160, uint256(stor[code.data[1065 len 32] + (2 * stor0.length)]))
        bool(stor290D[stor0.length].field_0) = 0
        uint255(stor290D[stor0.length].field_1) = uint255(msg.value - (msg.value / 20))
        if balance >= 2 * msg.value - (msg.value / 20):
            require payoutIdx < stor0.length
            call address(stor[code.data[1065 len 32] + (2 * stor1)]) with:
               value 2 * msg.value - (msg.value / 20) wei
                 gas 0 wei
            balance -= 2 * msg.value - (msg.value / 20)
            payoutIdx++
}
