...
function minPricePerEdition() {
    return minPricePerEdition
}
...
function setMinPricePerEdition(uint256 arg1) {
    require msg.sender == owner
    minPricePerEdition = arg1
}
...
function isEnabledForAccount(address arg1) {
    ...
    call accessControlsAddress.0x2bd3152d with:
...
function createEditionFor(address arg1, uint256 arg2, uint256 arg3, uint256 arg4, string arg5, bool arg6) {
    ...
    if arg3 < minPricePerEdition:
        revert with 0, 'Price must be greater than minimum'
    if owner != msg.sender:
        require ext_code.size(accessControlsAddress)
        call accessControlsAddress.0xe29cda48 with:
             gas gas_remaining wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        require return_data.size >= 32
        if not ext_call.return_data[0]:
            require ext_code.size(accessControlsAddress)
            call accessControlsAddress.0x46f7c98a with:
                 gas gas_remaining wei
                args arg1
            if not ext_call.success:
                revert with ext_call.return_data[0 len return_data.size]
...
function createEdition(uint256 arg1, uint256 arg2, uint256 arg3, string arg4, bool arg5) {
    ...
    if not stor7:
        if not sub_2ec8d5bb[msg.sender]:
            ...
            sub_2ec8d5bb[msg.sender] = block.timestamp
        else:
            if block.timestamp - sub_2ec8d5bb[msg.sender] >= sub_b83281b0:
                sub_43ce3354[msg.sender] = 1
                sub_2ec8d5bb[msg.sender] = block.timestamp
            else:
                if sub_43ce3354[msg.sender] + 1 > maxInvocations:
