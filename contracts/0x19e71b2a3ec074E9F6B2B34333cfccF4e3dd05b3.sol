...
function _fallback() payable {
    mem[96 len -2341] = code.data[2782 len -2341]
    mem[64] = -2245
...
function reFundByOther(address arg1) payable {
    ...
    totalSupply = sub_b8491317 + balanceOf[stor2].field_0
...
function _fallback() payable {
    ...
    require block.timestamp <= stor10
    if stor10 - (336 * 24 * 3600) > block.timestamp:
        ...
        idx = (block.timestamp + -stor10 + (336 * 24 * 3600) / 48 * 24 * 3600) + 1
        s = msg.value
        while idx > 0:
            idx = idx - 1
            s = 95 * s / 100
