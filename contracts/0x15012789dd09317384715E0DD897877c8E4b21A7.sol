...
function _fallback() {
    stor0 = msg.sender
    stor3 = code.data[2161 len 20]
    return code.data[112 len 2037]
}
...
address stor3;
uint256 lockedFunds;
uint256 lockedFundsReleaseTime;

function getLockedFunds() {
    return lockedFunds
}

function getLockedFundsReleaseTime() {
    return lockedFundsReleaseTime
}
...
function payOut(address arg1) {
    if contractOwner != msg.sender:
        return 0
    if block.timestamp < lockedFundsReleaseTime:
        return 51013
    if not lockedFunds:
        return 51012
...
function payIn() {
    ...
    require ext_call.success
    if lockedFunds:
        if ext_call.return_data[0] == lockedFunds:
            return 51017
    else:
        if not ext_call.return_data[0]:
            return 51012
        lockedFunds = ext_call.return_data[0]
        lockedFundsReleaseTime = 423229 * 3600
    return 1
}
