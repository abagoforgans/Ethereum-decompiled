...
function _fallback() payable {
    ...
    stor0 = msg.sender
    stor1 = code.data[2949 len 20]
    return code.data[120 len 2817]
}
...
function depositDividend() payable {
    ...
    call tokenAddress.0x981b24d0 with:
...
function claimDividend(uint256 arg1) {
    ...
    require ext_code.size(tokenAddress)
    call tokenAddress.0x4ee2cd7e with:
         gas gas_remaining - 710 wei
    ...
    if arg1 == stor4[address(msg.sender)]:
        require arg1 + 1 >= arg1
    ...
    while idx < dividends.length:
        ...
        if not uint8(stor[(7 * idx) + ('name', 'dividends', 3) + 6][address(msg.sender)].field_0):
            ...
            if not uint8(dividends[idx].field_1280):
                ...
                mem[0] = msg.sender
                mem[32] = 4
                if idx == stor4[address(msg.sender)]:
                    require idx + 1 >= idx
                    mem[0] = msg.sender
                    mem[32] = 4
