...
uint256 rwGas;
uint256 optInXferGas;
uint256 optInFcnMinGas;
uint256 vestTime;
uint8 settingsState;
...
function optInXferGas() {
    return optInXferGas
}

function getAccountInfo(address arg1) {
    return balanceOf[address(arg1)].field_256, balanceOf[address(arg1)].field_768, balanceOf[address(arg1)].field_512
}
...
function optInFcnMinGas() {
    return optInFcnMinGas
}
...
function optInFromClassic() {
    if not oldE4Address:
        ...
        if balanceOf[address(msg.sender)].field_256:
            emit StatEvent(Array(len=31, data='Account has already has tokens!'));
        else:
            ...
            if not ext_call.return_data[0]:
                ...
                if ext_call.return_data[0] < ext_call.return_data[0]:
                    ...
                    require gas_remaining >= optInXferGas + optInFcnMinGas
                    require ext_code.size(oldE4Address)
                    call oldE4Address.0x23b872dd with:
                         gas optInXferGas wei
