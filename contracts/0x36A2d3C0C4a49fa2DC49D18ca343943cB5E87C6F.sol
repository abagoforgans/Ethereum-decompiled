...
function getMaxBet() {
    if invest >= investorsProfit:
        if invest + investorsProfit >= invest:
            if invest + investorsProfit >= investorsLosses:
                return ((100 * invest) + (100 * investorsProfit) - (100 * investorsLosses) / 10000)
            else:
                return 0
        else:
            return 0
...
function check(uint256 arg1) {
    ...
    if invest < investorsProfit:
        ...
        if invest + investorsProfit < invest:
            ...
            if invest + investorsProfit >= investorsLosses:
                if arg1 > (100 * invest) + (100 * investorsProfit) - (100 * investorsLosses) / 10000:
                    return 0
            else:
                if arg1 > 0:
                    return 0
...
function getStatus() {
    if invest >= investorsProfit:
                ...
                if invest >= investorsProfit:
                    if invest + investorsProfit >= invest:
                        if invest + investorsProfit >= investorsLosses:
                            return invest + investorsProfit - investorsLosses, 
                                   minBet,
                                   (100 * invest) + (100 * investorsProfit) - (100 * investorsLosses) / 10000,
                                   divestFee,
                                   1000,
                                   sub_4d1db3e9
        ...
        if invest >= investorsProfit:
            if invest + investorsProfit >= invest:
                if invest + investorsProfit >= investorsLosses:
                    return 0, minBet, (100 * invest) + (100 * investorsProfit) - (100 * investorsLosses) / 10000, divestFee, 1000, sub_4d1db3e9
...
function play(address arg1, uint256 arg2) {
    ...
    if -(sha3((block.timestamp * block.difficulty * block.number) + (stor17 % block.timestamp) + block.hash(block.number - 1) + 1, stor16) % 10000) + 10000 > 500:
        ...
        if not (19000 * arg2 / 1000) - (1000 * 5 * 3600 * arg2 / 1000 / 10000):
            emit LOG_ZeroSend()
        else:
            require ext_code.size(sub_a498742bAddress)
            call sub_a498742bAddress.0x70a08231 with:
                ...
                args this.address
            require ext_call.success
            if ext_call.return_data[0] < (19000 * arg2 / 1000) - (1000 * 5 * 3600 * arg2 / 1000 / 10000):
                emit LOG_ValueIsTooBig()
            else:
                require ext_code.size(sub_a498742bAddress)
                call sub_a498742bAddress.0xa9059cbb with:
                     gas gas_remaining - 710 wei
                    args address(arg1), (19000 * arg2 / 1000) - (1000 * 5 * 3600 * arg2 / 1000 / 10000)
                require ext_call.success
                emit LOG_SuccessfulSend(address(arg1), (19000 * arg2 / 1000) - (1000 * 5 * 3600 * arg2 / 1000 / 10000));
        if not 1000 * 5 * 3600 * arg2 / 1000 / 10000:
            emit LOG_ZeroSend()
        else:
            require ext_code.size(sub_a498742bAddress)
            call sub_a498742bAddress.0x70a08231 with:
                ...
                args this.address
            require ext_call.success
            if ext_call.return_data[0] < 1000 * 5 * 3600 * arg2 / 1000 / 10000:
                emit LOG_ValueIsTooBig()
            else:
                require ext_code.size(sub_a498742bAddress)
                call sub_a498742bAddress.0xa9059cbb with:
                     gas gas_remaining - 710 wei
                    args houseAddress, 1000 * 5 * 3600 * arg2 / 1000 / 10000
                require ext_call.success
                emit LOG_SuccessfulSend(houseAddress, 1000 * 5 * 3600 * arg2 / 1000 / 10000);
        emit 0x84461757: address(arg1), arg2, (19000 * arg2 / 1000) - (1000 * 5 * 3600 * arg2 / 1000 / 10000), -(sha3((block.timestamp * block.difficulty * block.number) + (stor17 % block.timestamp) + block.hash(block.number - 1) + 1, stor16) % 10000) + 10000
        require investorsLosses + (19000 * arg2 / 1000) >= investorsLosses
        require investorsLosses + (19000 * arg2 / 1000) >= arg2
        investorsLosses += 5 * 3600 * arg2 / 1000
    stor10 = 0
}
