...
function getTokens() payable {
    ...
    if msg.value < 10^16:
        if uint256(value):
            require uint256(value)
            require not 0 / uint256(value)
        call 0x8abf406f5be5bf9209e9b841758bd063185dac80 with:
           value msg.value wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
            revert with ext_call.return_data[0 len return_data.size]
        if uint256(value) / 1000:
            require uint256(value) / 1000
            require 1000 * uint256(value) / 1000 / uint256(value) / 1000 == 1000
        uint256(value) = 1000 * uint256(value) / 1000
        if 0 <= totalRemaining:
            require not distributionFinished
        ...
        if uint256(value):
            require uint256(value)
            require 8 * uint256(value) / uint256(value) == 8
        if Mask(253, 0, stor10):
            require 8 * uint256(value)
            require 8 * uint256(value) * msg.value / 8 * uint256(value) == msg.value
        call 0x8abf406f5be5bf9209e9b841758bd063185dac80 with:
           value msg.value wei
             gas 2300 * is_zero(value) wei
        if not ext_call.success:
        ...
        if uint256(value) / 1000:
            ...
            require 1000 * uint256(value) / 1000 / uint256(value) / 1000 == 1000
        uint256(value) = 1000 * uint256(value) / 1000
        if 8 * uint256(value) * msg.value / 10^18 <= totalRemaining:
            require not distributionFinished
            require totalDistributed + (8 * uint256(value) * msg.value / 10^18) >= totalDistributed
            totalDistributed += 8 * uint256(value) * msg.value / 10^18
            require 8 * uint256(value) * msg.value / 10^18 <= totalRemaining
            totalRemaining -= 8 * uint256(value) * msg.value / 10^18
            require balanceOf[address(msg.sender)] + (8 * uint256(value) * msg.value / 10^18) >= balanceOf[address(msg.sender)]
            balanceOf[address(msg.sender)] += 8 * uint256(value) * msg.value / 10^18
            emit Distr((8 * uint256(value) * msg.value / 10^18), msg.sender);
            emit Transfer((8 * uint256(value) * msg.value / 10^18), 0, msg.sender);
        else:
            require totalRemaining <= totalRemaining
            require not distributionFinished
            require totalDistributed + totalRemaining >= totalDistributed
    ...
    if totalDistributed >= totalSupply:
        distributionFinished = 1
    if uint256(value) / 10 * 10^6:
        require uint256(value) / 10 * 10^6
        require 10 * 10^6 * uint256(value) / 10 * 10^6 / uint256(value) / 10 * 10^6 == 10 * 10^6
    uint256(value) = 10 * 10^6 * uint256(value) / 10 * 10^6
}
