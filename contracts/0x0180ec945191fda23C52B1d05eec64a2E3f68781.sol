...
function _fallback() payable {
    ...
    require 4465500 > block.number
    return code.data[779 len 2349]
}
...
const WITHDRAWAL_END = 4469000

const MAX_TOTAL_AMOUNT_TO_RECEIVE_ETH = 1

const PRESALE_START = 4465500

const PRESALE_END = 4466550

const VERSION = '0.2.0-bns-test-03-max_1_eth'
...
function abort() {
    ...
    if block.number >= 4465500:
        if block.number > 4466550:
            require eth.balance(this.address)
            require block.number <= 4469000
            require total_received_amount >= 0
        else:
            if total_received_amount >= 10^18:
                require eth.balance(this.address)
                require block.number <= 4469000
                require total_received_amount >= 0
            else:
        ...
        if block.number > 4466550:
            ...
            if total_received_amount >= 10^18:
                ...
                if not uint8(stor8.field_8):
                    ...
                    if total_received_amount + msg.value <= 10^18:
                        balances[address(msg.sender)] += msg.value
                        total_received_amount += msg.value
                    else:
                        balances[address(msg.sender)] = -total_received_amount + balances[address(msg.sender)] + 10^18
                        total_received_amount = 10^18
                        call msg.sender with:
                           value total_received_amount + msg.value - 10^18 wei
                             gas 2300 * is_zero(value) wei
                        require ext_call.success
                else:
                    require eth.balance(this.address)
