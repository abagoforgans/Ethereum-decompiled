...
function _fallback() payable {
    mem[96 len -3099] = code.data[3644 len -3099]
    mem[64] = -2939
    stor4DB7 = 1
    stor1 = mem[96]
...
function remove() payable {
    ...
    emit 0x56d36fc8: 1, Array(len=35, data='[Identity][remove] Identity Remo', 'ved')
...
function sub_fc080f62(?) payable {
    if 1 == stor0[tx.origin]:
        stor0[arg1] = arg2
        emit 0x56d36fc8: 1, Array(len=51, data='[Identity][amendOtoVEAdmin] amen', 'dOtoVEAdmin changed')
}

function sub_c2a0f88f(?) payable {
    call address(sub_8666ea1dAddress).0xcc4bd7c1 with:
         gas gas_remaining - 25050 wei
        args address(owner)
    require ext_call.success
    return ext_call.return_data[0]
...
function sub_538de0ec(?) payable {
    if 1 == stor0[tx.origin]:
        ...
        emit 0x56d36fc8: 1, Array(len=45, data='[Identity][setDefaultVE] setDefa', 'ultVE changed')
}

function setOwner(address arg1) payable {
    if tx.origin == address(owner):
        uint256(stor2) = arg1 or Mask(96, 160, uint256(stor2))
        emit 0x56d36fc8: 1, Array(len=37, data='[Identity][setOwner] setOwner ch', 'anged')
}
...
function sub_ff3214af(?) payable {
    if tx.origin == address(owner):
        if not arg1:
            ...
            emit 0x56d36fc8: 1, Array(len=42, data='[Identity][setData] setData idDa', 'ta changed')
}
