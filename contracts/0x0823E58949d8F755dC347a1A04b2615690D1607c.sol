...
function _fallback() payable {
    ...
    mem[96 len -7074] = code.data[7613 len -7074]
    mem[64] = -6978
    stor0 = msg.sender
...
function freezeAll(bool arg1) {
    ...
    while idx < stor10.length:
        ...
        mem[32] = 9
        stor9[address(stor10[idx])] = uint8(arg1)
        mem[96] = address(stor10[idx])
        mem[128] = arg1
        emit FrozenFunds(address(stor10[idx]), arg1);
        idx = idx + 1
        continue 
}
...
function sub_b35fd678(?) {
    ...
    require arg1
    require balanceOf[address(msg.sender)] > 10^16 * arg2
    require balanceOf[address(arg1)] + (10^16 * arg2) > balanceOf[address(arg1)]
    require not stor9[address(msg.sender)]
    require not stor9[address(arg1)]
    balanceOf[address(msg.sender)] += -1 * 10^16 * arg2
    balanceOf[address(arg1)] += 10^16 * arg2
    emit Transfer((10^16 * arg2), msg.sender, arg1);
}
    ...
    while idx < stor10.length:
        ...
        if not stor10.length <= stor10.length + 1:
            ...
            while sha3(10) + stor10.length > s:
                ...
                s = s + 1
                continue 
        address(stor10[stor10.length]) = mem[(32 * idx) + 140 len 20]
