...
function _fallback() payable {
    ...
    if not msg.value:
        stor0 = msg.sender
        require code.data[4978 len 20]
        require code.data[5010 len 20]
        stor3 = code.data[4978 len 20]
        stor4 = code.data[5010 len 20]
        if ext_code.size(stor3):
            call stor3.getReserveBalance(address rg1) with:
                 gas gas_remaining - 710 wei
                args stor4
            if ext_call.success:
                return code.data[954 len 4012]
    revert
}
...
function _fallback() payable {
    ...
    call etherTokenAddress.deposit() with:
       value msg.value wei
